...
<body>
	...
	<style>
        .info-card {
            background: linear-gradient(45deg,#f2f5f4, #ebf5e6); /* สีเขียวอ่อนแบบ gradient */ 
            ...
            <div class="info-card bg-info" style="box-shadow: 5px 5px 10px rgba(0, 0, 0, 0.3);">
                ...
                <div style="text-align: right;">
                    <div style="text-align: right;">
                        ...
                        <a href="<?= Url::to(['f16dentures/run-curl', 'date1' => $date1, 'date2' => $date2]) ?>" 
					   class="btn btn-info" 
					   style="font-size: 16px; border-radius: 25px;">
						RunToken <i class="fa fa-arrow-circle-right"></i>
				    	</a>
						
                         <a href="<?= \yii\helpers\Url::to(['fdhdentures/index']) ?>" class="btn btn-warning modalLink" style="font-size: 16px;" target="_blank">
                        Query <i class="fa fa-arrow-circle-right"></i>
                    </a>
                    <?= Html::a('Export', ['f16dentures/exports'], ['class' => 'btn btn-success']) ?>
                    </div>
                    </div>
...
<div class="page-container">
    <div class="fixed-header">
      <?= Html::beginForm(['f16dentures/data'], 'post', ['name' => 'frmMain']); ?>
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        <table class="table my-striped-table" width="1000" border="0" bordercolor="#ddd" style="border-collapse: collapse; box-shadow: 2px 2px 5px rgba(0,0,0,0.2);">
            <thead>
                <tr>
                    <th width="30" style="background-color: #f2f2f2; position: sticky; top: 0; z-index: 1;">
                        <div align="center">
                    ...
                    <th width="150" style="background-color: #f2f2f2; position: sticky; top: 0; z-index: 1;">
                    ...
                    <th width="30" style="background-color: #f2f2f2; position: sticky; top: 0; z-index: 1;">
                        <div align="left" style="font-size: 14px;"> Code16 </div>
                    </th>
					<th width="30" style="background-color: #f2f2f2; position: sticky; top: 0; z-index: 1;">
                        <div align="left" style="font-size: 14px;"> chrgitem </div>
                    </th>
                    ...
                    <!-- คอลัมน์สำหรับปุ่มแก้ไข -->
                    <th width="30" style="background-color: #f2f2f2; position: sticky; top: 0; z-index: 1;">
                        <div align="center" style="font-size: 14px;"> แก้ไข </div>
                    </th>
                </tr>
            </thead>
            <tbody>
                ...
                foreach ($dataProvider->getModels() as $key => $value) : 
                    $diag = $value["Diag"];
                    $duplicate = $diagCounts[$diag] > 1;
                ?>
                    <tr style="background-color: <?php echo (empty($value['messagecode']) ? '#f9edfc' : '#e4f7f0'); ?>;">
                        <td>
                            <input type="checkbox" name="chkDel[]" id="chkDel<?= $i; ?>" value="<?php echo $value["visit_id"]; ?><?php echo $value["hn"]; ?>"style="width: 17px; height: 17px;">
                        </td>
                        ...
                        <td class="text-nowrap" style="font-size: 14px;"><?php echo $value["code"]; ?></td>
                        ...
                        <td class="text-nowrap" style="font-size: 14px;"><?php echo $value["code16"]; ?></td>
						 <td class="text-nowrap" style="font-size: 14px;"><?php echo $value["chrgitem"]; ?></td>
                        ...
                        <!-- ปุ่มแก้ไขในแต่ละแถว -->
                        <td>
    <a href="/m30-fdh/web/index.php?r=visitinvoice/index&visit_id=<?php echo $value['visit_id']; ?>" class="btn btn-primary edit-btn" style="font-size: 14px;" target="_blank" onclick="changeButtonColor(this);">
        แก้ไข
    </a>
</td>
...
function changeButtonColor(button) {
    ...
    button.style.color = 'white'; // เปลี่ยนสีข้อความเป็นขาว
}
</script>
...
$(document).ready(function() {
    $('.btn-edit').on('click', function() {
        ...
        $.ajax({
            url: '/m30-fdh/web/index.php?r=visitinvoice/index',
            type: 'GET',
