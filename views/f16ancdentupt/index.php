...
$this->title = 'ระบบบริหารจัดการข้อมูล [f16ancdentupt]';
...
/* ══ BADGE ROW (ปรับขนาดใหญ่พิเศษตามคำขอ) ═══════════════ */
.dashboard-badge-row {
    display: flex; flex-wrap: wrap; gap: 12px; margin-bottom: 22px;
}
.chip-badge {
    display: inline-flex; align-items: center; gap: 10px;
    padding: 14px 28px; border-radius: 14px; /* ปรับความกว้างและเหลี่ยมโค้งมนมนให้ดูแกร่งขึ้น */
    font-size: 15px; font-weight: 700; /* ขยายตัวอักษรและเพิ่มความหนา */
...
.chip-active { box-shadow: 0 0 0 4px rgba(108,72,255,.4) !important; border-width: 2px !important; }
...
.card-label { font-size: 12px; font-weight: 700; letter-spacing: .5px; text-transform: uppercase; margin-bottom: 4px; }
.card-value { font-size: 26px; font-weight: 700; color: #111827; line-height: 1.1; margin-bottom: 10px; }
.card-value small { font-size: 13px; font-weight: 400; color: #6b7280; }
...
.date-form-inner .form-control {
    ...
    font-size: 13px !important; height: 36px; width: 140px !important;
...
.quick-links-bar {
    ...
    gap: 5px; margin-bottom: 16px; font-size: 13px; flex-wrap: wrap;
}
.quick-links-bar a {
...
.kv-grid-table tbody td {
    ...
    vertical-align: middle !important; border-color: #f1f5f9 !important;
}
...
.d-update-text { font-size: 11px; color: #64748b; margin-top: 3px; font-weight: 400; }

/* ══ FLOATING BUTTON ════════════════ */
.floating-container { position: fixed; bottom: 36px; left: 50%; transform: translateX(-50%); z-index: 9999; }
.btn-floating {
    ...
    border: 3px solid #fff !important; transition: all .2s; letter-spacing: .2px;
}
.btn-floating:hover { transform: scale(1.04); box-shadow: 0 12px 28px rgba(34,197,94,.55); color: #fff !important; }

/* ══ DATEPICKER FIX ✅ ════════════ */
.ui-datepicker { z-index: 9999 !important; border-radius: 12px !important; box-shadow: 0 10px 30px rgba(0,0,0,.15) !important; border: 1px solid #e5e7eb !important; }
.ui-datepicker-header { background: linear-gradient(135deg,#6c48ff,#9d7bff) !important; border: none !important; color: #fff !important; border-radius: 8px 8px 0 0 !important; padding: 8px !important; }
...
.ui-datepicker td .ui-state-active { background: #6c48ff !important; border-color: #6c48ff !important; color: #fff !important; }
.ui-datepicker td .ui-state-hover { background: #ede9ff !important; color: #6c48ff !important; }
");
?>
...
<div class="anc-page-wrapper">
    ...
    <div class="anc-page-title">
        ...
        เงื่อนไข :: สิทธิ์บัตรทอง &nbsp;|&nbsp; รหัสโรค Z348 &nbsp;|&nbsp; หัตถการทำฟัน + UPT (Pregnancy test)
    </div>
    ...
    <div class="control-container">
        ...
        <div class="control-card card-teal">
            ...
            <a href="<?= Url::to(['f16ancdentupt/run-curl']) ?>" class="pill-btn pill-teal">
    ...
    <div class="quick-links-bar">
        ...
        <?= Html::a('<i class="fa fa-window-restore"></i> Query ในหน้านี้ (Modal)', '#', [
            ...
            'data-url' => Url::to(['/fdhancdentupt/index']),
        ]) ?>
        
        <span class="sep">|</span>
        <a href="<?= Url::to(['f16ancdentupt/exports', 'date1' => $date1, 'date2' => $date2]) ?>" class="ql-success">
            <i class="fa fa-file-excel-o"></i> Export Excel
        </a>
    </div>

    <?= Html::beginForm(['f16ancdentupt/data'], 'post', ['name' => 'frmMain']) ?>
    ...
    <div class="table-shell">
        <div class="grid-wrapper">
            <?= GridView::widget([
                ...
                'layout'       => '{items}',
                'emptyText'    => '<div class="text-center" style="padding:50px;color:#dc2626;font-weight:700;font-size:15px;"><i class="fa fa-exclamation-circle fa-2x" style="display:block;margin-bottom:10px;"></i>ไม่พบข้อมูลตามเงื่อนไขที่ระบุ</div>',
                'rowOptions'   => function($model) {
                    return ['class' => !empty($model['messagecode']) ? 'row-sent' : 'row-wait'];
                },
                'columns' => [
                    [
                        ...
                        'value'  => function($model) {
                            ...
                            return '<input type="checkbox" name="chkDel[]" value="' . Html::encode($val) . '" style="width:15px;height:15px;accent-color:#6c48ff;">';
                        },
                    ],
                    [
                        'header' => '#', 'width' => '46px', 'format' => 'raw',
                        'value'  => function($model, $key, $index) {
                            return '<span class="seq-badge">' . ($index + 1) . '</span>';
                        },
                    ],
                    ['attribute' => 'regdate',   'header' => 'วันที่',     'value' => function($m){ return $m['regdate']   ?? ''; }],
                   ### ['attribute' => 'visit_id',  'header' => 'เลขบริการ',  'value' => function($m){ return $m['visit_id']  ?? ''; }],
                    ['attribute' => 'hn',        'header' => 'HN',         'value' => function($m){ return $m['hn']        ?? ''; }],
                    ['attribute' => 'fullname',  'header' => 'ชื่อ-สกุล',  'value' => function($m){ return $m['fullname']  ?? ''; }],
                    ['attribute' => 'age',       'header' => 'อายุ',       'hAlign' => GridView::ALIGN_CENTER, 'value' => function($m){ return $m['age'] ?? ''; }],
                    ['attribute' => 'unit_name', 'header' => 'แผนก',       'value' => function($m){ return $m['unit_name'] ?? ''; }],
                    ['attribute' => 'Diagx',     'header' => 'โรคหลัก',    'value' => function($m){ return $m['Diagx']     ?? ''; }],
                    [
                        'attribute' => 'Diag', 'header' => 'รหัสโรค',
                        'format' => 'raw', 'hAlign' => GridView::ALIGN_CENTER,
                        'value' => function($model) {
                            $cls = !empty($model['messagecode']) ? 'tb-green' : 'tb-red';
                            return '<span class="tb ' . $cls . '">' . Html::encode($model['Diag'] ?? '') . '</span>';
                        },
                    ],
                   [
							'attribute' => 'oper',
							...
							'value'     => function ($model) {
								...
								// ตัดแสดงแค่ตัวแรก + ปุ่ม
								$first = explode(',', $oper)[0];
								$count = substr_count($oper, ',') + 1;
								$escaped = Html::encode($oper);
								return '
								<button type="button"
									class="btn-oper-detail"
									data-oper="' . $escaped . '"
									style="background:linear-gradient(135deg,#f3e8ff,#e9d5ff);
										   color:#6d28d9;border:1px solid #c4b5fd;border-radius:20px;
										   padding:3px 10px;font-size:11px;font-weight:bold;cursor:pointer;">
									<i class="fa fa-list"></i> ' . Html::encode($first) . '
									' . ($count > 1 ? '<span style="background:#6d28d9;color:#fff;border-radius:10px;padding:0 5px;font-size:10px;">+' . ($count-1) . '</span>' : '') . '
								</button>';
							},
						],
                    ['attribute' => 'inscl', 'header' => 'สิทธิ์',  'value' => function($m){ return $m['inscl'] ?? ''; }],
                    [
                        'attribute' => 'hospmain', 'header' => 'สถานหลัก',
                        'format' => 'raw', 'hAlign' => GridView::ALIGN_CENTER,
                        'value' => function($model) {
                            $val = $model['hospmain'] ?? '';
                    ...
                    ['attribute' => 'hospsub', 'header' => 'สถานรอง', 'value' => function($m){ return $m['hospsub'] ?? ''; }],
                    
                    [
                        'attribute' => 'amount', 'header' => 'ค่ารักษา',
                        'hAlign' => GridView::ALIGN_RIGHT, 'format' => 'raw',
                        'value' => function($model) {
                            $v = (float)($model['amount'] ?? 0);
                            return '<span class="tb ' . ($v == 0 ? 'tb-red' : 'tb-blue') . '">' . number_format($v, 2) . '</span>';
                        },
                    ],
                    [
                        'attribute' => 'ret_statement', 'header' => 'ชดเชย',
                        'hAlign' => GridView::ALIGN_RIGHT, 'format' => 'raw',
                        'value' => function($model) {
					...
					[
                        'attribute' => 'messagecode', 'header' => 'สถานะ',
                        'format' => 'raw', 'hAlign' => GridView::ALIGN_CENTER,
                        'value' => function($model) {
                            $msg     = $model['messagecode'] ?? '';
                            $dUpdate = $model['d_update']    ?? '';
                            if (!empty($msg)) {
                                $badge = '<span class="tb-pill tb-green"><span class="tb-icon icon-green">&#10003;</span>' . Html::encode($msg) . '</span>';
                            } else {
                                $badge = '<span class="tb-pill tb-red"><span class="tb-icon icon-red">&#10005;</span>รอส่ง</span>';
                            }
                            $time = !empty($dUpdate)
                                ? '<div class="d-update-text"><i class="fa fa-clock-o"></i> ' . Html::encode($dUpdate) . '</div>'
                                : '';
                            return '<div style="display:flex;flex-direction:column;align-items:center;">' . $badge . $time . '</div>';
                        },
                    ],
					[
					'label' => 'เช็คสถานะ',
                    ...
                    ['attribute' => 'claimcode',  'header' => 'authen',    'value' => function($m){ return $m['claimcode']  ?? ''; }],
                ],
            ]); ?>
        </div>
    </div>
    ...
    ?>
        <div class="floating-container">
            <?= Html::submitButton(
                '<i class="fa fa-paper-plane"></i> ส่งข้อมูล [ANC-DENT-UPT]',
...
<?php $this->registerJs(
    ...
    "var _url     = '" . Url::to(['f16ancdentupt/check']) . "';",
...
// ═══ 2. MODAL ใหม่สำหรับแสดงหน้าต่าง QUERY ระบบ fdhancdentupt ═══
Modal::begin([
    'id'     => 'queryDataModal',
    'header' => '<h4 style="font-weight:700;color:#6c48ff;margin:0;"><i class="fa fa-window-restore"></i> ข้อมูลระบบตรวจสอบแผนกเดนท์ [fdhancdentupt]</h4>',
    'size'   => Modal::SIZE_LARGE,
]);
