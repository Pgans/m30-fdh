...
<!-- จบปุ่มลอย -->
	 ...
	 <style>
        ...
        .btn-custom:hover {
            background: linear-gradient(45deg, #0099e6, #9900e6);
            transform: scale(1.05);
        }
    </style>
    <style>
...
<style>
    ...
    <style>
	 ...
	 <style>
        .info-card {
           background: linear-gradient(45deg, #d3fbf5, #e8bdfb); /* Pink gradient */####ชมพูอ่อน
...
.my-striped-table tbody tr:hover {
                ...
                <div style="text-align: right;">
                    <div style="text-align: right;">
                        ...
                        echo Html::a('เปิดอ่านไฟล์', '#', [
                            ...
                            'data-url' => \yii\helpers\Url::to(['f16ipreferinout/list-files-partial']), // URL สำหรับโหลดเนื้อหา
                        ]);
                        ?>
 ...
 <div class="col-xl-3 col-md-3 mb-3">
            <div class="info-card bg-warning" style="box-shadow: 5px 5px 10px rgba(0, 0, 0, 0.3);">
               ...
               <div style="text-align: right;">
                <a href="<?= Url::to(['f16ipreferinout/run-curl', 'date1' => $date1, 'date2' => $date2]) ?>" 
								   class="btn btn-info" 
        ...
        <a href="javascript:void(0);" id="exportExcelButton" class="btn btn-primary" style="font-size: 16px; border: 4px solid #91ffff;" onclick="openPopup();">
            รายชื่อและส่งออก ExcelFiles <i class="fa fa-arrow-circle-right"></i>
                    ...
                    <a href="<?= \yii\helpers\Url::to(['fdhipreferinout/index']) ?>" class="btn btn-warning modalLink" style="font-size: 16px;" target="_blank">
                        Query <i class="fa fa-arrow-circle-right"></i>
                    </a>
                    <?= Html::a('Export', ['f16ipreferinout/exports'], ['class' => 'btn btn-info']) ?>
...
<br>
    ...
    <div style="height: 600px; overflow-y: auto;">
        ...
        <?= Html::beginForm(['f16ipreferinout/data'], 'post', ['name' => 'frmMain']); ?>
		<!-- Hidden fields สำหรับเก็บวันที่ -->
				<?= Html::hiddenInput('date1', $date1) ?>
				<?= Html::hiddenInput('date2', $date2) ?>  

       <!-- Radio Buttons สำหรับเลือกสถานพยาบาล -->
<div style="padding: 15px; background: linear-gradient(to right, #f8f5fa , #f8f5fa); border: 1px solid #ddd; margin-bottom: 20px; display: flex; align-items: center; color: #9b59b6;">
    <label style="font-size: 18px; margin-right: 20px;">เลือกสถานพยาบาล:</label>
    <!-- เปลี่ยนจาก name="$referin" เป็น name="referin" -->
    <input type="radio" name="referin" id="sappasit" value="10669" checked required style="margin-right: 10px;">
    <label for="sappasit" style="margin-right: 20px;">สรรพสิทธิ์ (10669)</label>

    <input type="radio" name="referin" id="50years" value="21984" required style="margin-right: 10px;">
    <label for="50years">50พรรษา (21984)</label>
</div>
<?php
...
?>
    ...
    <div class="floating-button" 
        ...
        <button type="submit" 
            ...
            ส่งข้อมูล IP-REFERIN-OUT
        </button>
    </div>
<?php endif; ?>


    <!--<input name="btnButton1" class="btn btn-success btn btn-block" id="selectAll" type="submit" name="selectAll" value="ส่งข้อมูล IP REFERIN-OUT" style="background: linear-gradient(45deg, #f2dffa , #f2dffa); border: 4px solid #dadada; font-size: 16px; color: #8a4baf; box-shadow: 0px 4px 6px rgba(0, 0, 0, 0.1);">

        -->
       
  <style>
    .table-container {
        max-height: 500px; /* ปรับความสูงของตาราง */
    ...
    thead th {
        position: sticky;
        top: 0;
        background: #ccc; /* เปลี่ยนเป็นสีเทา */
        z-index: 10;
        border-bottom: 2px solid #000;
    }

    tbody tr:nth-child(even) {
        background-color: #f2f2f2; /* สีแถวสลับ */
    }
    ...
    <thead>
        ...
        <tbody>
            ...
            <tr style="background-color: <?php echo (empty($value['messagecode']) ? '#f4e9f7' : '#d4f4e8'); ?>;">
            <td><input type="checkbox" name="chkDel[]" id="chkDel<?= $i; ?>" value="<?php echo $value["visit_id"]; ?><?php echo $value["an"]; ?>" style="width: 17px; height: 17px;"></td>
            ...
            <td style="font-size: 14px;"><?php echo $value["cid"]; ?></td>
