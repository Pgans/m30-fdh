...
<body>
   ...
   <h4><a>เงื่อนไข:: </a> สิทธิ์บัตรทอง  ('03', '04', '33', '00', '23')  จ่าย Condom รหัสโรค z308 , Drug_id = 0282  ราคา 10 บาท</h4>
    <div class="row">
        ...
        <div class="col-xl-3 col-md-3 mb-3">
            <div class="info-card bg-info" style="box-shadow: 5px 5px 10px rgba(0, 0, 0, 0.3);">
                ...
                <div style="text-align: right;">
                    <a href="<?= Url::to(['f16condom/run-curl']) ?>" class="btn btn-info" style="font-size: 16px;">
        ...
        <div class="col-xl-3 col-md-3 mb-3">
            <div class="info-card bg-info" style="box-shadow: 5px 5px 10px rgba(0, 0, 0, 0.3);">
                ...
                <div class="info-box-content">
                    ...
                    <script>
                        function openPopup() {
                            const url = "<?= \yii\helpers\Url::to(['f16condom/exportexcel']); ?>"; // ลิงก์ไปยังไฟล์ Excel
                    ...
                    <a href="<?= \yii\helpers\Url::to(['fdhcondom/index']) ?>" class="btn btn-warning modalLink" style="font-size: 16px;" target="_blank">
                        Query <i class="fa fa-arrow-circle-right"></i>
                    </a>
					<?= Html::a('Export', ['f16condom/exports'], ['class' => 'btn btn-success']) ?>
                </div>
            </div>
        </div>
        <div class="col-xl-3 col-md-3 mb-3">
            <div class="info-card bg-info" style="box-shadow: 5px 5px 10px rgba(0, 0, 0, 0.3); width: 350px; height: 140px;">
                <div class="container">
                    <div class="row justify-content-center">
                        <div class="col-md-3">
                            <?php $form = ActiveForm::begin(['action' => ['f16condom/index']]); ?>
...
<style>
    .floating-buttons {
        position: fixed;
        bottom: calc(3 * 2rem); /* ระยะห่างจากด้านล่าง */
        left: 50%; /* เริ่มต้นที่กลาง */
        transform: translateX(-50%); /* ย้ายไปทางซ้ายเพื่อจัดกลาง */
        z-index: 1000;
...
<div class="floating-buttons">
    <?= Html::beginForm(['f16condom/data'], 'post', ['name' => 'frmMain']); ?>
        <button type="submit" 
                name="btnButton1" 
                id="selectAll" 
            ...
            ส่งข้อมูล CONDOM PCU
        </button>
    <?= Html::endForm(); ?>

    <?= Html::beginForm(['f16condom/data1'], 'post', ['name' => 'frmExport']); ?>
        <button type="submit" 
                name="btnButton2" 
                id="exportFiles" 
                class="btn btn-success" 
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        document.addEventListener("mouseup", () => {
            ...
            <input name="btnButton1" class="btn btn-success btn btn-block" id="selectAll" type="submit" value="ส่งข้อมูล FDH CONDOM PCU" style="background-color: #00b8c1; color: #FFFFFF; border: none; font-size: 20px; padding: 10px 20px; transition: all 0.3s;">

            <table class="table my-striped-table" width="1000" border="0" bordercolor="#ddd" style="border-collapse: collapse;">

                <td width="30" style="background-color: #00d6e1; color: #FFFFFF;">
                    <div align="center">
                ...
                <td width="30" style="background-color: #00d6e1; color: #FFFFFF;">
                    <div align="left" style="font-size: 14px;">รหัสยา
                </td>
                ...
                <?php foreach ($dataProvider->getModels() as $key => $value) : ?>
                    <tr style="background-color: <?php echo (empty($value['messagecode']) ? '#F2DEF9' : '#d4f4e8'); ?>;">
                        <td><input type="checkbox" name="chkDel[]"  id="chkDel<?= $key; ?>" value="<?php echo $value["visit_id"] . $value["hn"]; ?>" style="width: 16px; height: 16px;"></td>
                        ...
                        <td class="text-nowrap" style="font-size: 14px;"><?php echo $value["drug_id"]; ?></td>
