...
$this->title = 'รายงานบริการผู้ป่วยนอก  ตรวจโรคทั่วไป';
?>
...
<div class="form-group">
   ...
   <div class="row">
    ...
    <div class="col-md-6">
        <div class="form-group">
            <?= Html::label('สิทธิการรักษา', 'inscl', ['class' => 'form-label font-weight-bold']) ?>
            <?php
            // ดึงข้อมูลสิทธิการรักษาจากฐานข้อมูล
            $inscls = \yii\helpers\ArrayHelper::map(
                \app\models\Maininscl::find()->select(['inscl', 'inscl_name'])->asArray()->all(),
                'inscl',
                'inscl_name'
            );

            // เพิ่มตัวเลือก "ทั้งหมด" ไว้ในรายการ
            $inscls = ['ALL' => 'ทั้งหมด'] + $inscls;
            ?>
            <?= Html::dropDownList(
                'inscl',  // ชื่อพารามิเตอร์ที่จะส่งในฟอร์ม
                Yii::$app->request->get('inscl', 'ALL'), // ค่าเริ่มต้นคือ 'ALL'
                $inscls, // รายการตัวเลือกที่ได้จากฐานข้อมูล
                [
                    'class' => 'form-control',
                    'onchange' => 'this.form.submit()', // เมื่อเลือกค่าจะทำการส่งฟอร์ม
                ]
            ) ?>
        </div>
    </div>
...
<div class="row">
    <div class="col-md-6">
        <div class="form-group">
            <?= Html::label('รหัสโรค(จาก)', 'icd_code1', ['class' => 'form-label font-weight-bold']) ?>
            <?= Html::input('text', 'icd_code1', Yii::$app->request->get('icd_code1', ''), [
                'class' => 'form-control',
                'placeholder' => 'ว่างเพื่อเลือกทั้งหมด'
            ]) ?>
        </div>
    </div>
    <div class="col-md-6">
        <div class="form-group">
            <?= Html::label('รหัสโรค(ถึง)', 'icd_code2', ['class' => 'form-label font-weight-bold']) ?>
...
<div class="row">
    <div class="col-md-6">
        <div class="form-group">
            <?= Html::label('ช่วงอายุ(จาก)', 'age_start', ['class' => 'form-label font-weight-bold']) ?>
            <?= Html::input('text', 'age_start', Yii::$app->request->get('age_start', ''), [
                'class' => 'form-control',
                'placeholder' => 'ว่างเพื่อเลือกทั้งหมด'
            ]) ?>
        </div>
    ...
    <div class="col-md-6">
        <div class="form-group">
            <?= Html::label('อายุ (ถึง)', 'age_end', ['class' => 'form-label font-weight-bold']) ?>
            <?= Html::input('text', 'age_end', Yii::$app->request->get('age_end', ''), [
                'class' => 'form-control',
                'placeholder' => 'ว่างเพื่อเลือกทั้งหมด'
            ]) ?>
        </div>
...
<div class="col-md-12">
    <div class="form-group form-inline">
        <?= Html::radioList('mobile', Yii::$app->request->get('mobile', 'ALL'), [
            'ALL' => 'นอกหน่วยบริการ',
            'SPECIFIC' => 'ในหน่วยบริการ'
        ], ['class' => 'mr-3']) ?>
    </div>
</div>
            ...
            <div class="well">
                <div class="report-table">
                    <?= GridView::widget([
                        ...
                        'layout' => "{items}",
                        'showPageSummary' => true, // ✅ ใช้ได้ใน Kartik GridView
                        'columns' => [
                            ['class' => 'kartik\grid\SerialColumn'],
                            [
                                'attribute' => 'inscl_name',
                                'label' => 'Insurance Name',
                                'pageSummary' => 'รวมทั้งหมด', // ✅ แสดงคำว่า "รวมทั้งหมด"
                                'footerOptions' => ['style' => 'font-weight:bold;'],
                            ],
                            [
                                'attribute' => 'Visit',
                                'label' => 'ครั้ง',
                                'pageSummary' => true, // ✅ สรุปผลรวม
                            ],
                            [
                                'attribute' => 'amount',
                                'label' => 'คน',
        ...
        'panel' => [
            ...
            'after'=>'ประมวลผล '.date('Y-m-d H:i:s')
                            ...
                            'columns' => [
								...
								'format' => ['datetime', 'php:d/m/Y H:i:s'],  // เพิ่มเวลาในรูปแบบ d/m/Y H:i:s
								'contentOptions' => ['class' => 'text-center'],
								],
