...
<style>
	...
	<style>
        .info-card {
            background: linear-gradient(45deg, #a8e6cf, #d0f0c0); /* สีเขียวอ่อนแบบ gradient */
...
.my-striped-table tbody tr:hover {
            ...
            <div class="info-card bg-info" style="box-shadow: 5px 5px 10px rgba(0, 0, 0, 0.3);">
                ...
                <div style="text-align: right;">
                    <div style="text-align: right;">
                        ...
                        <a href="<?= Url::to(['f16opae/run-curl']) ?>" class="btn btn-info" style="font-size: 16px;">
                                    RunToken <i class="fa fa-arrow-circle-right"></i></a>
                                    <a href="<?= \yii\helpers\Url::to(['fdhopae/index']) ?>" class="btn btn-warning modalLink" style="font-size: 16px;" target="_blank">
                        Query <i class="fa fa-arrow-circle-right"></i>
                    </a>
                    <?= Html::a('Export', ['f16osec8/exports'], ['class' => 'btn btn-success']) ?>
                    </div>
                    </div>
 ...
 <div class="container">
                    <div class="row justify-content-center">
                        <div class="col-md-4">
        <?php $form = ActiveForm::begin(['action' => ['f16opae/index']]); ?>
...
<div class="floating-button" style="position: fixed; bottom: calc(3 * 2rem); left: 55%; transform: translateX(-45%); z-index: 1000; display: flex; gap: 10px;">
    <button type="submit" 
            name="btnButton1" 
            id="selectAll" 
    ...
    <?= Html::beginForm(['f16opae/data1'], 'post', ['name' => 'frmExport']); ?>
    <button type="submit" 
            name="btnButton2" 
            id="exportFiles" 
            class="btn btn-success" 
            style="background-color: #00a4FF; border: 4px solid #dadada; padding: 10px 20px; border-radius: 30px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1); font-size: 1.5rem; text-transform: uppercase; cursor: pointer; width: auto;">
        <i class="fa fa-arrow-circle-right" style="margin-right: 10px;"></i>
        ส่งออกไฟล์ตรวจสอบ
    </button>
    <?= Html::endForm(); ?>
...
<script>
          ...
          <!--  <input name="btnButton1" class="btn btn-success btn btn-block btn-blink" id="selectAll" type="submit" name="selectAll" value="ส่งข้อมูล FDH OP-AE" style="background-color: #00a400; border: 4px solid #dadada; font-size: 20px; transition: all 0.3s;">-->
            ...
            <table class="table my-striped-table" width="1000" border="0" bordercolor="#ddd" style="border-collapse: collapse; box-shadow: 2px 2px 5px rgba(0,0,0,0.2);">
                ...
                <?php foreach ($dataProvider->getModels() as $key => $value) : ?>
                    <tr style="background-color: <?php echo (empty($value['messagecode']) ? '#F2DEF9' : '#a7f2cd'); ?>;">
                        <td><input type="checkbox" name="chkDel[]"  id="chkDel<?= $i; ?>" value="<?php echo $value["visit_id"]; ?><?php echo $value["hn"]; ?>"style="width: 17px; height: 17px;"></td>
    ...
    <td class="text-nowrap" style="font-size: 14px;"><?php echo $value["hospname"]; ?></div>
    </td>
