...
class IpucController extends \yii\web\Controller
{
...
###################################################################################################################
      ...
      (SELECT 
CONCAT(CASE 
WHEN g.PRENAME not in('') THEN TRIM(g.PRENAME) 
WHEN TIMESTAMPDIFF(year,g.BIRTHDATE,NOW())< '20' AND g.sex='1' AND g.MARRIAGE = '4'THEN 'สามเณร' 
WHEN TIMESTAMPDIFF(year,g.BIRTHDATE,NOW()) >= '20' AND g.sex='1' AND g.MARRIAGE = '4'THEN 'พระภิกษุ' 
WHEN TIMESTAMPDIFF(year,g.BIRTHDATE,NOW()) < '15' AND g.sex='1' THEN 'ด.ช.' 
WHEN TIMESTAMPDIFF(year,g.BIRTHDATE,NOW()) >= '15' AND g.sex='1' THEN 'นาย' 
WHEN TIMESTAMPDIFF(year,g.BIRTHDATE,NOW()) < '15' AND g.sex='2' THEN 'ด.ญ.' 
WHEN TIMESTAMPDIFF(year,g.BIRTHDATE,NOW()) >= '15' AND g.sex='2' AND g.MARRIAGE ='1' THEN 'น.ส.' 
ELSE 'นาง' 
END ,TRIM(g.FNAME), '  ',TRIM(g.LNAME)) as 'Doctor', b.hn, a.ADM_ID,
CONCAT(CASE 
...
s.UNIT_NAME as 'หอผู้ป่วยใน',(k.cg01+k.cg01_1+k.cg01_2+k.cg02+k.cg03+k.cg04+k.cg05+k.cg06+k.cg07+k.cg08+k.cg09+k.cg10+k.cg11+k.cg12+k.cg13+k.cg14+k.cg15+k.cg16+k.cg17+k.cg18+k.cg19) as 'แจ้งหนี้ Mbase' 
...
ORDER BY k.ret_statement
       ...
       # ORDER BY  No DESC 
            ";
			 $rawData1 = \Yii::$app->db2->createCommand($sqlData1)->queryAll();
