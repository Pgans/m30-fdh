 ...
 */
class Ktp305Controller extends Controller
{
    ...
    private function queryDataWithReason()
    {
        $sql = "
            SELECT
                ...
                c.reason
            FROM  KTP_C305    c
            LEFT  JOIN KTP_C305_HN k ON c.tran_id = k.tran_id
            INNER JOIN cid_hn      h ON h.cid      = k.pid
            GROUP BY c.tran_id
        ";
        try {
    ...
    private function exportToCsv(array $data, ?string $month = null): void
    {
        ...
        $filename = $month
            ? 'c305_ktp_' . $month . '_' . date('Ymd_His') . '.csv'
            : 'c305_ktp_' . date('Ymd_His') . '.csv';
    ...
    {
        $defaultReasons = $this->getDefaultReasons();
        $filename = 'c305_ktp_reason_' . date('Ymd_His') . '.csv';
...
private function exportToPdf(array $data): void
{
    ...
    // ── [4] Header + เปิด table ───────────────────────────────────
    $mpdf->WriteHTML('
<h2>แบบแจ้งเหตุผลความจำเป็นกรณีไม่ได้จัดให้ผู้มีสิทธิ<br>แสดงตนยืนยันสิทธิเมื่อสิ้นสุดการรับบริการ</h2>
<p class="subtitle">ชื่อหน่วยบริการ โรงพยาบาลม่วงสามสิบ &nbsp;&nbsp; รหัสหน่วยบริการ 10953</p>
<p class="subtitle">งวดจ่ายเดือน มกราคม 2568- มกราคม 2569 &nbsp;&nbsp; กรณีบริการ  KTP 305</p>
<hr>
<table class="main">
<thead><tr>
...
<tbody>', \Mpdf\HTMLParserMode::HTML_BODY);
    ...
    foreach ($data as $idx => $row) {
        ...
        $chunkHtml .=
            ...
            . '<td class="tc">'   . $no                                                               . '</td>'
            . '<td class="tc">KTP</td>'
            . '<td class="tc">'   . htmlspecialchars((string)($row['tran_id2'] ?? ''), ENT_QUOTES, 'UTF-8') . '</td>'
...
<table class="sign"><tr>
    ...
    $filename = 'c305_ktp_' . date('Ymd_His') . '.pdf';
