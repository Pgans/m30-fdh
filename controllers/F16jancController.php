...
class F16jancController extends \yii\web\Controller
{
    public function actionIndex()
    {
       // ดึงค่าวันที่จาก Request (ถ้าไม่มีค่าส่งมา ให้ Default เป็นค่าว่าง null)
$date1 = Yii::$app->request->get('date1');
$date2 = Yii::$app->request->get('date2');

// --- เช็คค่าว่างเพื่อตั้ง Default ย้อนหลัง 1 เดือน ---
if (empty($date1)) {
    $date1 = date('Y-m-d', strtotime('-1 month')); // ย้อนหลัง 1 เดือนนับจากวันนี้
}
if (empty($date2)) {
    $date2 = date('Y-m-d'); // วันปัจจุบัน
}

// นำมาต่อเวลาตรรกะเดิมของคุณเพื่อใช้ดึงข้อมูลใน Database
...
$date2_full = date('Y-m-d 23:59', strtotime($date2));
		...
		,log.messagecode
          FROM fdhanc_proc f
		  #LEFT JOIN visitancpregnancy v ON v.pid = f.pid AND f.pregno = v.pregno
		  ...
		  LEFT JOIN mbase_data1.log_fdh_opd_ck   as log ON log.visit_id = f.seq
            WHERE f.date_serv BETWEEN '$date1' AND '$date2'
        ...
        WHERE  v.users = 'janc' AND v.messagecode <> ''
       ...
       AND l.messagecode <> '' AND l.users = 'janc'
       ORDER BY l.d_update DESC
        ...
        AND l.messagecode <> 'success' AND l.users = 'janc'
        ORDER BY l.d_update DESC
    ...
    public function actionData()
    {
        ...
        foreach ($vn as $r) {
            ...
            foreach ($tables as $table) {
                $query = "SELECT main_query FROM fdh_anc WHERE main_table = '$table'";
                ...
                while ($row = fgetcsv($file, 0, "|")) {
                    ...
                    if ($visitCount > 0) {
                        ...
                        $logSQLUpdate = "UPDATE log_fdh_opd_ck SET pid = :pid, messages = :messages, messagecode = :messagecode, response = :response,
                         users = 'janc', datereg = :datereg, d_update = NOW() WHERE visit_id = :visit_id";
                        ...
                        VALUES (:visit_id, :pid, :messages, :messagecode, :response, 'janc',:datereg ,NOW())";
    ...
    public function actionExports()
    {
        ...
        $zipFilename = $baseDirectory . 'F16_10953_janc_' . $currentDateTime . '.zip';
