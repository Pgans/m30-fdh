...
use app\models\Logtotalvisit;
use yii\web\NotFoundHttpException;
use app\models\CloseVisit; 


class TotalvisitsipdController extends \yii\web\Controller
{
...
(
    SELECT 
        ir.dsc_dt AS 'admitdate',
        TIME(ir.dsc_dt) AS time_start,
...
END AS charge_total,
        o.inscl,
CASE
WHEN f.inscl in ('18','19')  THEN 'A9'
WHEN f.inscl in ('31')  THEN 'SSS'
WHEN f.inscl in ('73','74')  THEN 'OFC'
WHEN f.inscl in ('16')  THEN 'XXX'
WHEN f.inscl in ('05')  THEN 'OTH'
WHEN f.inscl in ('00','23')  THEN 'UCS'
ELSE 'UCS'
END as nhsocode,
				f.nhso_code,
				f.inscl_name,
   ...
   # LEFT JOIN icd10new icd1 ON icd1.icd10 = d1.icd10 
    #LEFT JOIN log_totalvisits lc ON lc.visit_id = o.visit_id  
    INNER JOIN ipd_reg ir ON o.visit_id = ir.visit_id 
		LEFT JOIN service_units e ON ir.WARD_NO = e.unit_id   
    WHERE ir.DSC_DT BETWEEN '2024-10-01 00:01' AND '2024-12-31 23:59'
	#AND o.unit_reg NOT IN  ('50','42')
        AND o.is_cancel = '0'
				AND ir.is_cancel = '0'
        #AND p.NATN_ID = '99'
        AND o.visit_id NOT IN (SELECT visit_id FROM log_totalvisits  )
		
		AND o.visit_id NOT IN (SELECT visit_id FROM mobile_visits )
    GROUP BY o.visit_id
) AS data,
(SELECT @n := 0) AS init
ORDER BY No DESC, charge_total DESC;
 ";
    ...
    $visitProvider = new \yii\data\ArrayDataProvider([
        ...
        ########################################################################################################
        $sqlPass = "select l.id, l.visit_id, l.pid , l.messagecode, l.users, l.send_date
        FROM log_totalvisits l 
        WHERE l.send_date BETWEEN DATE_SUB(NOW(), INTERVAL 1 DAY) AND NOW()
        AND l.messagecode = 'success' AND l.users = 'ipd'
        ...
        ########################################################################################################
        $sqlError = "select l.id, l.visit_id, l.pid , l.messagecode, l.users, l.send_date
        FROM log_totalvisits l 
        WHERE l.send_date BETWEEN DATE_SUB(NOW(), INTERVAL 1 DAY) AND NOW()
        AND l.messagecode <> 'success' AND l.users = 'ipd'
        ORDER BY l.send_date DESC
    ...
    public function actionCheck()
    {
       ...
       ## $sqltoken = "SELECT MAX(token) as token30 FROM fdh_token";
        ...
        data.*
			  FROM 
		(SELECT CONCAT(DATE_FORMAT(o.reg_datetime, '%Y-%m-%dT%H:%i:%s'), 'Z') AS regdate
		,DATE_FORMAT(o.reg_datetime, '%Y-%m-%d %H:%i') as 'regdatex'
		...
		, CONCAT(
          ...
          ,p.cid
,ir.adm_id as an
,CONCAT(DATE_FORMAT(o.reg_datetime, '%Y-%m-%dT%H:%i:%s'), 'Z') as datetime_admit
,CONCAT(DATE_FORMAT(o.reg_datetime, '%Y-%m-%dT%H:%i:%s'), 'Z') as datetime_discharge
,o.inscl
,CASE
WHEN f.inscl in ('18','19')  THEN 'A9'
WHEN f.inscl in ('31')  THEN 'SSS'
		...
		GROUP BY o.VISIT_ID 
            ...
            foreach ($closeData as $closeRow) {
                $resultArray = [
                    ...
                    "hn" => $closeRow['hn'],
                    "an" => $closeRow['an'],
                    "datetime_admit" => $closeRow['datetime_admit'],
					"datetime_discharge" => $closeRow['datetime_discharge'],
                    "datetime_service" => $closeRow['regdate'],
                    "insurance_code" => $closeRow['nhsocode'],
                    "charge_total" => $closeRow['amount']
                ];
            }
           ...
           // echo $resultText;
            ...
            //$token = $token_fdh;
			  ...
			  #$url = "https://uat-fdh.inet.co.th/api/v1/data_hub/total_visit";
			  $url = "https://fdh.moph.go.th/api/v1/data_hub/total_visit";
            ...
            $totalvisit = json_decode($response, true);
            $err = curl_error($curl);
            //curl_close($curl);
             //echo $response;
            curl_close($curl);
            // $cid = $total['results']['cid'];
            $message = $totalvisit['message'];
            $message_th = $totalvisit['message_th'];
            $status = $totalvisit['status'];
			#$transaction_uid = $totalvisit['data']['transaction_uid'];
            // echo $status;
            //echo $response;

            ############################INSERT TABLE Log_totalvisits #############################   cost_visits->visit_id-> return send_date-> booK-id->status 200

            if (strlen($response) > 0) {
    $strSQL = "REPLACE INTO log_totalvisits (visit_id, pid, an , status, messagecode, message_th ,users, send_date, regdate) 
               VALUES ('$visit', '$hn','{$closeRow['an']}', $status, '$message', '$message_th','ipd', NOW(), '{$closeRow['regdatex']}')";
...
public function actionDeleteMultiple()
{
    ...
    if (!empty($selection)) {
        Logtotalvisit::deleteAll(['id' => $selection]); // ลบรายการตาม ID ที่เลือก
    ...
    public function actionDeleteSpecific()
    {
        ...
        $sql = "DELETE FROM log_totalvisits
