...
class F16telemedController extends \yii\web\Controller
{
 ...
 FROM (SELECT @n := 0) m,
      ...
      LEFT JOIN (SELECT visit_id, MAX(d_update) AS latest_d_update FROM log_all.log_fdh_opd GROUP BY visit_id) log_max ON log_max.visit_id = o.visit_id
 ...
 ORDER BY No DESC;
        ...
        AND l.messagecode = 'success' AND l.users = 'telemed'
    ...
    foreach ($vn as $r) {
        ...
        foreach ($tables as $table) {
			...
			foreach ($fileData as $table => $data) {
				// Remove duplicates if the table is 'pat'
				if ($table === 'pat') {
					$data = $this->removeDuplicatePersonId($data);
				}
		...
		private function removeDuplicatePersonId($data)
		{
			$uniqueData = [];
			$personIds = [];

			foreach ($data as $row) {
				if (!in_array($row['Person_id'], $personIds)) {
					$uniqueData[] = $row;
        ...
        if (file_exists($chtFilePath)) {
            ...
            while ($row = fgetcsv($file, 0, "|")) {
                ...
                if ($visitCount > 0) {
                    ...
                    $logSQLUpdate = "UPDATE log_fdh_opd_ck SET pid = :pid, messages = :messages, messagecode = :messagecode, response = :response, users = 'telemed', d_update = NOW() WHERE visit_id = :visit_id";
                    ...
                    $logSQLInsert = "INSERT INTO log_fdh_opd_ck (visit_id, pid, messages, messagecode, response, users, d_update) VALUES (:visit_id, :pid, :messages, :messagecode, :response, 'telemed', NOW())";
