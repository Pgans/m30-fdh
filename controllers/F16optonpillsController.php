...
class F16optonpillsController extends \yii\web\Controller
{
    ...
    public function actionIndex()
    {
	  ...
	  (SELECT   DATE_FORMAT(o.reg_datetime, '%Y-%m-%d %H:%i') as 'regdate'
       ...
       , CONCAT(
        ...
        AND l.messagecode <> '' AND l.users = 'optonpills'
...
##################################################################
    ...
    if ($table === 'pat') {
        ...
        // ถ้าไม่มีข้อมูลใน fdh_ins ให้ใช้จาก fdh_optonpills
        if (empty($results)) {
            $queryHerb = "SELECT main_query FROM fdh_optonpills WHERE main_table = :table";
...
/*
            foreach ($tables as $table) {
                $query = "SELECT main_query FROM fdh_optonpills WHERE main_table = '$table'";
                $results = $db2->createCommand($query)->queryAll();
*/
                    ...
                    $visitCount = Yii::$app->db2->createCommand($logSQLCheck)
                        ...
                        if ($visitCount > 0) {
                            ...
                            $logSQLUpdate = "UPDATE log_fdh_opd_ck SET pid = :pid, messages = :messages, messagecode = :messagecode, response = :response,
                             users = 'optonpills', datereg = :datereg, d_update = NOW() WHERE visit_id = :visit_id";
                            ...
                            VALUES (:visit_id, :pid, :messages, :messagecode, :response, 'optonpills',:datereg ,NOW())";
