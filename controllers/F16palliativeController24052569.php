    ...
    public function actionData()
{
    ...
    foreach ($vn as $r) {
        ...
        foreach ($tables as $table) {
            $query = "SELECT main_query FROM fdh_palliative WHERE main_table = '$table'";
...
public function actionCheckData()
{
    ...
    try {
        ...
        // ✅ 7 แฟ้ม ตรงกับ actionData — ทุกแฟ้มบังคับ
        $tables   = ['adp', 'ins', 'odx', 'opd', 'pat', 'cha', 'cht'];
        $result   = [];
        $hasError = false;
        ...
        foreach ($tables as $table) {
            ...
            try {
                // ✅ ใช้ fdh_palliative เหมือน actionData
                $results = $db2->createCommand(
                    "SELECT main_query FROM fdh_palliative WHERE main_table = :t"
                )->bindValue(':t', $table)->queryAll();

                if (empty($results)) {
                    $status   = 'no_config';
                    $message  = 'ไม่พบ query config ใน fdh_palliative';
                    $hasError = true;
                ...
                if ($table === 'adp') {
                    try {
                        ...
                        Yii::warning("visit_invoice error: " . $e->getMessage(), 'checkdata');
                        $invoiceData = [];
                    }
                }
                ...
                $hasError = true;
                Yii::error("palliCheckData error table={$table}: " . $e->getMessage(), 'checkdata');
            }
            ...
            $result[] = [
                ...
                'message'      => $message,
                'rows'         => $allData,
                'rows_invoice' => $invoiceData,  // ✅ ส่ง visit_invoice มาด้วย (เฉพาะ adp)
            ];
        }
...
######################################################################################################################
                ...
                while ($row = fgetcsv($file, 0, "|")) {
                   ...
                   if ($visitCount > 0) {
                            ...
                            $logSQLUpdate = "UPDATE log_fdh_opd_ck SET pid = :pid, messages = :messages, messagecode = :messagecode, response = :response,
                             users = 'palliative', datereg = :datereg, d_update = NOW() WHERE visit_id = :visit_id";
                            ...
                            VALUES (:visit_id, :pid, :messages, :messagecode, :response, 'palliative',:datereg ,NOW())";
		...
		public function actionExports()
    {
        ...
        $zipFilename = $baseDirectory . 'F16_10953_Palliaitve_' . $currentDateTime . '.zip';
