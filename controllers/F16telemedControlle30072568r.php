...
class F16telemedController extends \yii\web\Controller
{
 ...
 FROM 
      ...
      LEFT JOIN (SELECT visit_id, MAX(d_update) AS latest_d_update FROM log_fdh_opd_ck GROUP BY visit_id) log_max ON log_max.visit_id = o.visit_id
 ...
 WHERE o.IS_CANCEL = 0
       ...
       AND (o.UNIT_REG IN ('63', '68', '70', '71') OR o.UNIT_ID IN ('63', '68', '70', '71'))
 ...
 GROUP BY o.VISIT_ID
      ...
      ORDER BY 
        ...
        for ($i = 0; $i < sizeof($data); $i++) {
		 ...
		 #########  จำนวนไม่ผ่าน ################################################################
		 $sqlCount2 = "SELECT COUNT(v.visit_id) as amountx
        FROM log_fdh_opd_ck v 
        WHERE v.users = 'telemed' 
              AND (v.messagecode IS NULL OR v.messagecode = '') 
              -- AND v.messages = 'waited' 
              AND DATE(v.d_update) = CURDATE()";  // ใช้ DATE() เพื่อให้ได้ข้อมูลของวันนี้เท่านั้น

			$data = \Yii::$app->db2->createCommand($sqlCount2)->queryAll();

			$amountx = 0;  // กำหนดค่าเริ่มต้น

			if (!empty($data)) {
				$amountx = $data[0]['amountx'];  // ใช้ index 0 ตรง ๆ เนื่องจาก COUNT() คืนค่ามาเป็นแค่ 1 แถว
			}
