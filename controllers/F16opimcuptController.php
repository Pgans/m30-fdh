...
class F16opimcuptController extends \yii\web\Controller
{
...
(SELECT 
       ...
       , CONCAT(
        ...
        WHERE  v.users = 'opimcupt' AND v.messagecode <> ''
		#AND v.messages <> ''
        ...
        AND l.messagecode <> '' AND l.users = 'opimcupt'
        ...
        AND l.messagecode <> 'success' AND l.users = 'opimcupt'
        ORDER BY l.d_update DESC
    ...
    public function actionData()
    {
        ...
        foreach ($vn as $r) {
            ...
            foreach ($tables as $table) {
                $query = "SELECT main_query FROM fdh_imc WHERE main_table = '$table'";
                ...
                while ($row = fgetcsv($file, 0, "|")) {
                    ...
                    if ($visitCount > 0) {
                            ...
                            $logSQLUpdate = "UPDATE log_fdh_opd_ck SET pid = :pid, messages = :messages, messagecode = :messagecode, response = :response,
                             users = 'opimcupt', datereg = :datereg, d_update = NOW() WHERE visit_id = :visit_id";
                            ...
                            VALUES (:visit_id, :pid, :messages, :messagecode, :response, 'opimcupt',:datereg ,NOW())";
