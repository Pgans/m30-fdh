...
class F16ipucepController extends \yii\web\Controller
{
     public function actionIndex()
    {
      ...
      AND ir.dsc_dt <> '0'
      AND o.INSCL in ('29','32')
      AND ir.WARD_NO != '50'
      #AND o.visit_id  not in (SELECT vs.visit_id from log_fdh_ipd vs where vs.users='ipstp' )
      GROUP BY o.VISIT_ID
      ORDER BY ir.adm_id           
      ) AS data,
        (SELECT @n := 0) AS init
      ORDER BY 
        ...
        WHERE  v.users = 'ipstp' and v.messagecode <> '' 
        ...
        AND l.messagecode = 'success' AND l.users = 'ipucep'
    ...
    public function actionData()
    {
        ...
        foreach ($vn as $r) {
           ...
           // echo $an;
            ...
            $baseDirectory = 'uploads/ipucep/';
            $mode = 0777;

            $tables = ['adp', 'ins', 'pat', 'ipd', 'iop', 'irf', 'idx','aer', 'dru', 'cha', 'cht' ];
            foreach ($tables as $table) {
                $query = "SELECT main_query FROM fdh_ipucep WHERE main_table = '$table'";
    ...
    private function getHeaderForTable($table)
    {
        switch ($table) {
            case 'adp':
                return ['HN', 'AN', 'DATEOPD', 'TYPE', 'CODE', 'QTY', 'RATE', 'SEQ', 'CAGCODE', 'DOSE', 'CA_TYPE', 'SERIALNO', 'TOTCOPAY', 'USE_STATUS', 'TOTAL', 'QTYDAY', 'TMLTCODE', 'STATUS1', 'BI', 'CLINIC', 'ITEMSRC', 'PROVIDER', 'GRAVIDA', 'GA_WEEK', 'DCIP/E_SCREEN', 'LMP','SP_ITEM'];
            case 'cha':
            ...
            case 'dru':
                return ['HCODE', 'HN', 'AN', 'CLINIC', 'PERSON_ID', 'DATE_SERV', 'DID', 'DIDNAME', 'AMOUNT', 'DRUGPRICE', 'DRUGCOST', 'DIDSTD', 'UNIT', 'UNIT_PACK', 'SEQ', 'DRUGREMARK', 'PA_NO', 'TOTCOPAY', 'USE_STATUS', 'TOTAL', 'SIGCODE', 'SIGTEXT', 'PROVIDER','SP_ITEM'];
            case 'ins':
    ...
    private function sendDataToAPI($visit, $an)
    {
        ...
        $sqltoken = "SELECT MAX(token) as token30 FROM fdh_token WHERE staff_id = 'tea'";
        ...
        if ($data && isset($data['token30'])) {
            ...
            // กำหนดเส้นทางของไฟล์
            $filePaths = [
                __DIR__ . '/../web/uploads/ipucep/PAT.txt',
                __DIR__ . '/../web/uploads/ipucep/INS.txt',
                __DIR__ . '/../web/uploads/ipucep/IPD.txt',
                __DIR__ . '/../web/uploads/ipucep/ADP.txt',
                __DIR__ . '/../web/uploads/ipucep/IDX.txt',
                __DIR__ . '/../web/uploads/ipucep/IRF.txt',
                __DIR__ . '/../web/uploads/ipucep/IOP.txt',
                __DIR__ . '/../web/uploads/ipucep/DRU.txt',
                __DIR__ . '/../web/uploads/ipucep/AER.txt',
                __DIR__ . '/../web/uploads/ipucep/CHA.txt',
                __DIR__ . '/../web/uploads/ipucep/CHT.txt',
            ];
            ...
            $logSQL = "REPLACE INTO log_fdh_ipd_ck(visit_id, pid, an, messagecode, response, users, d_update) 
            VALUES ('$visit', '', '$an','$message', '$message_th', 'ipucep', NOW())";
...
public function actionListFiles()
{
    ...
    $dirPath = Yii::getAlias('@webroot/uploads/ipucep');
...
{
$filePath = Yii::getAlias('@webroot/uploads/ipucep/' . $fileName);
    ...
    public function actionRunCurl()
    {
        ...
        curl_setopt_array($curl, array(
            ...
            CURLOPT_POSTFIELDS => json_encode([
                 'user' => 'kanchana.10953',
                'password_hash' => 'ABBF3B9675851202FAA4C5817E8FDAD7F733E722E932010792C1A56908DD94E4',
                'hospital_code' => '10953'
            ]),
            CURLOPT_HTTPHEADER => [
