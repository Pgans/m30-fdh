...
class F16anccareController extends \yii\web\Controller
{
...
(SELECT 
       ...
       , CONCAT(
          ...
          WHERE o.IS_CANCEL = 0
				...
				AND o.unit_reg = '45'
				AND icd.icd10_tm  BETWEEN 'Z390' AND 'Z392' 
    ...
    if ($table === 'pat') {
        ...
        if (empty($results)) {
            $queryHerb = "SELECT main_query FROM fdh_anccare WHERE main_table = :table";
...
#########################################################################################################
     private function sendDataToAPI($visit, $hn, $zone = 'test')
{
