...
use  app\models\Fdhancdentuslab1upt;
...
class F16ancdentuslab1uptController extends \yii\web\Controller
{
...
/*
        foreach ($tables as $table) {
            $query = "SELECT main_query FROM fdh_ancdent_us_lab1_upt WHERE main_table = '$table'";
...
##################################################################
    ...
    if ($table === 'pat') {
        ...
        // ถ้าไม่มีข้อมูลใน fdh_ins ให้ใช้จาก fdh_ancdent_us_lab1_upt
        if (empty($results)) {
            $queryHerb = "SELECT main_query FROM fdh_ancdent_us_lab1_upt WHERE main_table = :table";
                ...
                $visitCount = Yii::$app->db2->createCommand($logSQLCheck)
                    ...
                    if ($visitCount > 0) {
                        ...
                        $logSQLUpdate = "UPDATE log_fdh_opd_ck SET pid = :pid, messages = :messages, messagecode = :messagecode, response = :response,
                         users = 'ancdentuslab1upt', datereg = :datereg, d_update = NOW() WHERE visit_id = :visit_id";
                        ...
                        VALUES (:visit_id, :pid, :messages, :messagecode, :response, 'ancdentuslab1upt',:datereg ,NOW())";
