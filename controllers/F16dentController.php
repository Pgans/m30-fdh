...
class F16dentController extends \yii\web\Controller
{
    ...
    public function actionData()
{
    ...
    foreach ($vn as $r) {
        ...
        $tables = ['adp', 'ins', 'odx', 'opd','oop', 'pat', 'cha', 'cht'];
        foreach ($tables as $table) {
            $query = "SELECT main_query FROM fdh_dent WHERE main_table = '$table'";
                ...
                $visitCount = Yii::$app->db2->createCommand($logSQLCheck)
                    ...
                    if ($visitCount > 0) {
                        ...
                        $logSQLUpdate = "UPDATE log_fdh_opd_ck SET pid = :pid, messages = :messages, messagecode = :messagecode, response = :response,
                         users = 'dent', datereg = :datereg, d_update = NOW() WHERE visit_id = :visit_id";
                        ...
                        VALUES (:visit_id, :pid, :messages, :messagecode, :response, 'dent',:datereg ,NOW())";
...
###########################################################################################################
	...
	public function actionExports()
    {
        ...
        $zipFilename = $baseDirectory . 'F16_10953_dent_' . $currentDateTime . '.zip';
