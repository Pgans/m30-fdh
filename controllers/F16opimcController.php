...
use yii\db\Expression;
	...
	class F16opimcController extends \yii\web\Controller
	{
...
(SELECT 
       ...
       , CONCAT(
          ...
          LEFT JOIN lab_lists l ON l.lab_id = lr.lab_id 
					LEFT JOIN opd_operations od ON o.VISIT_ID=od.VISIT_ID AND od.IS_CANCEL = 0 
					LEFT JOIN icd9cm oo ON od.icd9=oo.icd9  
          ...
          LEFT JOIN authen_kiosk ak ON p.CID = ak.cid  AND date(o.REG_DATETIME)=date(ak.d_update)
					...
					LEFT JOIN visit_invoice v1 on v1.visit_id=o.visit_id AND v1.is_cancel = 0 and v1.code16 between '9202' and '9205'
          ...
          AND p.NATN_ID = 99
		      AND icd.ICD10_TM = 'U61.10' 
         # AND o.visit_id in ('0003081127','0003085682','0003095040','0003103497','0003103674','0003103714','0003103772','0003105982','0003108969')
        ...
        WHERE  v.users = 'opimc' AND v.messagecode <> ''
		#AND v.messages <> ''
        ...
        AND l.messagecode <> '' AND l.users = 'opimc'
        ...
        AND l.messagecode <> 'success' AND l.users = 'opimc'
        ORDER BY l.d_update DESC
  ...
  public function actionData()
{
    ...
    $visits = [];
    $allFileData = []; // เก็บข้อมูลจากทุก visit

    $baseDirectory = 'uploads/fdh_opd/';
    $tables = ['adp', 'ins', 'odx', 'opd', 'pat', 'cha', 'cht', 'oop','dru'];
    ...
    foreach ($vn as $r) {
        ...
        $fileData = []; // สำหรับ visit ปัจจุบัน

        foreach ($tables as $table) {
            try {
                // ดึง main_query จาก fdh_pat, fdh_ins, fdh_imc ตามลำดับ
                if ($table === 'pat') {
                    ...
                    if (empty($results)) {
                        $queryHerb = "SELECT main_query FROM fdh_imc WHERE main_table = :table";
                ...
                foreach ($results as $result) {
                    ...
                    try {
                        ...
                        $data = []; // กำหนดเป็น array ว่างถ้า error
                    }
    ...
    // สร้าง HTTP client และส่งคำขอ ## https://fdh.moph.go.th/api/v2/data_hub/16_files  ***  https://uat-fdh.inet.co.th/api/v2/data_hub/16_files
                ...
                while ($row = fgetcsv($file, 0, "|")) {
                    ...
                    if ($visitCount > 0) {
                            ...
                            $logSQLUpdate = "UPDATE log_fdh_opd_ck SET pid = :pid, messages = :messages, messagecode = :messagecode, response = :response,
                             users = 'opimc', datereg = :datereg, d_update = NOW() WHERE visit_id = :visit_id";
                            ...
                            VALUES (:visit_id, :pid, :messages, :messagecode, :response, 'opimc',:datereg ,NOW())";
        ...
        public function actionRunCurl()
        {
           ...
           try {
            ...
            $sqltoken = "SELECT MAX(token) as token30, staff_id FROM fdh_token WHERE staff_id = 'yoo'";
    ...
    public function actionExports()
        {
            ...
            $zipFilename = $baseDirectory . 'F16_10953_ฝังเข็ม_' . $currentDateTime . '.zip';
