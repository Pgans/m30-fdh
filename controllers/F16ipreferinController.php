...
class F16ipreferinController extends \yii\web\Controller
{
    ...
    public function actionIndex()
    {
      ...
      AND ir.dsc_dt <> '0'
              ...
              AND ir.WARD_NO != '50'
      #AND o.visit_id  not in (SELECT vs.visit_id from log_all.log_fdh_ipd vs where vs.users='ipreferin' )
      GROUP BY o.VISIT_ID
      ORDER BY ir.adm_id           
      ) AS data,
        (SELECT @n := 0) AS init
      ORDER BY 
        ...
        WHERE  v.users = 'ipreferin' and v.messagecode <> '' 
        ...
        AND l.messagecode <> '' AND l.users = 'ipreferin'
        ...
        FROM log_fdh_ipd_ck l 
        WHERE l.d_update BETWEEN CURDATE() AND NOW()
        AND l.messagecode <> 'success' AND l.users = 'ipreferin'
        ORDER BY l.d_update DESC
        ...
        if (!empty($vn)) {
            foreach ($vn as $r) {
                ...
                foreach ($tables as $table) {
                    $query = "SELECT main_query FROM fdh_ipreferin WHERE main_table = :table";
            ...
            if ($data && isset($data['token30'])) {
                ...
                $statusMessageTh = isset($responseDecoded['data'][0]['status_message_th']) ? $responseDecoded['data'][0]['status_message_th'] : '';
                 ...
                 if ($visitCount > 0) {
                     ...
                     $logSQLUpdate = "UPDATE log_fdh_ipd_ck SET pid = :pid, messages = :messages, messagecode = :messagecode, response = :response, users = 'ipreferin', d_update = NOW() WHERE visit_id = :visit_id AND an = :an";
                     ...
                     $logSQLInsert = "INSERT INTO log_fdh_ipd_ck (visit_id, pid, an, messages, messagecode, response, users, d_update) VALUES (:visit_id, :pid, :an, :messages, :messagecode, :response, 'ipreferin', NOW())";
...
#########################################################################################################
    ...
    public function actionExports()
    {
        ...
        $zipFilename = $baseDirectory . 'F16_10953_Ipreferin_' . $currentDateTime . '.zip';
