...
public function actionData()
{
    ...
    if (empty($vn)) {
        Yii::$app->session->setFlash('error', 'ไม่มีข้อมูลใน chkDel');
    ...
    foreach ($vn as $r) {
        ...
        foreach ($tables as $table) {
            ...
            foreach ($results as $result) {
                // สมมติ main_query มีตัวแปร $visit ที่ต้องแทนที่ด้วยค่า visit แบบมี ' '
        ...
        $this->sendToNhsoApi($payload, $visit, $hn);
    }

    Yii::$app->session->setFlash('success', 'ส่งข้อมูลเรียบร้อย');
    Yii::$app->session->set('visits', $visits);
    Yii::$app->session->set('hn', $hn);

    return $this->redirect(['index', 'visit' => end($visits), 'hn' => $hn]);
}


#############################################################################################
private function sendToNhsoApi($payload, $visit, $hn)
{
    // ตรวจสอบค่าก่อน
    if (empty($visit) || empty($hn)) {
        Yii::error("❌ visit หรือ hn ว่างเปล่า ไม่สามารถส่งข้อมูลหรือบันทึก log ได้", __METHOD__);
        return false;
    }
    ...
    try {
        ...
        if (!$data || !isset($data['token30'])) {
            Yii::error("❌ ไม่พบ token สำหรับ staff_id='yoo'", __METHOD__);
            return false;
        }
        ...
        // บันทึก log
        Yii::$app->db2->createCommand()->insert('log_fdh_opd_ck', [
            'visit_id' => $visit,
            'pid' => $hn,
            'messages' => 'ส่งข้อมูลเรียบร้อย',
            'messagecode' => $httpcode,
            'response' => $response,
            'users' => Yii::$app->user->identity->username ?? 'system',
...
// สมมติ $payload เตรียมเรียบร้อยแล้ว
Yii::info("Payload เตรียมส่ง: " . json_encode($payload, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE), __METHOD__);
