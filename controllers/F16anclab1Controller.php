...
class F16anclab1Controller extends \yii\web\Controller
{
  ...
  public function actionData()
{
    ...
    $tables = ['adp', 'ins', 'oop', 'odx', 'opd', 'pat', 'cha', 'cht', 'dru'];
...
/*
        foreach ($tables as $table) {
            $query = "SELECT main_query FROM fdh_anc_lab1 WHERE main_table = '$table'";
...
##################################################################
    ...
    if ($table === 'pat') {
        ...
        // ถ้าไม่มีข้อมูลใน fdh_ins ให้ใช้จาก fdh_anc_lab1
        if (empty($results)) {
            $queryHerb = "SELECT main_query FROM fdh_anc_lab1 WHERE main_table = :table";
...
#############################################################################
            ...
            // ตรวจสอบว่าแฟ้มมีข้อมูลหรือไม่
            if (empty($fileData[$table])) {
                $emptyTables[] = $table;
            }
        }

        // ถ้ามีแฟ้มที่ว่าง ให้แจ้งเตือนและไม่ดำเนินการต่อ
    ...
    Yii::$app->session->set('visits', array_column($vn, 0, 10));
                ...
                $visitCount = Yii::$app->db2->createCommand($logSQLCheck)
                    ...
                    if ($visitCount > 0) {
                        ...
                        $logSQLUpdate = "UPDATE log_fdh_opd_ck SET pid = :pid, messages = :messages, messagecode = :messagecode, response = :response,
                         users = 'anclab1', datereg = :datereg, d_update = NOW() WHERE visit_id = :visit_id";
                        ...
                        VALUES (:visit_id, :pid, :messages, :messagecode, :response, 'anclab1',:datereg ,NOW())";
...
###########################################################################################################
	...
	public function actionExports()
    {
        ...
        $zipFilename = $baseDirectory . 'F16_10953_Anclab1_' . $currentDateTime . '.zip';
