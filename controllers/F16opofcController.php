...
class F16opofcController extends \yii\web\Controller
{
...
(SELECT 
   ...
   , CONCAT(
	  ...
	  LEFT JOIN lab_lists l ON l.lab_id = lr.lab_id 
				...
				LEFT JOIN refers r1 ON r1.visit_id = o.visit_id AND r1.rf_type= 2  AND r1.TRANSPORT !='1'  AND r1.is_cancel = 0
				...
				LEFT JOIN ultrasound_rpt ul ON ul.visit_id = o.visit_id AND ul.is_cancel = 0
				LEFT JOIN xray_requests x ON x.visit_id = o.visit_id AND x.is_cancel = 0
	  WHERE o.IS_CANCEL = 0
			AND o.REG_DATETIME BETWEEN '$date1' AND '$date2'
			AND o.INSCL in ('01','25')
			AND o.unit_reg not in ( '42','79')
			AND icd.icd10_tm not BETWEEN 'Z020' AND 'Z029'   #### ใบรับรองแพทย์
			AND icd.icd10_tm <> 'Z251' 				 
	...
	WHERE  v.users = 'opofc' AND v.messagecode <> ''
	#AND v.messages <> ''
	...
	AND l.messagecode <> '' AND l.users = 'opofc'
	...
	AND l.messagecode <> 'success' AND l.users = 'opofc'
	ORDER BY l.d_update DESC
...
{
    // 1. กำหนดให้ตอบกลับเป็นรูปแบบ JSON เสมอเพื่อส่งข้อมูลให้ AJAX หน้าบ้าน
    ...
    try {
        ...
        if (empty($vn)) {
            return [
                'status' => 'fail',
                'message' => 'ไม่พบข้อมูลรายการที่ส่งมา'
            ];
        }

        // หน้าบ้านจะแกะและส่งมาทีละ 1 รายการใน Array ($vn[0]) เสมอ
        $r = $vn[0];
        $hn = substr($r, 10);
        $visit = substr($r, 0, 10);
        $visits = [$visit]; 
        ...
        // 2. แยก Sub-folder ชั่วคราวตามเลข $visit ป้องกันไฟล์ .txt เขียนทับกัน
        $baseDirectory = 'uploads/nhso_opd/' . $visit . '/';
        $fullPath = Yii::getAlias('@webroot/' . $baseDirectory);

        if (!is_dir($fullPath)) {
            if (!mkdir($fullPath, 0777, true)) {
                throw new \Exception("ไม่สามารถสร้างโฟลเดอร์ชั่วคราว $fullPath ได้");
            }
        }
        ...
        // ดึง Query และจัดการข้อมูลเฉพาะของ Visit ปัจจุบัน
        foreach ($tables as $table) {
            ...
            foreach ($results as $result) {
                ...
                // ใช้ลูปดึงข้อมูลทีละแถวแทน array_merge เพื่อคุม Memory
                foreach ($data as $row) {
                    $fileData[$table][] = $row;
                }

                // [จุดที่ 1 ลด Memory] เคลียร์ข้อมูลก้อนใหญ่จาก DB ทันทีที่ย้ายเสร็จในแต่ละรอบ
                unset($data);
            }
            // [จุดที่ 2 ลด Memory] เคลียร์ผลลัพธ์โครงสร้าง Query ของตารางนี้
            unset($results);
        }

        // สร้างไฟล์ .txt ลงโฟลเดอร์เฉพาะกิจ
        foreach ($tables as $table) {
            ...
            // [จุดที่ 3 ลด Memory สำคัญมาก] เขียนเสร็จ 1 แฟ้ม ลบ Data ของแฟ้มนั้นออกจากแรมทันที!
            // ทำให้แรมไม่บวมสะสม โดยเฉพาะแฟ้มใหญ่อย่าง adp หรือ dru
            unset($fileData[$table], $data);
        }

        // เคลียร์ตัวแปรที่เกี่ยวกับ data ทั้งหมดก่อนเริ่มกระบวนการ API
        unset($fileData);

        // รายการแฟ้มที่จะส่ง API
        $fileList = [
            'ins' => 'UTF-8', 'pat' => 'UTF-8', 'opd' => 'CP874',
            'oop' => 'UTF-8', 'orf' => 'UTF-8', 'odx' => 'UTF-8',
            'dru' => 'CP874', 'cha' => 'UTF-8', 'cht' => 'UTF-8',
            'aer' => 'UTF-8', 'adp' => 'UTF-8',
        ];

        // สร้าง payload จากโฟลเดอร์ย่อยเฉพาะของเคสนี้
        $results = $this->buildApiPayload($baseDirectory, $fileList);

        // เรียกส่ง API พร้อมบันทึก log
        $this->sendToNhsoApi($results, $visits, $hn);

        // 3. ส่งผลลัพธ์กลับแบบ JSON เมื่อกระบวนการทำงานสำเร็จ
        return [
            'status' => 'success',
            'hn' => $hn,
            'visit' => $visit,
...
public function actionCheckData()
{
    ...
    try {
        ...
        // ✅ 11 แฟ้ม เหมือน actionData
        // true  = บังคับมีข้อมูล
        // false = ไม่บังคับ (oop, dru, orf, aer)
        $tables = [
            'pat' => true,
            'opd' => true,
            'odx' => true,
        ...
        // ✅ ดึง query ของทุก table ไว้ก่อน เหมือน actionData
        $tableQueries = [];
        foreach (array_keys($tables) as $table) {
            try {
                $tableQueries[$table] = $db2->createCommand(
                    "SELECT main_query FROM fdh_opofc WHERE main_table = :table"
                )->bindValue(':table', $table)->queryAll();
            } catch (\Exception $e) {
                $tableQueries[$table] = [];
                Yii::warning("fdh_opofc query error table={$table}: " . $e->getMessage(), 'checkdata');
            }
        }
        ...
        foreach ($tables as $table => $required) {
            ...
            try {
                $rows = $tableQueries[$table] ?? [];

                if (empty($rows)) {
                    $status  = $required ? 'no_config' : 'na';
                    $message = 'ไม่พบ query config ใน fdh_opofc';
                ...
                $status  = 'error';
                $message = $e->getMessage();
                Yii::error("checkData error table={$table} visit={$visit}: " . $e->getMessage(), 'checkdata');
                if ($required) $hasError = true;
            }
            ...
            $result[] = [
                ...
                'message'      => $message,
                'rows'         => $allData,
                'rows_invoice' => $invoiceData,  // ✅ ADP ส่งใบเสร็จให้ _check_modal.php
            ];
        }
...
foreach ($visits as $visit) {
	...
	if ($visitCount > 0) {
		// อัปเดต
		$logSQLUpdate = "UPDATE log_fdh_opd_ck SET pid = :pid, messages = :messages, messagecode = :messagecode, response = :response, users = 'opofc', d_update = NOW() WHERE visit_id = :visit_id";
		...
		$logSQLInsert = "INSERT INTO log_fdh_opd_ck (visit_id, pid, messages, messagecode, response, users, d_update)
						 VALUES (:visit_id, :pid, :messages, :messagecode, :response, 'opofc', NOW())";
...
public function actionExports()
	{
		...
		$zipFilename = $baseDirectory . 'F16_10953_opofc_' . $currentDateTime . '.zip';
