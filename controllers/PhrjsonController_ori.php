...
LEFT JOIN hospitals hs1 ON hs1.hosp_id = uc.hospsub
   where o.visit_id = '$visit'";
       ...
       ,o.BP_DIAS as bp_diastolic
        ...
        WHERE o.visit_id='$visit'
        ";
           $datavitalSigns = \yii::$app->db14->createCommand($strVituals)->queryAll();
...
//}
 ############# Send Moph-PHR #############
       ...
       // sleep(5);
        ...
        //print_r($phr);
       $ok = $phr['Message'];
        $result = $phr['processing_warning'][0];
...
print_r ($ok);
        ...
        */
        ...
        if (strlen($response) > 0) {
            $strSQL = "REPLACE INTO log_phr (visit_id, cid , pid,status, messagecode ,response , users, regdate,d_update) VALUES ('$visit','$cid','$hn','$ok200','$ok','$result' ,'api2','$regdate',NOW())";
