...
{
    // 1. ดักรับค่าวันที่ส่งมาจากหน้าเว็บ (รองรับทั้ง POST และ GET เพื่อป้องกันการหลุดเวลากดปุ่ม Filter)
    // 1. ดักรับค่าวันที่ส่งมาจากหน้าเว็บ (แก้ไขวงเล็บเกินเรียบร้อยครับ)
$date1x = Yii::$app->request->post('date1', Yii::$app->request->get('date1', date('Y-m-d', strtotime('-5 days'))));
$date2x = Yii::$app->request->post('date2', Yii::$app->request->get('date2', date('Y-m-d')));
    // 2. รับค่าสถานะ Filter ที่กดมาจากปุ่ม Dashboard (Default เป็น all)
    $statusFilter = Yii::$app->request->post('status', Yii::$app->request->get('status', 'all')); 

    // 3. กำหนดเวลาเซ็ตค่าเพื่อเอาไปใช้ยิงเข้าคำสั่ง SQL ค้นหาเท่านั้น (แยกตัวแปรไม่ให้ทับซ้อน)
    $date1_sql = date('Y-m-d 00:00:00', strtotime($date1x));
    $date2_sql = date('Y-m-d 23:59:59', strtotime($date2x));
    
    // ----------------------------------------------------
    // [คิวรี่หลัก] ดึงรายการผู้ป่วยตามเงื่อนไข (เปลี่ยนมาใช้ตัวแปร _sql)
    // ----------------------------------------------------
    $sqlData = "SELECT 
        @n := @n + 1 AS 'No',
        data.*
    ...
    (SELECT DATE_FORMAT(o.reg_datetime, '%Y-%m-%d %H:%i') as 'regdate'
        ...
        , CONCAT(
          ...
          GROUP_CONCAT(DISTINCT i9.code) as oper,
      ...
      LEFT JOIN icd9cm i9 ON i9.icd9 = op.icd9
      ...
      WHERE o.IS_CANCEL = 0
      AND o.REG_DATETIME BETWEEN '$date1_sql' AND '$date2_sql'
      AND o.INSCL in ('03','04','23','33','00') 
    ...
    $rawData = \Yii::$app->db7->createCommand($sqlData)->queryAll();

    // ----------------------------------------------------
    // [คิวรี่ Dashboard] ดึงสถิติตามเงื่อนไขช่วงเวลาที่เลือก
    // ----------------------------------------------------
    // 1. เคสทั้งหมด (Total) ในช่วงเวลานั้น
    $totalCases = count($rawData);

    // 2. จำนวนส่งแล้ว (มี messagecode คืนกลับมา)
    $sentCases = 0;
    foreach ($rawData as $row) {
        if (!empty($row['messagecode'])) {
            $sentCases++;
        }
    }
 ...
 // ── 3. คำนวณ stats + today ───────────────────────────────
    ...
    foreach ($rawData as $row) {
        $isUser  = ($row['users'] ?? '') === $targetUser;
        $hasSent = !empty($row['messagecode']) && $isUser;
        $isToday = $hasSent && strpos($row['d_update'] ?? '', $today) === 0;

        if ($hasSent)                   { $sentCases++;   }
        if (empty($row['messagecode'])) { $waitCases++;   }  // ✅ ไม่กรอง users
        if ($isToday)                   { $amountToday++; }

        $amt = (float)($row['amount']        ?? 0);
        $ret = (float)($row['ret_statement'] ?? 0);
        if ($isUser)                  { $totalAmount    += $amt; }
        if ($ret > 0 && $isUser)      { $compensated    += $ret; }
        elseif ($ret == 0 && $isUser) { $notCompensated += $amt; }
    }

    // ── 4. กรอง statusFilter ─────────────────────────────────
    $filteredData = $rawData;  // ✅ default แสดงทั้งหมด

    if ($statusFilter === 'success') {
        $filteredData = array_values(array_filter($filteredData, function($r) use ($targetUser) {
            return !empty($r['messagecode']) && ($r['users'] ?? '') === $targetUser;
        }));
    } elseif ($statusFilter === 'waiting') {
        $filteredData = array_values(array_filter($filteredData, function($r) {
            return empty($r['messagecode']);  // ✅ ไม่กรอง users
        }));
    } elseif ($statusFilter === 'not_compensated') {
        $filteredData = array_values(array_filter($filteredData, function($r) use ($targetUser) {
            return (float)($r['ret_statement'] ?? 0) == 0 && ($r['users'] ?? '') === $targetUser;
        }));
    } elseif ($statusFilter === 'today') {
        $filteredData = array_values(array_filter($filteredData, function($r) use ($today, $targetUser) {
            return !empty($r['messagecode'])
                && strpos($r['d_update'] ?? '', $today) === 0
                && ($r['users'] ?? '') === $targetUser;
        }));
    }
...
foreach ($vn as $r) {
	...
	$fileData = [];
	$emptyTables = []; // ใช้เก็บแฟ้มที่ไม่มีข้อมูล
    ...
    if ($table === 'pat') {
        ...
        $results = $db7->createCommand($query)->queryAll();
    } else {
        // ตรวจสอบว่ามีใน fdh_ins หรือไม่
        $queryIns = "SELECT main_query FROM fdh_ins WHERE main_table = :table";
        $results = $db7->createCommand($queryIns)
            ->bindValue(':table', $table)
            ->queryAll();

        // ถ้าไม่มีข้อมูลใน fdh_ins ให้ใช้จาก fdh_dent
        if (empty($results)) {
            $queryHerb = "SELECT main_query FROM fdh_ancdent WHERE main_table = :table";
            $results = $db7->createCommand($queryHerb)
                ->bindValue(':table', $table)
                ->queryAll();
        }
    }
#############################################################################
		foreach ($results as $result) {
			...
			$data = $db7->createCommand($mainQueryResult)->queryAll();
		...
		// ถ้าไม่มีข้อมูล ให้บันทึกชื่อแฟ้มที่ว่าง
		if (empty($fileData[$table])) {
			$emptyTables[] = $table;
		}
	}

	// 🔴 ถ้าพบว่าแฟ้มไหนไม่มีข้อมูล ให้แจ้งเตือนและหยุดทำงาน
	if (!empty($emptyTables)) {
		$emptyTablesList = implode(', ', $emptyTables);
		Yii::$app->session->setFlash('error', "❌ ไม่มีข้อมูลสำหรับ HN: $hn");
		return $this->redirect(['index']); // กลับไปที่หน้าเดิม
	}
	...
	foreach ($fileData as $table => $data) {
		...
		// ลบข้อมูลซ้ำ และแปลง PAT.txt เป็น Windows CRLF
		if ($table === 'pat') {
...
return $this->redirect(['index']); // กลับไปที่หน้า index
}
		...
		if (file_exists($chtFilePath)) {
			...
			while ($row = fgetcsv($file, 0, "|")) {
				...
				if ($visitCount > 0) {
				 ...
				 users = 'ancdent', datereg = :datereg, d_update = NOW() WHERE visit_id = :visit_id";
				...
				VALUES (:visit_id, :pid, :messages, :messagecode, :response, 'ancdent',:datereg ,NOW())";
	...
	public function actionExports()
{
	...
	$zipFilename = $baseDirectory . 'F16_10953_ancdent_' . $currentDateTime . '.zip';
