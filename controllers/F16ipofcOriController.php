        ...
        foreach ($vn as $r) {
           ...
           // echo $an;
            ...
            foreach ($tables as $table) {
                $query = "SELECT main_query FROM fdh_ipofc WHERE main_table = '$table'";
     ...
     private function sendToNhsoApi($payload, $visit, $an, $hn)
{
    ...
    if (!empty($an)) {
        ...
        $curl = curl_init();
        curl_setopt_array($curl, [
            CURLOPT_URL => 'https://nhsoapi.nhso.go.th/FMU/ecimp/v1/checkstatus',
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_SSL_VERIFYHOST => 0,
            CURLOPT_SSL_VERIFYPEER => 0,
            CURLOPT_ENCODING => '',
    ...
    if ($visitCount > 0) {
        $logSQLUpdate = "UPDATE log_fdh_ipd_ck 
            SET pid = :pid, 
                ...
                users = 'ipofc', 
                d_update = NOW()
            WHERE visit_id = :visit_id AND an = :an";
        Yii::$app->db2->createCommand($logSQLUpdate)
            ...
            ->bindValue(':claim_status', $statusMessage)
            ->bindValue(':claim_code', $statusCode)
            ->bindValue(':status_response', $statusResponse)
            ->bindValue(':visit_id', $visit)
            ->bindValue(':an', $an)
            ->execute();
    } else {
        $logSQLInsert = "INSERT INTO log_fdh_ipd_ck 
            (visit_id, pid, an, messages, messagecode, response, claim_status, claim_code, status_response, users, d_update) 
            VALUES (:visit_id, :pid, :an, :messages, :messagecode, :response, :claim_status, :claim_code, :status_response, 'ipofc', NOW())";
