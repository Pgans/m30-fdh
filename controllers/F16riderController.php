...
class F16riderController extends \yii\web\Controller
{
    ...
    {
        $date1x = Yii::$app->request->get('date1', date('Y-m-01')); // ค่าเริ่มต้น = ต้นเดือน
        ...
        (SELECT 
            ...
            LOCATE(' ', SUBSTRING_INDEX(lr.lab_result, '=', -1)) - 1
                ...
                LEFT(e.unit_name, 15) AS 'unit_name',
        ...
        // เรียงตามจำนวนรวมมากไปน้อย
        uasort($unitSummary, function($a, $b) { return $b['total'] - $a['total']; });

        // ==========================================
        // กรองรายการตาม status ที่เลือก
        // ==========================================
        ...
        // ==========================================
        // DataProvider สำหรับตาราง (กรองแล้ว)
        // ==========================================
    ...
    // ====================================================
    // actionData, actionRunCurl, และ method อื่นๆ
    // ====================================================
    private $visit;
    private $hn;
            ...
            foreach ($tables as $table) {
                if ($table === 'pat') {
                    ...
                    if (empty($results)) {
                        ...
                        $results    = $db2->createCommand($queryRider)->bindValue(':table', $table)->queryAll();
                    }
                }
    ...
    private function sendDataToAPI($visit, $hn, $baseDirectory)
    {
        ...
        if ($data && isset($data['token30'])) {
            ...
            foreach ($tables as $t) {
                ...
                if (file_exists($file)) $filePaths[] = $file;
            }
            ...
            while ($row = fgetcsv($file, 0, "|")) {
                ...
                if ($visitExists) {
                    $db->createCommand("UPDATE log_fdh_opd_ck SET pid=:pid, messages=:messages, messagecode=:messagecode, response=:response, users='rider', datereg=:datereg, d_update=NOW() WHERE visit_id=:visit_id")
                        ->bindValues([':pid' => $hn, ':messages' => $messages, ':messagecode' => $statusMessageTh, ':response' => $trackRes, ':visit_id' => $visit_id, ':datereg' => $datereg])->execute();
                } else {
                    $db->createCommand("INSERT INTO log_fdh_opd_ck (visit_id, pid, messages, messagecode, response, users, datereg, d_update) VALUES (:visit_id, :pid, :messages, :messagecode, :response, 'rider', :datereg, NOW())")
                        ->bindValues([':visit_id' => $visit_id, ':pid' => $hn, ':messages' => $messages, ':messagecode' => $statusMessageTh, ':response' => $trackRes, ':datereg' => $datereg])->execute();
                }
            }
            fclose($file);
        } else {
	...
	public function actionCheck()
{
    ...
    foreach ($vn as $r) {
        ...
        if (!empty($res['data']) && is_array($res['data'])) {
            ...
            // ✅ ถ้า API แจ้งว่าไม่มีรายการในระบบ
            // ให้ messagecode เป็นค่าว่าง
            if ($apiMessage == 'ไม่มีรายการนี้ส่งเข้ามาในระบบ') {
                $statusMessageTh = '';
            } else {
                $statusMessageTh = $apiMessage;
            }
