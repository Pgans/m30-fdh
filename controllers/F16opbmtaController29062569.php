...
// คงชื่อคลาสเดิมตามที่คุณแจ้งไว้ครับ
class F16opbmtaController extends \yii\web\Controller
{
    ...
    (SELECT 
       ...
       , CONCAT(
        ...
        // [ปรับปรุง] เปลี่ยนมานับ Log ในกลุ่มผู้ใช้ของระบบใหม่ 'opbmdata'
        $sqlCount1 = "SELECT COUNT( v.visit_id) as amount
        FROM log_fdh_opd_ck v 
        WHERE  v.users = 'opbmdata' AND v.messagecode <> ''
        ...
        // [ปรับปรุง] ดึงคิวประวัติส่งผ่านของระบบใหม่ 'opbmdata'
        ...
        AND l.messagecode <> '' AND l.users = 'opbmdata'
        ORDER BY l.d_update ASC";
        $rawDataPass = \Yii::$app->db2->createCommand($sqlPass)->queryAll();

        $passProvider = new \yii\data\ArrayDataProvider([
            'allModels' => $rawDataPass,
            'pagination' => [
                'pageSize' => 300,
            ],
        ]);

        // [ปรับปรุง] ดึงคิวประวัติเออเร่อของระบบใหม่ 'opbmdata'
        ...
        AND l.messagecode <> 'success' AND l.users = 'opbmdata'
        ORDER BY l.d_update DESC";
        $rawDataError = \Yii::$app->db2->createCommand($sqlError)->queryAll();

        $errorProvider = new \yii\data\ArrayDataProvider([
            'allModels' => $rawDataError,
            'pagination' => [
                'pageSize' => 350,
            ],
        ]);
   ...
   public function actionData()
    {
        // แก้ไขปัญหากระโดดหน้าข้ามฝั่ง (Redirect พัง) โดยการส่ง JSON ตอบกลับไปหา Ajax หน้าบ้านตรงๆ 
        ...
        try {
            ...
            // [เพิ่มความปลอดภัย] ตรวจรูปแบบของค่า visit+hn ที่ส่งมาจาก client ก่อนนำไปต่อ SQL ด้วย str_replace ด้านล่าง
            foreach ($vn as $rawVn) {
                if (!preg_match('/^[A-Za-z0-9]+$/', $rawVn)) {
                    return ['status' => 'error', 'message' => 'พบรูปแบบข้อมูลรายการที่ไม่ถูกต้อง'];
                }
            }
            ...
            // --- ล้างไฟล์เก่าและเขียนหัวตาราง (header) รอไว้ ---
            // เปิดด้วย mode 'w' (เขียนทับ) เฉพาะรอบนี้รอบเดียว เพื่อเคลียร์ไฟล์เก่าจากการรันครั้งก่อน
            foreach ($tables as $table) {
                $filePath = $fullPath . strtoupper($table) . '.txt';
                $header = $this->getHeaderForTable($table);
                $this->exportToTextFile([], $filePath, $header, false); // false = เขียนทับ (truncate)
            }
 
            // --- วนลูปสร้างคิวข้อมูลรายบุคคล ---
            foreach ($vn as $r) {
                $hn = substr($r, 10);
                $visit = substr($r, 0, 10);
                $visits[] = $visit;
                ...
                foreach ($tables as $table) {
                    // [ปรับปรุงจุดตาย] ดึงชุด Query โครงสร้างไฟล์ 16 แฟ้มจากตารางระบบใหม่ fdh_opbmdata
                    ...
                    foreach ($results as $result) {
                        ...
                        if (!empty($data)) {
                            $filePath = $fullPath . strtoupper($table) . '.txt';
                            // [แก้บั๊กสำคัญ] เดิมเปิดไฟล์แบบ 'w' (เขียนทับ) ทุกครั้งที่เรียก
                            // ทำให้ข้อมูลของ visit ก่อนหน้า / query ก่อนหน้าในตารางเดียวกันถูกทับหายไป
                            // แก้เป็น append (true) เพื่อต่อข้อมูลแทนการเขียนทับ
                            $this->exportToTextFile($data, $filePath, null, true);
                        }
                        unset($data);
                    }
                    unset($results);
            ...
            // ส่งข้อมูลขึ้นระบบ API และเก็บลง Logฐานข้อมูล
            $this->sendToNhsoApi($results, $visits, $hn);
 
            // คืนค่าสถานะความสำเร็จกลับไปให้ jQuery หน้าบ้านทำ Progress Bar 100% ได้อย่างสมบูรณ์
            return [
                'status' => 'success',
                'message' => 'ส่งข้อมูลสำเร็จเรียบร้อย',
                'visit' => !empty($vn) ? substr(end($vn), 0, 10) : null,
                'hn' => $hn
            ];
    ...
    private function sendToNhsoApi($payload, $visits = [], $hn = null)
    {
        ...
        // [แนะนำเพิ่ม] เช็ค curl error ก่อนถือว่าสำเร็จ
        $curlError = curl_error($curl);
        curl_close($curl);
 
        if ($curlError) {
            Yii::error("cURL error ตอนส่ง NHSO API: $curlError", __METHOD__);
        }
