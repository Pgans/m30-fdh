...
class F16stemiController extends \yii\web\Controller
{
...
(SELECT 
       ...
       , CONCAT(
        ...
        WHERE  v.users = 'stemi' AND v.messagecode <> ''
		#AND v.messages <> ''
        ...
        AND l.messagecode <> '' AND l.users = 'stemi'
        ...
        AND l.messagecode <> 'success' AND l.users = 'stemi'
        ORDER BY l.d_update DESC
    ...
    public function actionData()
    {
        ...
        foreach ($vn as $r) {
            ...
            $tables = ['ins', 'odx', 'opd', 'pat', 'cha', 'cht', 'oop', 'dru'];
            foreach ($tables as $table) {
                $query = "SELECT main_query FROM fdh_stemi WHERE main_table = '$table'";
                ...
                while ($row = fgetcsv($file, 0, "|")) {
                    ...
                    if ($visitCount > 0) {
                            ...
                            $logSQLUpdate = "UPDATE log_fdh_opd_ck SET pid = :pid, messages = :messages, messagecode = :messagecode, response = :response,
                             users = 'stemi', datereg = :datereg, d_update = NOW() WHERE visit_id = :visit_id";
                            ...
                            VALUES (:visit_id, :pid, :messages, :messagecode, :response, 'stemi',:datereg ,NOW())";
    ...
    public function actionExports()
        {
            ...
            $zipFilename = $baseDirectory . 'F16_10953_stemi_' . $currentDateTime . '.zip';
