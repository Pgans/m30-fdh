...
public function actionData()
{
    ...
    try {
        ...
        $hn = substr($r, 10);
        $visit = substr($r, 0, 10);
        $visits = [$visit]; // แปลงเป็น Array สำหรับส่งเข้าฟังก์ชันส่ง API เดิม
        
        $fileData = [];

        // 2. แยก Sub-folder ชั่วคราวตามเลข $visit ป้องกันไฟล์ .txt เขียนทับกันเวลากดส่งพร้อมกันหลายๆ เครื่อง
        ...
        foreach ($tables as $table) {
            ...
            foreach ($results as $result) {
                ...
                // ปรับใช้ loop ดึงข้อมูล แทน array_merge เพื่อช่วยลดภาระ Memory ของ Server
                foreach ($data as $row) {
                    $fileData[$table][] = $row;
                }
            }
        ...
        // เรียกส่ง API พร้อมบันทึก log (ตรวจสอบให้แน่ใจว่าหาก API ส่งไม่ผ่าน ฟังก์ชันนี้จะ throw Exception ออกมา)
        $this->sendToNhsoApi($results, $visits, $hn);

        // 3. ส่งผลลัพธ์กลับแบบ JSON เมื่อกระบวนการทำงานสำเร็จสำเร็จ
        return [
            'status' => 'success',
            'hn' => $hn,
            'visit' => $visit,
        ...
        // หากเกิดข้อผิดพลาดระหว่างทาง (เช่น DB ค้าง, สปสช ปฏิเสธ หรือโค้ดพัง) จะส่งสถานะ fail กลับหน้าบ้านทันที
        return [
            'status' => 'fail',
