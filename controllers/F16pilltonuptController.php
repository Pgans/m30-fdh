...
class F16pilltonuptController extends \yii\web\Controller
{
...
(SELECT 
       ...
       , CONCAT(
          ...
          AND p.NATN_ID = '99'
		  #AND pr.DRUG_ID ='2376' ###รหัสยาคุม 
		  AND lr.lab_id = '039'  #### UPT  
		  #AND o.visit_id = '0003319375'
        ...
        WHERE  v.users = 'pillupt' AND v.messagecode <> ''
		#AND v.messages <> ''
        ...
        AND l.messagecode <> '' AND l.users = 'pillupt'
        ...
        AND l.messagecode <> 'success' AND l.users = 'pillupt'
        ORDER BY l.d_update DESC
    ...
    $vn = Yii::$app->request->post('chkDel', []);
        ...
        foreach ($vn as $r) {
            ...
            foreach ($tables as $table) {
                $query = "SELECT main_query FROM fdh_pilltonupt WHERE main_table = '$table'";
                ...
                while ($row = fgetcsv($file, 0, "|")) {
                    ...
                    if ($visitCount > 0) {
                            ...
                            $logSQLUpdate = "UPDATE log_fdh_opd_ck SET pid = :pid, messages = :messages, messagecode = :messagecode, response = :response,
                             users = 'pillupt', datereg = :datereg, d_update = NOW() WHERE visit_id = :visit_id";
                            ...
                            VALUES (:visit_id, :pid, :messages, :messagecode, :response, 'pillupt',:datereg ,NOW())";
