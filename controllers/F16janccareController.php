...
class F16janccareController extends \yii\web\Controller
{
    public function actionIndex()
    {
        ...
        data.*
      FROM 
        (SELECT date_format(f.datecare,'%Y-%m-%d')as date_serv 
		...
		,log.messagecode
          FROM  fdhanc_care f
		  LEFT JOIN mbase_data1.opd_visits o ON o.hn = f.hn AND f.datecare = date(o.reg_datetime)  and o.is_cancel = 0
			LEFT JOIN mbase_data1.log_fdh_opd_ck   as log ON log.visit_id = f.visitno
            WHERE f.datecare BETWEEN '$date1' AND '$date2'
			#AND f.mapright = '0100'
        ...
        WHERE  v.users = 'janccare' AND v.messagecode <> ''
       ...
       AND l.messagecode <> '' AND l.users = 'janccare'
       ORDER BY l.d_update DESC
        ...
        AND l.messagecode <> 'success' AND l.users = 'janccare'
        ORDER BY l.d_update DESC
    ...
    public function actionData()
    {
        ...
        foreach ($vn as $r) {
            ...
            foreach ($tables as $table) {
                $query = "SELECT main_query FROM fdh_anccare WHERE main_table = '$table'";
                ...
                while ($row = fgetcsv($file, 0, "|")) {
                    ...
                    if ($visitCount > 0) {
                        ...
                        $logSQLUpdate = "UPDATE log_fdh_opd_ck SET pid = :pid, messages = :messages, messagecode = :messagecode, response = :response,
                         users = 'janccare', datereg = :datereg, d_update = NOW() WHERE visit_id = :visit_id";
                        ...
                        VALUES (:visit_id, :pid, :messages, :messagecode, :response, 'janccare',:datereg ,NOW())";
    ...
    public function actionExports()
    {
        ...
        $zipFilename = $baseDirectory . 'F16_10953_เยี่ยมหลังคลอด_' . $currentDateTime . '.zip';
