...
class F16implantsController extends \yii\web\Controller
{
...
(SELECT 
       ...
       , CONCAT(
         ...
         # dr.drug_name,
          ...
          #AND p.NATN_ID = 99
		      AND oo.code = '2337550'
         # AND o.visit_id in (0003098087,0003119543,0003128353,0003136492,0003137101,0003152951)
        ...
        WHERE  v.users = 'implants' AND v.messagecode <> ''
		#AND v.messages <> ''
        ...
        AND l.messagecode <> '' AND l.users = 'implants'
        ...
        AND l.messagecode <> 'success' AND l.users = 'implants'
        ORDER BY l.d_update DESC
    ...
    public function actionData()
    {
        ...
        foreach ($vn as $r) {
            ...
            foreach ($tables as $table) {
                $query = "SELECT main_query FROM fdh_implants WHERE main_table = '$table'";
                ...
                while ($row = fgetcsv($file, 0, "|")) {
                    ...
                    if ($visitCount > 0) {
                            ...
                            $logSQLUpdate = "UPDATE log_fdh_opd_ck SET pid = :pid, messages = :messages, messagecode = :messagecode, response = :response,
                             users = 'implants', datereg = :datereg, d_update = NOW() WHERE visit_id = :visit_id";
                            ...
                            VALUES (:visit_id, :pid, :messages, :messagecode, :response, 'implants',:datereg ,NOW())";
