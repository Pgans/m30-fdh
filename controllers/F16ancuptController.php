...
class F16ancuptController extends \yii\web\Controller
{
    public function actionIndex()
    {
      ...
      (SELECT
       ...
       , CONCAT(
        ...
        WHERE  v.users = 'ancupt' AND v.messagecode <> ''
        ...
        AND l.messagecode <> '' AND l.users = 'ancupt'
       ...
       AND l.messagecode <> 'success' AND l.users = 'ancupt'
       ORDER BY l.d_update DESC
...
/*
        foreach ($tables as $table) {
            $query = "SELECT main_query FROM fdh_anc_upt WHERE main_table = '$table'";
...
##################################################################
    ...
    if ($table === 'pat') {
        ...
        // ถ้าไม่มีข้อมูลใน fdh_ins ให้ใช้จาก fdh_anc_upt
        if (empty($results)) {
            $queryHerb = "SELECT main_query FROM fdh_anc_upt WHERE main_table = :table";
                ...
                $visitCount = Yii::$app->db2->createCommand($logSQLCheck)
                    ...
                    if ($visitCount > 0) {
                        ...
                        $logSQLUpdate = "UPDATE log_fdh_opd_ck SET pid = :pid, messages = :messages, messagecode = :messagecode, response = :response,
                         users = 'ancupt', datereg = :datereg, d_update = NOW() WHERE visit_id = :visit_id";
                        ...
                        VALUES (:visit_id, :pid, :messages, :messagecode, :response, 'ancupt',:datereg ,NOW())";
...
###########################################################################################################
	...
	public function actionExports()
    {
        ...
        $zipFilename = $baseDirectory . 'F16_10953_Ancupt_' . $currentDateTime . '.zip';
