...
class F16thairedController extends \yii\web\Controller
{
   ...
   , CONCAT(
    ...
    - (TIMESTAMPDIFF(YEAR, p.BIRTHDATE, o.REG_DATETIME) * 12)
	  ...
	  WHERE o.IS_CANCEL = 0
			...
			AND icd.icd10_tm = 'Z130'
			AND o.REG_DATETIME >= DATE_ADD(p.BIRTHDATE, INTERVAL 13 YEAR)
			AND o.REG_DATETIME <  DATE_ADD(p.BIRTHDATE, INTERVAL 45 YEAR)
			#AND TIMESTAMPDIFF(year,p.BIRTHDATE,o.REG_DATETIME) BETWEEN '13' AND '44'
        ...
        WHERE  v.users = 'thaired' AND v.messagecode <> ''
       ...
       AND l.messagecode <> '' AND l.users = 'thaired'
       ORDER BY l.d_update DESC
        ...
        AND l.messagecode <> 'success' AND l.users = 'thaired'
        ORDER BY l.d_update DESC
...
#############################################################################################
    ...
    if ($table === 'pat') {
        ...
        $results = $db2->createCommand($query)->queryAll();
            ...
            $queryHerb = "SELECT main_query FROM fdh_thaired WHERE main_table = :table";
            ...
            while ($row = fgetcsv($file, 0, "|")) {
                ...
                if ($visitCount > 0) {
                    ...
                    $logSQLUpdate = "UPDATE log_fdh_opd_ck SET pid = :pid, messages = :messages, messagecode = :messagecode, response = :response,
                     users = 'thaired', datereg = :datereg, d_update = NOW() WHERE visit_id = :visit_id";
                    ...
                    VALUES (:visit_id, :pid, :messages, :messagecode, :response, 'thaired',:datereg ,NOW())";
