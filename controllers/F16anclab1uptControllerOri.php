    ...
    public function actionIndex()
    {
      ...
      (SELECT
       ...
       , CONCAT(
		  ...
		  LEFT JOIN log_all.log_fdh_opd_ck   as log ON log.visit_id = o.visit_id
          WHERE o.IS_CANCEL = 0
   ...
   public function actionData()
{
    ...
    foreach ($results as $result) {
        ...
        // For 'odx', handle multiple records correctly
        if ($table === 'odx') {
            foreach ($data as $record) {
                $fileData[$table][] = $record; // Append each record
            }
        } else {
            // For other tables, merge data as usual
    ...
    foreach ($fileData as $table => $data) {
        ...
        // Additional processing for PAT table (remove duplicates, convert CRLF)
        if ($table === 'pat') {
    ...
    // Send data to API (one visit at a time)
