...
class F16opbangkokController extends \yii\web\Controller
{
...
, CONCAT(
  ...
  WHERE o.IS_CANCEL = 0
	...
	WHERE  v.users = 'opbangkok' AND v.messagecode <> ''
	#AND v.messages <> ''
	...
	AND l.messagecode <> '' AND l.users = 'opbangkok'
	...
	AND l.messagecode <> 'success' AND l.users = 'opbangkok'
	ORDER BY l.d_update DESC
...
{
    // 1. ตั้งค่าให้ตอบกลับเป็นรูปแบบ JSON เสมอเพื่อให้ฝั่ง AJAX หน้าบ้านรับไปทำคิวต่อได้
    ...
    try {
        ...
        // ดักกรณีไม่มีการติ๊กเลือกรายการส่งมา
        if (empty($vn)) {
            return [
                'status' => 'fail',
                'message' => 'ไม่มีการเลือกรายการ หรือข้อมูลส่งมาไม่ถูกต้อง'
            ];
        }

        // 2. แกะข้อมูลจากแถวปัจจุบันที่หน้าบ้านยิงมา (รับทีละ 1 ค่าจาก Array เสมอ)
        $r = $vn[0]; 
        $hn = substr($r, 10);
        $visit = substr($r, 0, 10);
        $visits = [$visit]; 
        ...
        // 3. แยก Sub-folder ตาม $visit ป้องกันการเขียนไฟล์ชนกันตอนทำงานแบบคิวถี่ ๆ
        ...
        // วนลูปดึงข้อมูลจากตารางคิวรี่หลัก (fdh_opbangkok) เฉพาะของ visit นี้
        foreach ($tables as $table) {
            $results = $db2->createCommand("SELECT main_query FROM fdh_opbangkok WHERE main_table = :table")
        ...
        // 4. เจนไฟล์ .txt แยกรายตารางลงใน Sub-folder เฉพาะกิจ
        foreach ($tables as $table) {
        ...
        // =======================================================
        // 🧠 [เพิ่มตรงนี้] ลด Memory ทันทีเมื่อเขียนไฟล์ .txt ลงดิสก์เสร็จแล้ว
        // =======================================================
        unset($fileData); 
        // =======================================================

        // รายการแฟ้มที่จะส่ง API และรหัสฟอนต์
        $fileList = [
            'ins' => 'UTF-8',
            'pat' => 'UTF-8',
            'opd' => 'CP874',
        ...
        // สร้าง payload จากไฟล์ภายในโฟลเดอร์ย่อยของเคสนี้
        $resultsPayload = $this->buildApiPayload($baseDirectory, $fileList);

        // 5. เรียกส่ง API ไปที่ฝั่งรับข้อมูล และทำการดักผลลัพธ์การตอบกลับ
        $apiResponse = $this->sendToNhsoApi($resultsPayload, $visits, $hn);

        // ดักผลลัพธ์: หาก API ปลายทางรีเทิร์นสถานะล้มเหลว ให้สั่งดีดลงบล็อก catch ทันทีเพื่อแจ้งหน้าบ้าน
        if (isset($apiResponse['status']) && $apiResponse['status'] === 'fail') {
            $errorMsg = isset($apiResponse['message']) ? $apiResponse['message'] : 'API ปลายทางปฏิเสธข้อมูล';
            throw new \Exception($errorMsg);
        }

        // 6. ส่งสถานะความสำเร็จกลับไปให้คอนโซล Log ของหน้าบ้าน (แสดงผลสีเขียว -> ส่งสำเร็จ)
        return [
            'status' => 'success',
            'hn' => $hn,
            'visit' => $visit,
        ...
        // หากมีขั้นตอนไหนพัง (คิวรี่ผิด, โฟลเดอร์เต็ม หรือ API ปลายทางยิงไม่ผ่าน) จะส่งสถานะ fail และพ่น Error ไปที่หน้าบ้านทันที
        return [
            'status' => 'fail',
            'hn' => isset($hn) ? $hn : 'ไม่ระบุ',
            'visit' => isset($visit) ? $visit : 'ไม่ระบุ',
...
foreach ($visits as $visit) {
	$messages = 'ส่งข้อมูล OP-BKK สำเร็จ';
	$messagecode = '200';
	...
	if ($visitCount > 0) {
		// อัปเดต
		$logSQLUpdate = "UPDATE log_fdh_opd_ck SET pid = :pid, messages = :messages, messagecode = :messagecode, response = :response, users = 'opbangkok', d_update = NOW() WHERE visit_id = :visit_id";
		...
		$logSQLInsert = "INSERT INTO log_fdh_opd_ck (visit_id, pid, messages, messagecode, response, users, d_update)
						 VALUES (:visit_id, :pid, :messages, :messagecode, :response, 'opbangkok', NOW())";
...
/*
###########################################################################################################################
	public function actionRunCurlxxxx()
	{
		...
		curl_setopt_array($curl, array(
			...
			CURLOPT_POSTFIELDS => json_encode([
			   'user' => 'phornmeena.10953',
			   'password_hash' => '1D3827484E7AC7811EB53D394B2524AE718AA66637B81920A43C4A06717D1963',
				'hospital_code' => '10953'
			]),
			CURLOPT_HTTPHEADER => [
				'Content-Type: application/json',
	   ...
	   try {
		...
		$sqltoken = "SELECT MAX(token) as token30, staff_id FROM fdh_token WHERE staff_id = 'thong'";
...
public function actionExports()
	{
		...
		$zipFilename = $baseDirectory . 'F16_10953_opbangkok_' . $currentDateTime . '.zip';
