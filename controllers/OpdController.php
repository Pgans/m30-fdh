...
// ตรวจสอบการเลือก 'SPECIFIC' หรือ 'ALL' สำหรับหน่วยบริการ
if ($mobile === 'SPECIFIC') {
    $sql1 .= " AND o.VISIT_ID NOT IN (SELECT mobile_visits.VISIT_ID FROM mobile_visits WHERE mobile_visits.IS_CANCEL = 0)";
}
...
// ตรวจสอบการเลือก 'SPECIFIC' หรือ 'ALL' สำหรับหน่วยบริการ
if ($mobile === 'SPECIFIC') {
    $sql2 .= " AND o.VISIT_ID NOT IN (SELECT mobile_visits.VISIT_ID FROM mobile_visits WHERE mobile_visits.IS_CANCEL = 0)";
}
...
// ตรวจสอบการเลือก 'SPECIFIC' หรือ 'ALL' สำหรับหน่วยบริการ
if ($mobile === 'SPECIFIC') {
    $sql4 .= " AND o.VISIT_ID NOT IN (SELECT mobile_visits.VISIT_ID FROM mobile_visits WHERE mobile_visits.IS_CANCEL = 0)";
}
...
// ตรวจสอบการเลือก 'SPECIFIC' หรือ 'ALL' สำหรับหน่วยบริการ
if ($mobile === 'SPECIFIC') {
    $sql5 .= " AND a.VISIT_ID NOT IN (SELECT mobile_visits.VISIT_ID FROM mobile_visits WHERE mobile_visits.IS_CANCEL = 0)";
}
...
$sqlCount1 = "SELECT COUNT(DISTINCT v.id) as amount
	...
	// ส่งค่าข้อมูลไปยัง View
        return $this->render('index', [
		    ...
		    'age_start' => $age_start, // ✅ ส่งค่า age_start ไปที่ View
            'age_end' => $age_end, // ✅ ส่งค่า age_end ไปที่ View
			'sql2' => $sql2,
			'amount' => $amount,
        ]);
    }
