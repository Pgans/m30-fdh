...
class F16ancdentlab2Controller extends \yii\web\Controller
{
   ...
   public function actionData()
{
    ...
    foreach ($vn as $r) {
        ...
        $fileData = [];
        $emptyTables = []; // เก็บรายการแฟ้มที่ไม่มีข้อมูล
/*
        foreach ($tables as $table) {
            $query = "SELECT main_query FROM fdh_ancdent_lab2 WHERE main_table = '$table'";
...
##################################################################
    ...
    if ($table === 'pat') {
        ...
        $results = $db14->createCommand($queryIns)
            ->bindValue(':table', $table)
            ->queryAll();

        // ถ้าไม่มีข้อมูลใน fdh_ins ให้ใช้จาก fdh_ancdent_lab2
        if (empty($results)) {
            $queryHerb = "SELECT main_query FROM fdh_ancdent_lab2 WHERE main_table = :table";
            $results = $db14->createCommand($queryHerb)
                ->bindValue(':table', $table)
                ->queryAll();
        }
    }
#############################################################################
            ...
            // ตรวจสอบว่าตารางนี้ว่างหรือไม่
            if (empty($fileData[$table])) {
                $emptyTables[] = $table;
            }
        }

        // 🔹 แจ้งเตือนหากมีแฟ้มที่ไม่มีข้อมูล
        if (!empty($emptyTables)) {
            $emptyTablesList = implode(', ', $emptyTables);
            Yii::$app->session->setFlash('error', "❌ ไม่มีข้อมูลสำหรับ HN: $hn");
            return $this->redirect(['index']); // เปลี่ยนเป็น action ที่ต้องการให้ย้อนกลับ
        }

        // 🔹 ส่งออกข้อมูลไปยังไฟล์ทีละ Visit
                ...
                $visitCount = Yii::$app->db2->createCommand($logSQLCheck)
                    ...
                    if ($visitCount > 0) {
                        ...
                        $logSQLUpdate = "UPDATE log_fdh_opd_ck SET pid = :pid, messages = :messages, messagecode = :messagecode, response = :response,
                         users = 'ancdentlab2', datereg = :datereg, d_update = NOW() WHERE visit_id = :visit_id";
                        ...
                        VALUES (:visit_id, :pid, :messages, :messagecode, :response, 'ancdentlab2',:datereg ,NOW())";
...
###########################################################################################################
	...
	public function actionExports()
    {
        ...
        $zipFilename = $baseDirectory . 'F16_10953_ancdentlab2_' . $currentDateTime . '.zip';
