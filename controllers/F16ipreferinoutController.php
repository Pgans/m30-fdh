...
class F16ipreferinoutController extends \yii\web\Controller
{
    ...
    public function actionIndex()
    {
        ...
        (SELECT DATE_FORMAT(o.reg_datetime, '%Y-%m-%d %H:%i') as 'regdate'
             ...
             , CONCAT(
              ...
              IFNULL(ak.claimcode, '') AS claimcode
				...
				LEFT JOIN refers rf ON rf.visit_id = ir.visit_id AND rf.is_cancel = 0
      ...
      AND ir.dsc_dt <> '0'
              ...
              AND rf.RF_TYPE = '2'
              AND ir.WARD_NO != '50'
      #AND o.visit_id  not in (SELECT vs.visit_id from log_all.log_fdh_ipd vs where vs.users='ipreferinout' )
      GROUP BY o.VISIT_ID
      ORDER BY ir.adm_id           
      ) AS data,
        (SELECT @n := 0) AS init
      ORDER BY 
        ...
        WHERE  v.users = 'ipreferinout' and v.messagecode <> '' 
        ...
        AND l.messagecode <> '' AND l.users = 'ipreferinout'
        ...
        FROM log_fdh_ipd_ck l 
        WHERE l.d_update BETWEEN CURDATE() AND NOW()
        AND l.messagecode <> 'success' AND l.users = 'ipreferinout'
        ORDER BY l.d_update DESC
    ...
    public function actionData()
    {
        ...
        // รับค่าตัวแปร referin ที่ส่งมาจากฟอร์ม
        $referin = Yii::$app->request->post('referin', null); // รับค่า referin ถ้าไม่มีค่า จะเป็น null
        
        // ทำการ echo ค่า referin เพื่อตรวจสอบ
        echo "Selected Referin: " . $referin . "<br>";
        
        // รับค่าของ vn ที่ส่งมาจากฟอร์ม
        $vn = Yii::$app->request->post('chkDel', []);
        
        // ตรวจสอบว่ามีการเลือกค่า vn มาหรือไม่
        if (!empty($vn)) {
            foreach ($vn as $r) {
                $an = substr($r, 10); // แยกค่า an จากรหัสที่ส่งมา
                $visit = substr($r, 0, 10); // แยกค่า visit จากรหัสที่ส่งมา
                $visits[] = $visit; // เก็บค่า visit ลงในอาเรย์
                $db2 = \Yii::$app->db2;
    
                $baseDirectory = 'uploads/fdh_ipd/';
                $mode = 0777;
    
                // ตารางที่จะใช้ในการดึงข้อมูล
                $tables = ['adp', 'ins', 'pat', 'ipd', 'iop', 'irf', 'idx','aer', 'dru', 'cha', 'cht' ];
                foreach ($tables as $table) {
                    $query = "SELECT main_query FROM fdh_ipreferinout WHERE main_table = :table";
                    $results = $db2->createCommand($query, [':table' => $table])->queryAll();
                    foreach ($results as $result) {
                        $mainQueryResult = $result['main_query'];
                        $mainQueryResult = str_replace('$visit', $visit, $mainQueryResult); // แทนที่ $visit ใน query
                        
                        // เพิ่มการใช้ $referin ในคิวรี่
                        $mainQueryResult = str_replace('$referin', $referin, $mainQueryResult);
                        ...
                        $fileData[$table] = array_merge($fileData[$table], $data); // รวมข้อมูลที่ดึงมาเก็บในอาเรย์
                    }
                }
                ...
                foreach ($fileData as $table => $data) {
                    ...
                    $this->exportToTextFile($data, $filePath, $header); // ส่งข้อมูลไปเขียนไฟล์
                }
        
                $this->sendDataToAPI($visit, $an); // ส่งข้อมูลไปยัง API
            
                // เก็บค่า visits และ an ใน session
            ...
            echo "ไม่มีข้อมูลที่เลือกในฟอร์ม"; // แจ้งเตือนเมื่อไม่มีการเลือกข้อมูล
        }
    }
            ...
            if ($data && isset($data['token30'])) {
                ...
                $statusMessageTh = isset($responseDecoded['data'][0]['status_message_th']) ? $responseDecoded['data'][0]['status_message_th'] : '';
                 ...
                 if ($visitCount > 0) {
                     ...
                     $logSQLUpdate = "UPDATE log_fdh_ipd_ck SET pid = :pid, messages = :messages, messagecode = :messagecode, response = :response, users = 'ipreferinout', d_update = NOW() WHERE visit_id = :visit_id AND an = :an";
                     ...
                     $logSQLInsert = "INSERT INTO log_fdh_ipd_ck (visit_id, pid, an, messages, messagecode, response, users, d_update) VALUES (:visit_id, :pid, :an, :messages, :messagecode, :response, 'ipreferinout', NOW())";
...
#########################################################################################################
    ...
    public function actionExports()
    {
        ...
        $zipFilename = $baseDirectory . 'F16_10953_Ipreferinout_' . $currentDateTime . '.zip';
