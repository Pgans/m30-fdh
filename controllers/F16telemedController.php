    ...
    public function actionIndex()
    {
        ...
        $sqlData = "
            ...
            FROM (
                ...
                AND (
                    o.UNIT_REG IN ('63','68','70','71','75','81','82','83','84','85','86')
                    OR o.UNIT_ID  IN ('63','68','70','71','75','81','82','83','84','85','86')
                )
        ...
        foreach ($rawData as $row) {
            if (isset($row['messagecode']) && trim($row['messagecode']) != '') {
                $claimedMonth++;
            } else {
                $remainingMonth++;
            }
        ...
        // =====================================================
        // สรุปจำนวนแยกแผนก (unit_name) — ใช้ข้อมูลที่โหลดแล้ว
        // =====================================================
        $unitSummary = [];
        foreach ($rawData as $row) {
            $unit = trim($row['unit_name'] ?? '') ?: 'ไม่ระบุ';
            if (!isset($unitSummary[$unit])) {
                $unitSummary[$unit] = ['total' => 0, 'sent' => 0, 'waiting' => 0];
            }
            $unitSummary[$unit]['total']++;
            if (isset($row['messagecode']) && trim($row['messagecode']) != '') {
                $unitSummary[$unit]['sent']++;
            } else {
                $unitSummary[$unit]['waiting']++;
            }
        }
        uasort($unitSummary, function($a, $b) { return $b['total'] - $a['total']; });

        // =====================================================
        // ผ่านวันนี้
        // =====================================================
        $sqlCount1 = "
            SELECT COUNT(*) AS amount FROM log_fdh_opd_ck v
            WHERE v.users = 'telemed' AND v.messagecode <> ''
            AND DATE(v.d_update) = CURDATE()
        ";
        $amount = Yii::$app->db2->createCommand($sqlCount1)->queryScalar();

        // =====================================================
        // ไม่ผ่านวันนี้
        // =====================================================
        $sqlCount2 = "
            SELECT COUNT(*) AS amountx FROM log_fdh_opd_ck v
            WHERE v.users = 'telemed'
            AND (v.messagecode IS NULL OR v.messagecode = '')
            AND DATE(v.d_update) = CURDATE()
        ";
        $amountx = Yii::$app->db2->createCommand($sqlCount2)->queryScalar();
        ...
        $sqlPass = "
            ...
            AND l.messagecode <> '' AND l.users = 'telemed'
        ...
        $sqlError = "
            ...
            AND (l.messagecode IS NULL OR TRIM(l.messagecode) = '')
            AND l.users = 'telemed'
            ORDER BY l.d_update DESC
        ";
   ...
   public function actionData()
{
    ...
    if (empty($vn)) {
        return ['status' => 'error', 'message' => 'ไม่พบข้อมูลรายการที่ส่งมา'];
    }

    $db2          = Yii::$app->db2;
    $tables       = ['adp', 'ins', 'odx', 'opd', 'pat', 'cha', 'cht'];
    $baseDir      = Yii::getAlias('@webroot') . '/uploads/fdh_opd/';
    $successCount = 0;
    $visits       = [];
    $hn           = null;
    $visit        = null;

    // โหลด queryMap ครั้งเดียว
    $queryMap  = [];
    $patResult = $db2->createCommand("SELECT main_query FROM fdh_pat WHERE main_table = 'pat'")->queryAll();
    foreach ($patResult as $r) {
        $queryMap['pat'][] = $r['main_query'];
    }

    $otherTables = ['adp', 'ins', 'odx', 'opd', 'cha', 'cht'];
    $inList      = "'" . implode("','", $otherTables) . "'";

    $insResults = $db2->createCommand("SELECT main_table, main_query FROM fdh_ins WHERE main_table IN ($inList)")->queryAll();
    foreach ($insResults as $r) {
        $queryMap[$r['main_table']][] = $r['main_query'];
    }

    $teleResults = $db2->createCommand("SELECT main_table, main_query FROM fdh_telemed WHERE main_table IN ($inList)")->queryAll();
    foreach ($teleResults as $r) {
        if (!isset($queryMap[$r['main_table']])) {
            $queryMap[$r['main_table']][] = $r['main_query'];
        }
    }

    // วนลูปแต่ละ visit ใน batch
    foreach ($vn as $item) {
        $hn      = substr($item, 10);
        $visit   = substr($item, 0, 10);
        $visits[] = $visit;

        foreach ($tables as $table) {
            if (!isset($queryMap[$table])) continue;
            foreach ($queryMap[$table] as $rawQuery) {
                $sql    = str_replace('$visit', $visit, $rawQuery);
                $data   = $db2->createCommand($sql)->queryAll();
                $header = $this->getHeaderForTable($table);
                $this->exportToTextFile($data, $baseDir . strtoupper($table) . '.txt', $header);
                unset($data);
            }
        }
    ...
    return [
        'status'       => 'success',
        'message'      => "ส่งสำเร็จ $successCount รายการ",
...
##########################################################################################################
    private function getHeaderForTable($table)
    {
        switch ($table) {
            ...
            case 'cht': return ['HN','AN','DATE','TOTAL','PAID','PTTYPE','PERSON_ID','SEQ','OPD_MEMO','INVOICE_NO','INVOICE_LT'];
            ...
            case 'orf': return ['HN','DATEOPD','CLINIC','REFER','REFERTYPE','SEQ','REFERDATE'];
            case 'pat': return ['HCODE','HN','CHANGWAT','AMPHUR','DOB','SEX','MARRIAGE','OCCUPA','NATION','PERSON_ID','NAMEPAT','TITLE','FNAME','LNAME','IDTYPE'];
            case 'aer': return ['HN','AN','DATEOPD','AUTHAE','AEDATE','AETIME','AETYPE','REFER_NO','REFMAINI','IREFTYPE','REFMAINO','OREFTYPE','UCAE','EMTYPE','SEQ','AESTATUS','DALERT','TALERT'];
            default:    return [];
        }
    }
        ...
        if ($data && isset($data['token30'])) {
            ...
            if (file_exists($chtFilePath)) {
                ...
                while ($row = fgetcsv($file, 0, "|")) {
                    ...
                    if ($visitCount > 0) {
                        Yii::$app->db2->createCommand("UPDATE log_fdh_opd_ck SET pid=:pid, messages=:messages, messagecode=:messagecode, response=:response, users='telemed', datereg=:datereg, d_update=NOW() WHERE visit_id=:visit_id")
                            ->bindValues([':pid' => $hn, ':messages' => $messages, ':messagecode' => $statusMessageTh, ':response' => $response, ':visit_id' => $visit, ':datereg' => $datereg])
                            ->execute();
                    } else {
                        Yii::$app->db2->createCommand("INSERT INTO log_fdh_opd_ck (visit_id, pid, messages, messagecode, response, users, datereg, d_update) VALUES (:visit_id, :pid, :messages, :messagecode, :response, 'telemed', :datereg, NOW())")
                            ->bindValues([':visit_id' => $visit, ':pid' => $hn, ':messages' => $messages, ':messagecode' => $statusMessageTh, ':response' => $response, ':datereg' => $datereg])
                            ->execute();
                    }
                }
                fclose($file);
    ...
    public function actionReadFile($fileName)
    {
        ...
        try {
            if (file_exists($filePath)) {
                ...
                $fileContent = nl2br(\yii\helpers\Html::encode($fileContent));
                return $this->renderPartial('readFile', ['content' => $fileContent, 'fileName' => $fileName]);
            } else {
	...
	public function actionCheck()
{
    ...
    foreach ($vn as $r) {
        ...
        $hn      = substr($r, 10, 6);
 Yii::info("raw=$r | visitId=$visitId | hn=$hn", 'fdh-check'); // ✅ ดู log
