...
class F16opsec8Controller extends \yii\web\Controller
{
public function actionIndex()
{
   ...
   , CONCAT(
	...
	WHERE  v.users = 'opsec8' AND v.messagecode <> ''
	#AND v.messages <> ''
	...
	AND l.messagecode <> '' AND l.users = 'opsec8'
	...
	AND l.messagecode <> 'success' AND l.users = 'opsec8'
	ORDER BY l.d_update DESC
...
$tables = ['adp', 'ins', 'odx', 'opd', 'pat', 'cha', 'cht', 'aer', 'oop', 'orf', 'dru'];
...
foreach ($vn as $r) {
	...
	foreach ($tables as $table) {
		$query = "SELECT main_query FROM fdh_opsec8 WHERE main_table = '$table'";
			...
			while ($row = fgetcsv($file, 0, "|")) {
				...
				if ($visitCount > 0) {
						...
						$logSQLUpdate = "UPDATE log_fdh_opd_ck SET pid = :pid, messages = :messages, messagecode = :messagecode, response = :response,
						 users = 'opsec8', datereg = :datereg, d_update = NOW() WHERE visit_id = :visit_id";
						...
						VALUES (:visit_id, :pid, :messages, :messagecode, :response, 'opsec8',:datereg ,NOW())";
...
public function actionExports()
	{
		...
		$zipFilename = $baseDirectory . 'F16_10953_opsec8_' . $currentDateTime . '.zip';
