...
class F16fittestController extends \yii\web\Controller
{
    public function actionIndex()
    {
         $date1 = Yii::$app->request->get('date1', date('Y-m-d'));
		$date2 = Yii::$app->request->get('date2', date('Y-m-d'));

		$date1_full = date('Y-m-d 00:01', strtotime($date1));
		$date2_full = date('Y-m-d 23:59', strtotime($date2));
   ...
   , CONCAT(
	  ...
	  (
    TIMESTAMPDIFF(MONTH, p.BIRTHDATE, o.REG_DATETIME)
    - (TIMESTAMPDIFF(YEAR, p.BIRTHDATE, o.REG_DATETIME) * 12)
) AS age_month,
	  ...
	  WHERE o.IS_CANCEL = 0
			...
			AND icd.icd10_tm = 'Z121'
            #AND e.unit_id = '45'
			AND o.REG_DATETIME >= DATE_ADD(p.BIRTHDATE, INTERVAL 50 YEAR)
			AND o.REG_DATETIME <  DATE_ADD(p.BIRTHDATE, INTERVAL 70 YEAR)
			#AND TIMESTAMPDIFF(year,p.BIRTHDATE,o.REG_DATETIME) BETWEEN '50' AND '70'
        ...
        WHERE  v.users = 'fittest' AND v.messagecode <> ''
       ...
       AND l.messagecode <> '' AND l.users = 'fittest'
       ORDER BY l.d_update DESC
        ...
        AND l.messagecode <> 'success' AND l.users = 'fittest'
        ORDER BY l.d_update DESC
        
         ";
        $rawData = \Yii::$app->db70->createCommand($sqlError)->queryAll();
    ...
    foreach ($vn as $r) {
        ...
        foreach ($tables as $table) {
            ...
            if ($table === 'pat') {
                ...
                $results = $db2->createCommand($query)
...
public function actionData()
{
    ...
    if (empty($vn)) {
        Yii::$app->session->setFlash('error', 'กรุณาเลือกรายการที่ต้องการประมวลผล');
    ...
    $allFileData = [];
    $baseDirectory = 'uploads/fdh_opd/';
    $tables = ['adp', 'ins', 'odx', 'opd', 'pat', 'cha', 'cht', 'oop'];

    // ตรวจสอบการเชื่อมต่อฐานข้อมูล
    $db2 = Yii::$app->db2;
    if (!$db2) {
        throw new \Exception('Database connection db2 ไม่ถูกต้อง');
    }
    ...
    foreach ($vn as $r) {
        ...
        foreach ($tables as $table) {
            try {
                ...
                // ดึง main_query ตามตาราง
                if ($table === 'pat') {
                    $query = "SELECT main_query FROM fdh_pat WHERE main_table = 'pat'";
                    $results = $db2->createCommand($query)->queryAll();
                } else {
                    // ตรวจสอบจาก fdh_fittest สำหรับตารางอื่นๆ
                    $queryHerb = "SELECT main_query FROM fdh_fittest WHERE main_table = :table";
                    $results = $db2->createCommand($queryHerb)
                                   ->bindValue(':table', $table)
                                   ->queryAll();
                }

                // ถ้าไม่มี main_query ให้ข้าม
                if (empty($results)) {
                    Yii::warning("No main_query found for table: $table, visit: $visit");
                    continue;
                }

                // ประมวลผล main_query แต่ละตัว
                foreach ($results as $result) {
                    $mainQueryResult = $result['main_query'] ?? '';

                    if (empty($mainQueryResult)) {
                        Yii::warning("Empty main_query for table: $table, visit: $visit");
                        continue;
                    }

                    // แทนค่า placeholders
                    $mainQueryResult = str_replace('$visit', $visit, $mainQueryResult);
                    $mainQueryResult = str_replace('$hn', $hn, $mainQueryResult);
                    ...
                    try {
                        ...
                        Yii::error("Error executing main_query for table $table, visit $visit: " . $e->getMessage());
                    }
                }
                
            } catch (\Exception $e) {
                Yii::error("Error processing table $table for visit $visit: " . $e->getMessage());
            }
        }

        // รวมข้อมูล visit ปัจจุบันเข้ากับ allFileData
        foreach ($fileData as $table => $data) {
            if (!isset($allFileData[$table])) {
                $allFileData[$table] = [];
            }
            $allFileData[$table] = array_merge($allFileData[$table], $data);
        }

        // ส่งข้อมูลไป API สำหรับ visit ปัจจุบัน
        try {
            $this->sendDataToAPI($visit, $hn);
        } catch (\Exception $e) {
            Yii::error("Error sending data to API for visit $visit: " . $e->getMessage());
        }
    }

    // Export ข้อมูลทั้งหมด
    foreach ($allFileData as $table => $data) {
        if (empty($data)) {
            Yii::warning("No data to export for table: $table");
            continue;
        }
        ...
        try {
            ...
            // ประมวลผลพิเศษสำหรับตาราง pat
            if ($table === 'pat') {
                $this->removeDuplicatesFromFile($filePath);
                $this->convertToWindowsCrLf($filePath);
            }
        } catch (\Exception $e) {
            Yii::error("Error exporting table $table: " . $e->getMessage());
        }
    }

    // เก็บ session
    Yii::$app->session->set('visits', $visits);
    Yii::$app->session->set('hn', $lastHn);
    Yii::$app->session->setFlash('success', 'ประมวลผลข้อมูลเรียบร้อย จำนวน ' . count($visits) . ' visit');
     ...
     private function sendDataToAPI($visit, $hn)
{
            ...
            while ($row = fgetcsv($file, 0, "|")) {
                ...
                if ($visitCount > 0) {
                    ...
                    $logSQLUpdate = "UPDATE log_fdh_opd_ck SET pid = :pid, messages = :messages, messagecode = :messagecode, response = :response,
                     users = 'fittest', datereg = :datereg, d_update = NOW() WHERE visit_id = :visit_id";
    ...
    $date2 = $request->get('date2', $request->post('date2'));
        ...
        try {
            ...
            // Retrieve the latest token for the staff_id 'nim'
            $sqltoken = "SELECT MAX(token) as token30, staff_id FROM fdh_token WHERE staff_id = 'nim'";
...
##############################################################################
    public function actionExports()
    {
        ...
        $zipFilename = $baseDirectory . 'F16_10953_FitTest_' . $currentDateTime . '.zip';
