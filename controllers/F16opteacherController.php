...
class F16opteacherController extends \yii\web\Controller
{
...
(SELECT 
       ...
       , CONCAT(
        ...
        WHERE  v.users = 'ooprailway' AND v.messagecode <> ''
		#AND v.messages <> ''
        ...
        AND l.messagecode <> '' AND l.users = 'oprailway'
        ...
        AND l.messagecode <> 'success' AND l.users = 'oprailway'
        ORDER BY l.d_update DESC
...
public function actionData()
{
    ...
    try {
        ...
        foreach ($tables as $table) {
            $results = $db2->createCommand("SELECT main_query FROM fdh_oppvt WHERE main_table = :table")
...
private function sendToNhsoApi($payload, $visits = [], $hn = null)
{
    ...
    foreach ($visits as $visit) {
        $messages = 'ส่งข้อมูล OP-PVT สำเร็จ';
        $messagecode = '200';
        ...
        if ($visitCount > 0) {
            // อัปเดต
            $logSQLUpdate = "UPDATE log_fdh_opd_ck SET pid = :pid, messages = :messages, messagecode = :messagecode, response = :response, users = 'oppvt', d_update = NOW() WHERE visit_id = :visit_id";
            ...
            $logSQLInsert = "INSERT INTO log_fdh_opd_ck (visit_id, pid, messages, messagecode, response, users, d_update)
                             VALUES (:visit_id, :pid, :messages, :messagecode, :response, 'oppvt', NOW())";
...
public function actionExports()
    {
        ...
        $zipFilename = $baseDirectory . 'F16_10953_OP_PVT_' . $currentDateTime . '.zip';
    ...
    if ($httpCode == 200) {
        ...
        if (isset($data['token'])) {
            try {
                ...
                Yii::$app->session->setFlash('success', "สร้าง token 6213033241622 สำเร็จแล้วและลบข้อมูลเก่าทั้งหมดเรียบร้อย");
            } catch (\Exception $e) {
