...
class F16opinstruController extends \yii\web\Controller
{
    public function actionIndex()
    {
       ...
       , CONCAT(
          ...
          LEFT JOIN authen_kiosk ak ON p.CID = ak.cid  AND date(o.REG_DATETIME)=date(ak.d_update)
					...
					LEFT JOIN visit_invoice v on v.visit_id = o.visit_id AND v.is_cancel = 0
          ...
          WHERE o.IS_CANCEL = 0
				...
				AND ps.drug_id NOT IN ('0364','0365','0366','0367','0368','0369','0370','0371','0372','0373')
				AND v.chrgitem = '21'
        ...
        WHERE  v.users = 'opstru' AND v.messagecode <> ''
        ...
        AND l.messagecode <> '' AND l.users = 'opstru'
    ...
    public function actionData()
{
    ...
    foreach ($vn as $r) {
        ...
        foreach ($tables as $table) {
            $query = "SELECT main_query FROM fdh_opinstrument WHERE main_table = '$table'";
...
public function actionCheckData()
{
    ...
    try {
        ...
        // ✅ ใช้ตารางเดียวกับ actionData เดิม
        $tables = [
            'pat' => true,
            'opd' => true,
            'odx' => true,
        ...
        foreach ($tables as $table => $required) {
            ...
            try {
                // ✅ ใช้ fdh_opinstrument เหมือน actionData เดิม
                $query = "SELECT main_query FROM fdh_opinstrument WHERE main_table = :table";
                $rows  = $db2->createCommand($query)
                             ->bindValue(':table', $table)
                             ->queryAll();
 
                if (empty($rows)) {
                    $status  = $required ? 'no_config' : 'na';
                    $message = 'ไม่พบ query config ใน fdh_opinstrument';
                    ...
                    foreach ($rows as $row) {
                        ...
                        // ✅ replace $visit เหมือน actionData เดิม
                ...
                $visitCount = Yii::$app->db2->createCommand($logSQLCheck)
                    ...
                    if ($visitCount > 0) {
                        ...
                        $logSQLUpdate = "UPDATE log_fdh_opd_ck SET pid = :pid, messages = :messages, messagecode = :messagecode, response = :response,
                         users = 'opstru', datereg = :datereg, d_update = NOW() WHERE visit_id = :visit_id";
                        ...
                        VALUES (:visit_id, :pid, :messages, :messagecode, :response, 'opstru',:datereg ,NOW())";
...
###########################################################################################################
	...
	public function actionExports()
    {
        ...
        $zipFilename = $baseDirectory . 'F16_10953_opd_instru_' . $currentDateTime . '.zip';
