...
class F16ipstpController extends \yii\web\Controller
{
    ...
    public function actionIndex()
    {
      ...
      ORDER BY 
        ...
        AND l.messagecode <> '' AND l.users = 'ipstp'
        ...
        FROM log_fdh_ipd_ck l 
        WHERE l.d_update BETWEEN CURDATE() AND NOW()
        AND l.messagecode <> 'success' AND l.users = 'ipstp'
        ORDER BY l.d_update DESC
   ...
   public function actionData()
{
    ...
    $tablesToCheck = ['adp', 'ins', 'pat', 'ipd', 'idx', 'cha', 'cht']; // เฉพาะแฟ้มที่ต้องเช็ค

    foreach ($vn as $r) {
        $an = substr($r, 10);
        $visit = substr($r, 0, 10);
        echo "visit_id: $visit, an: $an <br>";

        $fileData = [];
        $emptyTables = []; // เก็บรายการแฟ้มที่ไม่มีข้อมูล

        foreach ($tables as $table) {
            $query = "SELECT main_query FROM fdh_ipstp WHERE main_table = '$table'";
            ...
            // ตรวจสอบว่าแฟ้มที่ต้องเช็คมีข้อมูลหรือไม่
            if (in_array($table, $tablesToCheck) && empty($fileData[$table])) {
                $emptyTables[] = $table;
            }
        }

        // ❌ ถ้ามีแฟ้มว่างให้แจ้งเตือนและข้ามการส่ง API
        if (!empty($emptyTables)) {
            $emptyTablesList = implode(', ', $emptyTables);
            Yii::$app->session->setFlash('error', "❌ ไม่มีข้อมูลสำหรับ AN: $an");
            ...
            if ($data && isset($data['token30'])) {
                ...
                $statusMessageTh = isset($responseDecoded['data'][0]['status_message_th']) ? $responseDecoded['data'][0]['status_message_th'] : '';
                 ...
                 if ($visitCount > 0) {
                     ...
                     $logSQLUpdate = "UPDATE log_fdh_ipd_ck SET pid = :pid, messages = :messages, messagecode = :messagecode, response = :response, users = 'ipstp', d_update = NOW() WHERE visit_id = :visit_id AND an = :an";
                     ...
                     $logSQLInsert = "INSERT INTO log_fdh_ipd_ck (visit_id, pid, an, messages, messagecode, response, users, d_update) VALUES (:visit_id, :pid, :an, :messages, :messagecode, :response, 'ipstp', NOW())";
...
#########################################################################################################
    ...
    public function actionExports()
    {
        ...
        $zipFilename = $baseDirectory . 'F16_10953_Ipstp_' . $currentDateTime . '.zip';
