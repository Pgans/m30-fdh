...
class F16ipbmtaController extends \yii\web\Controller
{
    ...
    public function actionIndex()
    {
      ...
      AND ir.dsc_dt <> '0'
              ...
              AND ir.WARD_NO != '50'
      #AND o.visit_id  not in (SELECT vs.visit_id from log_all.log_fdh_ipd vs where vs.users='ipbmta' )
      GROUP BY o.VISIT_ID
      ORDER BY ir.adm_id           
      ) AS data,
        (SELECT @n := 0) AS init
      ORDER BY 
        ...
        WHERE  v.users = 'ipbmta' AND v.messagecode <> ' '
        ...
        AND l.messagecode <> ' ' AND l.users = 'ipbmta'
        ...
        foreach ($vn as $r) {
           ...
           // echo $an;
            ...
            foreach ($tables as $table) {
                $query = "SELECT main_query FROM fdh_ipbmta WHERE main_table = '$table'";
            ...
            if ($data && isset($data['token30'])) {
                ...
                $statusMessageTh = isset($responseDecoded['data'][0]['status_message_th']) ? $responseDecoded['data'][0]['status_message_th'] : '';
                 ...
                 if ($visitCount > 0) {
                     ...
                     $logSQLUpdate = "UPDATE log_fdh_ipd_ck SET pid = :pid, messages = :messages, messagecode = :messagecode, response = :response, users = 'ipbmta', d_update = NOW() WHERE visit_id = :visit_id AND an = :an";
                     ...
                     $logSQLInsert = "INSERT INTO log_fdh_ipd_ck (visit_id, pid, an, messages, messagecode, response, users, d_update) VALUES (:visit_id, :pid, :an, :messages, :messagecode, :response, 'ipbmta', NOW())";
