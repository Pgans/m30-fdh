...
class F16ipbangkokController extends \yii\web\Controller
{
    ...
    public function actionIndex()
    {
      ...
      AND ir.dsc_dt <> '0'
              ...
              AND ir.WARD_NO != '50'
      #AND o.visit_id  not in (SELECT vs.visit_id from log_all.log_fdh_ipd vs where vs.users='ipbangkok' )
      ...
      ORDER BY 
        ...
        AND l.messagecode <> ' ' AND l.users = 'ipbangkok'
   ...
   public function actionData()
{
    ...
    foreach ($vn as $r) {
        ...
        foreach ($tables as $table) {
            $results = $db2->createCommand("SELECT main_query FROM fdh_ipbangkok WHERE main_table = :table")
...
private function sendToNhsoApi($payload, $visit, $an, $hn)
{
    ...
    if ($visitCount > 0) {
        // อัปเดต log
        $logSQLUpdate = "UPDATE log_fdh_ipd_ck SET pid = :pid, messages = :messages, messagecode = :messagecode, response = :response, users = 'ipbangkok', d_update = NOW() WHERE visit_id = :visit_id AND an = :an";
        ...
        // เพิ่ม log ใหม่
        $logSQLInsert = "INSERT INTO log_fdh_ipd_ck (visit_id, pid, an, messages, messagecode, response, users, d_update) VALUES (:visit_id, :pid, :an, :messages, :messagecode, :response, 'ipobangkok', NOW())";
