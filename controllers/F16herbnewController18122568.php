...
class F16herbnewController extends \yii\web\Controller
{
    public function actionIndex()
    {
      ...
      FROM 
(SELECT * FROM 
(SELECT o.REG_DATETIME,o.HN,GROUP_CONCAT(DISTINCT TRIM(d.DRUG_NAME)) as 'Herb'
,cv.ret_statement,cv.hosp_claim,cv.stm_claim,cv.tran_id,cv.rep_no,o.VISIT_ID
, CONCAT(
          CASE 
                 WHEN c.PRENAME not in('') THEN TRIM(c.PRENAME)
                       ...
                       ELSE 'นาง' 
                END ,TRIM(c.FNAME),'  ',TRIM(c.LNAME)) as 'fullname',
...
LEFT JOIN close_visits  as cl ON cl.visit_id = o.visit_id
LEFT JOIN uc_inscl g ON c.CID= g.CID AND date(o.REG_DATETIME)>=g.uc_register AND (g.date_abort > date(o.REG_DATETIME) OR DAY(g.DATE_ABORT)=0)  and trim(g.hospmain) <>''
...
AND g.hospmain in ('10953') 
AND v.is_cancel = 0
AND o.IS_CANCEL = 0
#AND cv.rep_no = ''
GROUP BY o.VISIT_ID,v.visit_id
ORDER BY cv.ret_statement,o.REG_DATETIME) AS T
WHERE T.Herb !=''
) AS data,
        (SELECT @n := 0) AS init
      ORDER BY 
