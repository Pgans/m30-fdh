...
class F16anclab2Controller extends \yii\web\Controller
{
   ...
   public function actionData()
{
    ...
    foreach ($vn as $r) {
        ...
        $fileData = []; // Reset fileData for each visit
        $emptyTables = []; // ใช้เก็บชื่อแฟ้มที่ว่าง
/*
        foreach ($tables as $table) {
            $query = "SELECT main_query FROM fdh_anclab2 WHERE main_table = '$table'";
...
##################################################################
    ...
    if ($table === 'pat') {
        ...
        if (empty($results)) {
            $queryHerb = "SELECT main_query FROM fdh_anclab2 WHERE main_table = :table";
                ...
                $visitCount = Yii::$app->db2->createCommand($logSQLCheck)
                    ...
                    if ($visitCount > 0) {
                        ...
                        $logSQLUpdate = "UPDATE log_fdh_opd_ck SET pid = :pid, messages = :messages, messagecode = :messagecode, response = :response,
                         users = 'anclab2', datereg = :datereg, d_update = NOW() WHERE visit_id = :visit_id";
                        ...
                        VALUES (:visit_id, :pid, :messages, :messagecode, :response, 'anclab2',:datereg ,NOW())";
...
public function actionExports()
    {
        ...
        $zipFilename = $baseDirectory . 'F16_10953_AncLab2_' . $currentDateTime . '.zip';
