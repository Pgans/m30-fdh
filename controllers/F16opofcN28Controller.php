...
public function actionData()
{
    ...
    // เฉพาะแฟ้มจำเป็น
    $tables = ['ins', 'pat', 'opd', 'oop', 'orf', 'odx', 'dru', 'adp', 'cha', 'cht', 'aer'];
    $fileList = [
        'ins' => 'UTF-8',
        'pat' => 'UTF-8',
        'opd' => 'CP874',
    ...
    // ดึง main_query ล่วงหน้าเพื่อลด query ซ้ำ
    foreach ($tables as $table) {
        $mainQueries[$table] = $db2->createCommand("SELECT main_query FROM fdh_opofc WHERE main_table = :table")
    ...
    foreach ($vn as $r) {
        ...
        // เตรียมข้อมูลจากทุกแฟ้ม
        foreach ($tables as $table) {
            foreach ($mainQueries[$table] as $query) {
                $mainQuery = str_replace('$visit', $visit, $query);
        ...
        // เก็บค่า visit/hns สุดท้ายไว้ redirect
        $lastVisit = $visit;
        $lastHn = $hn;
    }

    Yii::$app->session->setFlash('info', 'ส่งข้อมูลเรียบร้อย');
    Yii::$app->session->set('visits', $vn);
