...
use  app\models\Fdhoptonpillsupt;
...
class F16optonpillsuptController extends \yii\web\Controller
{
...
(SELECT 
       ...
       , CONCAT(
        ...
        WHERE  v.users = 'oppillsupt' AND v.messagecode <> ''
        ...
        AND l.messagecode <> '' AND l.users = 'optonpillsupt'
    ...
    if ($table === 'pat') {
        ...
        // ถ้าไม่มีข้อมูลใน fdh_ins ให้ใช้จาก fdh_optonpillsupt
        if (empty($results)) {
            $queryHerb = "SELECT main_query FROM fdh_optonpillsupt WHERE main_table = :table";
...
/*
            foreach ($tables as $table) {
                $query = "SELECT main_query FROM fdh_optonpillsupt WHERE main_table = '$table'";
                    ...
                    $visitCount = Yii::$app->db2->createCommand($logSQLCheck)
                        ...
                        if ($visitCount > 0) {
                            ...
                            $logSQLUpdate = "UPDATE log_fdh_opd_ck SET pid = :pid, messages = :messages, messagecode = :messagecode, response = :response,
                             users = 'optonpillsupt', datereg = :datereg, d_update = NOW() WHERE visit_id = :visit_id";
                            ...
                            VALUES (:visit_id, :pid, :messages, :messagecode, :response, 'optonpillsupt',:datereg ,NOW())";
