...
use  app\models\Fdhancdentuslab2;
...
class F16ancdentuslab2Controller extends \yii\web\Controller
{
   ...
   public function actionData()
{
    ...
    foreach ($vn as $r) {
        ...
        $mode = 0777;
        $tables = ['adp', 'ins', 'oop', 'odx', 'opd', 'pat', 'cha', 'cht'];

        $fileData = [];
        $emptyTables = []; // เก็บตารางที่ไม่มีข้อมูล
/*
        foreach ($tables as $table) {
            $query = "SELECT main_query FROM fdh_ancdent_us_lab2 WHERE main_table = '$table'";
...
##################################################################
    ...
    if ($table === 'pat') {
        ...
        // ถ้าไม่มีข้อมูลใน fdh_ins ให้ใช้จาก fdh_ancdent_us_lab2
        if (empty($results)) {
            $queryHerb = "SELECT main_query FROM fdh_ancdent_us_lab2 WHERE main_table = :table";
...
#############################################################################
            foreach ($results as $result) {
                ...
                if (empty($data)) {
                    $emptyTables[] = $table; // เพิ่มตารางที่ไม่มีข้อมูลลงในรายการ
                } else {
                    $fileData[$table] = $data;
                }
            }
        ...
        foreach ($fileData as $table => $data) {
            ...
            // กรณีเป็น PAT.txt ให้ลบข้อมูลซ้ำและแปลงเป็น CRLF
            if ($table === 'pat') {
...
public function actionSubmit()
{
    ...
    // ทำบางอย่าง เช่นเก็บ session หรือส่งออกไฟล์

    // redirect กลับ พร้อมค่าที่เลือกไว้
    return $this->redirect(['index', 'date1' => $date1, 'date2' => $date2]);
}
                ...
                $visitCount = Yii::$app->db2->createCommand($logSQLCheck)
                    ...
                    if ($visitCount > 0) {
                        ...
                        $logSQLUpdate = "UPDATE log_fdh_opd_ck SET pid = :pid, messages = :messages, messagecode = :messagecode, response = :response,
                         users = 'ancdentuslab2', datereg = :datereg, d_update = NOW() WHERE visit_id = :visit_id";
                        ...
                        VALUES (:visit_id, :pid, :messages, :messagecode, :response, 'ancdentuslab2',:datereg ,NOW())";
