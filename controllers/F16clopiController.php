...
class F16clopiController extends \yii\web\Controller
{
...
(SELECT DATE_FORMAT(o.reg_datetime, '%Y-%m-%d %H:%i') as 'regdate'
       ...
       , CONCAT(
          ...
          LEFT JOIN lab_lists l ON l.lab_id = lr.lab_id 
					LEFT JOIN prescriptions pr on o.visit_id=pr.visit_id AND pr.is_cancel=0 AND pr.drug_id in ('1791') AND pr.RX_AMOUNT !='0'
      ...
      ORDER BY 
        ...
        WHERE  v.users = 'clopi' AND v.messagecode <> ''
		#AND v.messages <> ''
        ...
        AND l.messagecode <> '' AND l.users = 'clopi'
        ...
        AND l.messagecode <> 'success' AND l.users = 'clopi'
        ORDER BY l.d_update DESC
    ...
    public function actionData()
    {
        ...
        foreach ($vn as $r) {
            ...
            foreach ($tables as $table) {
                $query = "SELECT main_query FROM fdh_clopi WHERE main_table = '$table'";
                ...
                while ($row = fgetcsv($file, 0, "|")) {
                    ...
                    if ($visitCount > 0) {
                            ...
                            $logSQLUpdate = "UPDATE log_fdh_opd_ck SET pid = :pid, messages = :messages, messagecode = :messagecode, response = :response,
                             users = 'clopi', datereg = :datereg, d_update = NOW() WHERE visit_id = :visit_id";
                            ...
                            VALUES (:visit_id, :pid, :messages, :messagecode, :response, 'clopi',:datereg ,NOW())";
    ...
    public function actionExports()
        {
            ...
            $zipFilename = $baseDirectory . 'F16_10953_clopi_' . $currentDateTime . '.zip';
