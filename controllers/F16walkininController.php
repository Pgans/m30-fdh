...
class F16walkininController extends \yii\web\Controller
{
...
(SELECT 
       ...
       , CONCAT(
          ...
          LEFT(GROUP_CONCAT(DISTINCT TRIM(icd.ICD10_TM)), 30) AS Diag,
					...
					g.hospmain, left(hpt.hosp_name,20) as hospname,
					log.messagecode,
          ...
          LEFT JOIN lab_lists l ON l.lab_id = lr.lab_id 
					...
					INNER JOIN hospitals hpt on hpt.hosp_id=g.hospmain AND hpt.is_ubon = 1
          WHERE o.IS_CANCEL = 0
        ...
        WHERE  v.users = 'walkinin' AND v.messagecode <> ''
		#AND v.messages <> ''
        ...
        AND l.messagecode <> '' AND l.users = 'walkinin'
        ...
        AND l.messagecode <> 'success' AND l.users = 'walkinin'
        ORDER BY l.d_update DESC
 ...
 public function actionData()
{
    ...
    foreach ($vn as $r) {
        ...
        foreach ($tables as $table) {
            $query = "SELECT main_query FROM fdh_walkinin WHERE main_table = '$table'";
            $results = $db14->createCommand($query)->bindValue(':table', $table)->queryAll();
                ...
                while ($row = fgetcsv($file, 0, "|")) {
                    ...
                    if ($visitCount > 0) {
                            ...
                            $logSQLUpdate = "UPDATE log_fdh_opd_ck SET pid = :pid, messages = :messages, messagecode = :messagecode, response = :response,
                             users = 'walkinin', datereg = :datereg, d_update = NOW() WHERE visit_id = :visit_id";
                            ...
                            VALUES (:visit_id, :pid, :messages, :messagecode, :response, 'walkinin',:datereg ,NOW())";
