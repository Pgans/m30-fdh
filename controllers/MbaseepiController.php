<?php

namespace app\controllers;

use yii\helpers\Html;
use yii\data\ArrayDataProvider;
use yii\data\ActiveDataProvider;
use Yii;
use kartik\mpdf\Pdf;
//use mpdf\src\Config\ConfigVariables;
//use mpdf\src\Config\FontVariables;
use mPDF;
use yii\web\Controller;
use yii\web\Response;
use yii\filters\VerbFilter;
use app\models\LoginForm;
use app\models\ContactForm;
use app\models\UploadCSV;
use yii\web\UploadedFile;
use app\models\Vaccinetoken;
use app\models\Dt;

class MbaseepiController extends \yii\web\Controller
{
    public function actionIndex()
    {
        // $_token = $model->token;


        return $this->render('index');
    }
    public function actionDelete_all()
    {
        //return 'นายชาตรี บุญทา';

        //$selection = \Yii::$app->request->post('selection');
        $visits =  Yii::$app->request->post('chkDel');
        \Yii::$app->response->format = \yii\web\Response::FORMAT_JSON;
        return $visits;
    }

    ################ ActionHt-> ActionCheck #########################
    public function actionCheck()
    {
        $sqltoken = "SELECT MAX(token) as token30 FROM vaccine_token";

        $data = \yii::$app->db2->createCommand($sqltoken)->queryAll();
        for ($i = 0; $i < sizeof($data); $i++) {
            $token30 = $data[$i]['token30'];
        }
        ##################################################################     
        $vn =  Yii::$app->request->post('chkDel',[]);

        foreach ($vn  as $r) {
            $hn = substr($r, 10);
            //echo $hn.'<br />';
            $visit_id = substr($r, 0, 10);
            echo $visit_id;
            ############ EPI ############################
            $strDt = "SELECT @n := @n + 1 AS 'No',
        data.*
      FROM (
SELECT 
        
        o.visit_id as seq
        ,o.hn
        ,p.cid as pid
		,'1' as id_type
        ,CASE
    WHEN p.PRENAME not in('') THEN TRIM(p.PRENAME)
    WHEN TIMESTAMPDIFF(year,p.BIRTHDATE,NOW()) < '15'  AND p.sex='1' THEN 'ด.ช.'
    WHEN TIMESTAMPDIFF(year,p.BIRTHDATE,NOW()) >= '15' AND p.sex='1' THEN 'นาย'
    WHEN TIMESTAMPDIFF(year,p.BIRTHDATE,NOW()) < '15'  AND p.sex='2' THEN 'ด.ญ.'
    WHEN TIMESTAMPDIFF(year,p.BIRTHDATE,NOW()) >= '15' AND p.sex='2' AND p.MARRIAGE ='1' THEN 'น.ส.'
    ELSE 'นาง'  
    END  as title
  ,trim(p.fname) as fname
  ,trim(p.lname) as lname
  ,CASE
  WHEN p.marriage = '1' THEN '1'
  WHEN p.marriage = '2' THEN '2'
  WHEN p.marriage = '3' THEN '3'
  WHEN p.marriage = '4' THEN '6'
  ELSE '9'
  END AS marriage
,p.birthdate as dob
,p.sex
          ,oc.oc_name as nameoccptn
        ,TIMESTAMPDIFF(YEAR, p.birthdate, o.reg_datetime) AS age_year
        ,TIMESTAMPDIFF(MONTH, p.birthdate, o.reg_datetime)- TIMESTAMPDIFF(YEAR, p.birthdate, o.reg_datetime) * 12 AS age_month
		,concat('0',n.natn_id) as nation
		,h.hospcode as hcode
		,concat('โรงพยาบาล',h.hospname) as hospital_name
		,DATE_FORMAT(o.reg_datetime,'%Y-%m-%dT%T.000Z') as visit_date_time
		,'' as code_status
		,d.drug_id 
		,d.drug_name
     ,o.reg_datetime as regdate
        
        ,vc.lot_no
        
        ,cv.dose_time
		,m.inscl_name
		,uc.hospmain
		,u.unit_name
		,max(od.lot_no) as lot_number
        ,CASE
	    WHEN ak.claimcode = '' THEN 'ว่าง'
	    WHEN ak.claimcode <> '' THEN ak.claimcode
        END AS claimcode
         FROM  address_hosp h, opd_visits o 
         INNER JOIN cid_hn c ON o.HN=c.HN AND o.IS_CANCEL=0
         INNER JOIN population p ON p.CID=c.CID
         LEFT JOIN opd_diagnosis dx on dx.visit_id= o.visit_id AND dx.is_cancel=0
         LEFT  JOIN icd10new i on i.icd10=dx.icd10
         INNER  JOIN prescriptions pr ON pr.visit_id  = o.visit_id and pr.IS_CANCEL = 0
         LEFT JOIN drugs d ON d.drug_id = pr.drug_id  
         LEFT JOIN occupation_new oc ON oc.oc_id = p.oc_id
         LEFT JOIN nations n ON n.natn_id = p.natn_id
         LEFT JOIN ipd_reg ipd ON ipd.visit_id = o.visit_id AND ipd.is_cancel = 0
		 LEFT JOIN authen_kiosk ak ON ak.visit_id = o.visit_id  AND ak.cid = p.cid
         INNER JOIN cid_vaccinate_v2 cv ON cv.visit_id = o.visit_id
         INNER JOIN vaccinecovid vc ON vc.vaccine_id = cv.vaccine_id 
         LEFT JOIN vaccinate_plan vp ON vp.vac_plan_id = cv.vac_plan_id
		LEFT JOIN main_inscls m ON m.inscl = o.inscl 
		LEFT JOIN uc_inscl uc ON uc.cid = p.cid AND (uc.date_abort = date(o.REG_DATETIME) or day(uc.date_abort)=0 and trim(uc.hospmain) = '10953'  )
		LEFT JOIN order_details od ON od.ivt_id = pr.drug_id
		LEFT JOIN service_units u ON u.unit_id = o.unit_reg
        WHERE 
         o.visit_id = '$visit_id'
         AND ipd.adm_id is null
         AND TIMESTAMPDIFF(year,p.birthdate,o.reg_datetime) <= 10
		 #AND o.visit_id  not in (SELECT vs.visit_id from log_dt vs )
         AND d.drug_id in ('0500','0351','2629','2465','1810','2307','1812','1806','1809','2692')
         #AND o.visit_id <> '0003038570'
        GROUP BY o.visit_id  
) AS data,
        (SELECT @n := 0) AS init
      ORDER BY 
        No DESC ";

            $data = \yii::$app->db2->createCommand($strDt)->queryAll();
            $result = [];

            foreach ($data as $row) {
                $vaccine = [];
				$strSQL = "SELECT DISTINCT
    CASE
    WHEN od.ivt_id = '0351' THEN '041'  ##ฉีด
    WHEN od.ivt_id = '0500' THEN '010'  ##ฉีด   
    WHEN od.ivt_id = '1806' THEN '031'  ##ฉีด
    WHEN od.ivt_id = '1809' THEN '051'  ##ฉีด
    WHEN od.ivt_id = '1810' THEN '081'  ##กิน
    WHEN od.ivt_id = '1812' THEN '061'  ##ฉีด
    WHEN od.ivt_id = '0327' THEN '401'  ##ฉีด
    WHEN od.ivt_id = '2465' THEN 'D21'  ##ฉีด
		WHEN od.ivt_id = '2692' THEN 'xxx'  ##ฉีด
    WHEN od.ivt_id = '2629' 
    AND TIMESTAMPDIFF(YEAR, p.birthdate, o.reg_datetime) = '0' 
    AND (TIMESTAMPDIFF(MONTH, p.birthdate, o.reg_datetime)- TIMESTAMPDIFF(YEAR, p.birthdate, o.reg_datetime) * 12) BETWEEN 0 AND 2 
    THEN '091'  ##ฉีด
    WHEN od.ivt_id = '2629' 
    AND TIMESTAMPDIFF(YEAR, p.birthdate, o.reg_datetime) = '0' 
    AND (TIMESTAMPDIFF(MONTH, p.birthdate, o.reg_datetime)- TIMESTAMPDIFF(YEAR, p.birthdate, o.reg_datetime) * 12) BETWEEN 3 AND 4
    THEN '092'  ##ฉีด
    WHEN od.ivt_id = '2629' 
    AND TIMESTAMPDIFF(YEAR, p.birthdate, o.reg_datetime) = '0' 
    AND (TIMESTAMPDIFF(MONTH, p.birthdate, o.reg_datetime)- TIMESTAMPDIFF(YEAR, p.birthdate, o.reg_datetime) * 12) BETWEEN 5 AND 9 
    THEN '093'  ##ฉีด
    ELSE NULL
    END AS code
			,max(od.lot_no) as lot_number
			,'1' as dose_quantity
			,'' as manufacturer
			,DATE_FORMAT(max(od.exp_date),'%Y-%m-%d') as expiration_date
			,DATE_FORMAT(o.REG_DATETIME,'%Y-%m-%d %H:%i') as occurrence_time
			,'' as site_code
			,'IM' as route_code
			,'ว.20812' as license_no
			,'นพ.ประจักษ์ สีลาชาติ ' as 'name'
			,'' as note
			FROM opd_visits o 
			INNER JOIN prescriptions cv ON o.visit_id = cv.visit_id 
            LEFT JOIN cid_hn c ON c.hn = o.hn
			LEFT JOIN population p ON p.cid = c.cid
			LEFT JOIN order_details od ON od.ivt_id = cv.drug_id
			WHERE o.visit_id = '$visit_id'
			AND od.ivt_id IN ('0500','0351','2629','2465','1810','2307','1812','1806','1809','2692')
			LIMIT 1";
			

                $vaccineData = \yii::$app->db2->createCommand($strSQL)->queryAll();

                foreach ($vaccineData as $vaccineRow) {
                    $vaccineEntry = [
                        'code' => $vaccineRow['code'],
                        'lot_number' => $vaccineRow['lot_number'],
                        'dose_quantity' => $vaccineRow['dose_quantity'],
                        'manufacturer' => $vaccineRow['manufacturer'],
                        'expiration_date' => $vaccineRow['expiration_date'],
                        'occurrence_time' => $vaccineRow['occurrence_time'],
                        'site_code' => $vaccineRow['site_code'],
                        'route_code' => $vaccineRow['route_code'],
                        'license_no' => $vaccineRow['license_no'],
                        'name' => $vaccineRow['name'],
                        'note' => $vaccineRow['note'],
                    ];
                    $vaccine[] = $vaccineEntry;
                }

                $dt_report = [
                    "seq" =>  $row['seq'],
                    'hn' => $row['hn'],
                    'pid' => $row['pid'],
                    'id_type' => $row['id_type'],
                    'title' => $row['title'],
                    'fname' => $row['fname'],
                    'lname' => $row['lname'],
                    'marriage' => $row['marriage'],
                    'dob' => $row['dob'],
                    'sex' => $row['sex'],
                    'nation' => $row['nation'],
                    'hcode' => $row['hcode'],
                    'hospital_name' => $row['hospital_name'],
                    'visit_date_time' => $row['visit_date_time'],
                    'vaccine' => $vaccine,
                ];

                $result = [
                    $dt_report,
                ];
            }

            $resultText = json_encode($result, JSON_PRETTY_PRINT);
            // echo $resultText;
            $resultText = substr($resultText, 1, -1);

            // แปลง JSON กลับเป็น array
            $resultArray = json_decode($resultText, true);

            // แปลงกลับเป็น JSON ด้วยรูปแบบสวยงาม
            $resultTextPretty = json_encode($resultArray, JSON_PRETTY_PRINT);

            // พิมพ์ JSON ที่ได้
           // echo $resultTextPretty;

            ######################
            // $e = $dt_report;

            // //print_r($e);
             $resultText = json_encode($e, JSON_PRETTY_PRINT);
             $jsonData = $resultText;
             print_r($jsonData);
            ############# Send Moph-claim #############
            //$token = "eyJhbGciOiJSUzUxMiIsInR5cCI6IkpXVCJ9.-C4h-C4nuC4ouC4suC4muC4suC4peC4oeC5iOC4p-C4h-.j4ZVdl59SjKa7IwaHPl0amtNi3OerpaPofFlU7NQIZlv9379ZviGQ5ZO5kjKi4mozTzEheJisOBF_aT5bsZBLF3vCrHHa5TEIW2q0354Tuzn3J-0VA0MhUAYtmSB-N8ZKdAFBBlYv1jXGyIzKqDVTmixxERr2ZN-jGUurgU7Rnf-2TqKmexm_ia2yrB08KwGgAEuXviyo65vxwvVttz83QCy8Irgxy-JE4i8EJhYCXoYYGVW5VTznpfI4s9S-SyxHJF9Kc9kgMfl5v2qRBGRbtPX43XVCxGWxi6HijkT4lnIxZpabGzWyuSHjnzRu0q3e1THkUHsVfEOv2PslpdXH6Xz0o5RfpZBZqSSYZtNg1FFTW2WCzD9wOeteu0-yTLUI3Vr9cqW6rtsNgKjHp9542tiVE4XnX_5rEn-o7cmy__cKa8AAydSIh-D2581QoejqQHLUahnVJEQvQwGxBf77iRj8KGP-BVTkGTKmsa_7zL4k6nmwf0eQTdM-NxtnpNKEVxMMhLkL8GDqTYA0Ja2w9Z7lnSEzZ0OZn-RvYWr52Gc_rNGSr1EJtMm2R0gFMMgCMtbNF-ixY3QKNByO9bSb7oLF8bfJHhJyC724Wo-vVjvIibE-Df-1JfbOlvFIWUmRSX4aXd5npXtUeC0M6dT6n9HQLtP6Rb2eu_-s_Z8Q4Y";

            $token = $token30;
          
            # $url = "https://epidemcenter.moph.go.th/epidem/api/SendEPIDEM";
            $url = "https://claim-nhso.moph.go.th/api/v1/opd/service-admissions/epi";
            # $url = "https://epidemcenter.moph.go.th/epidem506";

           // $_token = $token30;
            $curl = curl_init($url);
            curl_setopt_array($curl, array(
                CURLOPT_URL => $url,
                CURLOPT_RETURNTRANSFER => 1,
                //SSL USE
                CURLOPT_SSL_VERIFYHOST => 0,
                CURLOPT_SSL_VERIFYPEER => 0,

                CURLOPT_CUSTOMREQUEST => "POST",
                CURLOPT_POSTFIELDS => $resultTextPretty,
                CURLOPT_HTTPHEADER => array(
                    "Content-type: application/json",
                    "Authorization: Bearer " . $token
                ),
            ));
            $response = curl_exec($curl);
            $epi = json_decode($response, true);
            $err = curl_error($curl);
            //curl_close($curl);
            // echo $response;
            curl_close($curl);
            $pid = $epi['results']['pid'];
            $message = $epi['message'];
            $message_th = $epi['message_th'];
           // echo $message_th;
            $status = $epi['status'];
           // echo $status;
            echo $response;
            ############################INSERT TABLE Log_dmht#############################

            if (strlen($response) > 0) {
                $strSQL = "REPLACE INTO log_dt (visit_id, pid,status, messagecode ,response , users,d_update) VALUES ('$visit_id','$hn',$status,'$message','$message_th','epi',NOW())";
                //  $strSQL = "REPLACE INTO log_dt (visit_id, pid, cid,status, messagecode ,response , users,d_update) VALUES ('$visit_id','$hn','$pid',$status,'$message','$message_th' ,'dt',NOW())";
                Yii::$app->db2->createCommand($strSQL)->execute();
                // Yii::$app->db2->createCommand()->insert('log_dmht', $strSQL)->execute();
            }
        }
         return $this->redirect(['epi']);
    }

    ################# ดึงข้อมูลให้ฟอร์ม DT ########################
    public function actionEpi()
    {
        $sql = " SELECT @n := @n + 1 AS 'No',
        data.*
      FROM (
SELECT 
        o.visit_id 
        ,o.hn
        ,p.cid as cid
		,p.birthdate
        , CONCAT(
              CASE 
                     WHEN p.PRENAME not in('') THEN TRIM(p.PRENAME)
                           WHEN TIMESTAMPDIFF(year,p.BIRTHDATE,NOW())< '20' AND p.sex='1' AND p.MARRIAGE = '4'THEN 'สามเณร'
                           WHEN TIMESTAMPDIFF(year,p.BIRTHDATE,NOW()) >= '20' AND p.sex='1' AND p.MARRIAGE  = '4'THEN 'พระภิกษุ'
                           WHEN TIMESTAMPDIFF(year,p.BIRTHDATE,NOW()) < '15'  AND p.sex='1' THEN 'เด็กชาย'
                           WHEN TIMESTAMPDIFF(year,p.BIRTHDATE,NOW()) >= '15' AND p.sex='1' THEN 'นาย'
                           WHEN TIMESTAMPDIFF(year,p.BIRTHDATE,NOW()) < '15'  AND p.sex='2' THEN 'เด็กหญิง'
                           WHEN TIMESTAMPDIFF(year,p.BIRTHDATE,NOW()) >= '15' AND p.sex='2' AND p.MARRIAGE ='1' THEN 'นางสาว'
                           ELSE 'นาง' 
                    END ,TRIM(p.FNAME),'  ',TRIM(p.LNAME)) as 'fullname'
          ,oc.oc_name as nameoccptn
          ,CASE
          WHEN p.marriage = '1' THEN '1'
          WHEN p.marriage = '2' THEN '2'
          WHEN p.marriage = '3' THEN '3'
          WHEN p.marriage = '4' THEN '6'
          ELSE '9'
          END AS marriage
        ,p.birthdate as dob
        ,p.sex
        ,TIMESTAMPDIFF(YEAR, p.birthdate, o.reg_datetime) AS age_year
        ,TIMESTAMPDIFF(MONTH, p.birthdate, o.reg_datetime)- TIMESTAMPDIFF(YEAR, p.birthdate, o.reg_datetime) * 12 AS age_month
        ,concat('0',n.natn_id) as nation
        ,o.reg_datetime as regdate
        ,'' as code_status
        ,vc.lot_no
        ,d.drug_id 
        ,d.drug_name
        ,cv.dose_time
		,m.inscl_name
		,uc.hospmain
		,u.unit_name
		,log.status
		,SUBSTRING(log.response, 1, 100) AS response
		,max(od.lot_no) as lot_number
        ,CASE
	    WHEN ak.claimcode = '' THEN 'ว่าง'
	    WHEN ak.claimcode <> '' THEN ak.claimcode
        END AS claimcode
         FROM  opd_visits o 
         INNER JOIN cid_hn c ON o.HN=c.HN AND o.IS_CANCEL=0
         INNER JOIN population p ON p.CID=c.CID
         LEFT JOIN opd_diagnosis dx on dx.visit_id= o.visit_id AND dx.is_cancel=0
         LEFT  JOIN icd10new i on i.icd10=dx.icd10
         INNER  JOIN prescriptions pr ON pr.visit_id  = o.visit_id and pr.IS_CANCEL = 0
         LEFT JOIN drugs d ON d.drug_id = pr.drug_id  
         LEFT JOIN occupation_new oc ON oc.oc_id = p.oc_id
         LEFT JOIN nations n ON n.natn_id = p.natn_id
         LEFT JOIN ipd_reg ipd ON ipd.visit_id = o.visit_id AND ipd.is_cancel = 0
		 LEFT JOIN authen_kiosk ak ON ak.visit_id = o.visit_id  AND ak.cid = p.cid
         INNER JOIN cid_vaccinate_v2 cv ON cv.visit_id = o.visit_id
         INNER JOIN vaccinecovid vc ON vc.vaccine_id = cv.vaccine_id 
         LEFT JOIN vaccinate_plan vp ON vp.vac_plan_id = cv.vac_plan_id
		LEFT JOIN main_inscls m ON m.inscl = o.inscl 
		LEFT JOIN uc_inscl uc ON uc.cid = p.cid AND (uc.date_abort = date(o.REG_DATETIME) or day(uc.date_abort)=0 and trim(uc.hospmain) = '10953'  )
		LEFT JOIN order_details od ON od.ivt_id = pr.drug_id
		LEFT JOIN service_units u ON u.unit_id = o.unit_reg
		LEFT JOIN log_dt log ON log.visit_id = o.visit_id
        WHERE 
         o.reg_datetime BETWEEN SUBDATE(CURDATE() ,INTERVAL 90 DAY) AND NOW()
         AND ipd.adm_id is null
         AND TIMESTAMPDIFF(year,p.birthdate,o.reg_datetime) <= 10
		 #AND o.visit_id  not in (SELECT vs.visit_id from log_dt vs )
         AND d.drug_id in ('0500','0351','2629','2465','1810','2307','1812','1806','1809','2692')
        GROUP BY o.visit_id  
) AS data,
        (SELECT @n := 0) AS init
      ORDER BY 
        No DESC 
		#limit 10
        ";
        $rawData = \yii::$app->db2->createCommand($sql)->queryAll();
        try {
            $rawData = \Yii::$app->db2->createCommand($sql)->queryAll();
        } catch (\yii\db\Exception $e) {
            throw new \yii\web\ConflictHttpException('sql error');
        }
        $epiProvider = new \yii\data\ArrayDataProvider([
            'allModels' => $rawData,
            'pagination' => [
                'pageSize' =>40,
            ],
        ]);
        $sqlCount1 = "SELECT COUNT(DISTINCT v.visit_id) as amount
            FROM log_dt v 
            WHERE v.status = '200'
            AND v.d_update BETWEEN CURDATE() AND NOW()
            AND v.users = 'epi'
            ";

        $data = \yii::$app->db2->createCommand($sqlCount1)->queryAll();
        for ($i = 0; $i < sizeof($data); $i++) {
            $amount = $data[$i]['amount'];
        }
        $sqlCamount = "SELECT COUNT(DISTINCT v.visit_id) as amountx
            FROM log_dt v 
            WHERE v.status <> '200'
            AND v.d_update BETWEEN CURDATE() AND NOW()
            AND v.users = 'epi'
             ";
        $data = \yii::$app->db2->createCommand($sqlCamount)->queryAll();
        for ($i = 0; $i < sizeof($data); $i++) {
            $amountx = $data[$i]['amountx'];
        }
        $total = "SELECT COUNT(DISTINCT v.visit_id) as total
            FROM log_dt v 
            WHERE v.messagecode = 'success'
            AND v.users <> 'epi' 
            AND v.d_update BETWEEN '2023-10-01' AND NOW()
            ";

        $data = \yii::$app->db2->createCommand($total)->queryAll();
        for ($i = 0; $i < sizeof($data); $i++) {
            $total = $data[$i]['total'];
        }
         ########################################################################################################
         $sqlPass = "select l.id, l.visit_id, l.pid, l.`status`, l.messagecode, l.response, l.users, l.d_update
         FROM log_dt l 
         WHERE l.d_update BETWEEN CURDATE() AND NOW() AND l.messagecode = 'success'
         ORDER BY l.d_update DESC
             ";
             $rawData = \Yii::$app->db143->createCommand($sqlPass)->queryAll();
 
             // สร้าง Flash Alert
             //Yii::$app->session->setFlash('success', 'รายการที่ไม่ผ่านตามเงื่อนไข');
             
             $passProvider = new \yii\data\ArrayDataProvider([
                 'allModels' => $rawData,
                 'pagination' => [
                     'pageSize' => 5,
                 ],
             ]);
         ########################################################################################################
         $sqlError = "select l.id, l.visit_id, l.pid, l.`status`, l.messagecode, l.response, l.users, l.d_update
         FROM log_dt l 
         WHERE l.d_update BETWEEN CURDATE() AND NOW() AND l.messagecode <> 'success'
         ORDER BY l.d_update DESC
             ";
             $rawData = \Yii::$app->db143->createCommand($sqlError)->queryAll();
 
             // สร้าง Flash Alert
             //Yii::$app->session->setFlash('success', 'รายการที่ไม่ผ่านตามเงื่อนไข');
             
             $errorProvider = new \yii\data\ArrayDataProvider([
                 'allModels' => $rawData,
                 'pagination' => [
                     'pageSize' => 15,
                 ],
             ]);

        return $this->render('epi', [
            // 'searchModel' => $searchModel,
            'epiProvider' => $epiProvider,
            'passProvider' => $passProvider,
            'errorProvider' => $errorProvider,
            'amount' => $amount,
            'amountx' => $amountx,
            'total' => $total,

        ]);
    }
    ######################################################################################################
    public function actionDeleteError($visit_id)
{
    try {
        // แสดงค่า visit_id ด้วย JavaScript alert
       // echo "<script>alert('Visit ID: $visit_id');</script>";
        
        // ดำเนินการลบข้อมูล
        $rowsAffected = Yii::$app->db14->createCommand()->delete('log_dt', ['visit_id' => $visit_id])->execute();

        Yii::$app->session->setFlash('success', "ลบข้อมูลสำเร็จ");
    } catch (\Exception $e) {
        Yii::$app->session->setFlash('error', 'เกิดข้อผิดพลาดในการลบ: ' . $e->getMessage());
    }

    return $this->redirect(['epi']);
}





    ################################
    public function actionLog_dt()
    {
        $sql = "select l.id, l.visit_id, l.pid, l.`status`, l.messagecode, l.response, l.users, l.d_update
            FROM log_dt l WHERE l.d_update >= CURDATE()-5
            #AND l.users = 'epi'
            ORDER BY l.d_update DESC
            ";
        $rawData = \yii::$app->db14->createCommand($sql)->queryAll();
        try {
            $rawData = \Yii::$app->db14->createCommand($sql)->queryAll();
        } catch (\yii\db\Exception $e) {
            throw new \yii\web\ConflictHttpException('sql error');
        }

        $logdtProvider = new \yii\data\ArrayDataProvider([
            'allModels' => $rawData,
            'pagination' => [
                'pageSize' => 15,
            ],
        ]);

        return $this->render('log_dt', [
            // 'searchModel' => $searchModel,
            'logdtProvider' => $logdtProvider,

        ]);
    }
    public function actionDeleteLog()
{
    try {
        $command = Yii::$app->db14->createCommand("DELETE FROM log_dt  WHERE (status <> '200' OR messagecode <> 'OK') AND status <> '400'");
        $rowsAffected = $command->execute();

        Yii::$app->session->setFlash('success', "[ลบข้อมูลสำเร็จ ] Successfully. จำนวน: $rowsAffected");
    } catch (\Exception $e) {
        Yii::$app->session->setFlash('error', 'Error deleting: ' . $e->getMessage());
    }

    return $this->redirect(['epi']); // Replace 'index' with the actual route you want to redirect to.
}
}

