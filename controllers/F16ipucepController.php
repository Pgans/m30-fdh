...
class F16ipucepController extends \yii\web\Controller
{
    ...
    public function actionIndex()
    {
      ...
      ORDER BY 
        ...
        WHERE  v.users = 'ipucep' and v.messagecode <> '' 
        ...
        AND l.messagecode <> ' ' AND l.users = 'ipucep'
            ...
            if ($data && isset($data['token30'])) {
                ...
                $statusMessageTh = isset($responseDecoded['data'][0]['status_message_th']) ? $responseDecoded['data'][0]['status_message_th'] : '';
                 ...
                 if ($visitCount > 0) {
                     ...
                     $logSQLUpdate = "UPDATE log_fdh_ipd_ck SET pid = :pid, messages = :messages, messagecode = :messagecode, response = :response, users = 'ipucep', d_update = NOW() WHERE visit_id = :visit_id AND an = :an";
                     ...
                     $logSQLInsert = "INSERT INTO log_fdh_ipd_ck (visit_id, pid, an, messages, messagecode, response, users, d_update) VALUES (:visit_id, :pid, :an, :messages, :messagecode, :response, 'ipucep', NOW())";
...
#########################################################################################################
	 ...
	 public function actionExports()
    {
        ...
        $zipFilename = $baseDirectory . 'F16_10953_Ipucep_' . $currentDateTime . '.zip';
