...
public function actionIndex()
{
  ...
  (SELECT
   ...
   , CONCAT(
	  ...
	  LEFT JOIN close_visits  cv ON cv.visit_id = o.visit_id  AND cv.is_cancel = 0
	...
	WHERE  v.users = 'ancdentuslab1upt' AND v.messagecode <> ''
	...
	AND l.messagecode <> '' AND l.users = 'ancdentuslab1upt'
   ...
   AND l.messagecode <> 'success' AND l.users = 'ancdentuslab1upt'
   ORDER BY l.d_update DESC
...
foreach ($vn as $r) {
	...
	foreach ($tables as $table) {
		...
		// ตรวจสอบว่าตารางไม่มีข้อมูล
		if (empty($fileData[$table])) {
			$emptyTables[] = $table;
		}
	}

	// ถ้ามีแฟ้มว่าง ให้แจ้งเตือนแล้วไม่ต้องทำงานต่อ
...
public function actionExports()
{
	...
	$zipFilename = $baseDirectory . 'F16_10953_Ancusdentlab1upt_' . $currentDateTime . '.zip';
