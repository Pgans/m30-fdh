...
class Closevisit2Controller extends \yii\web\Controller
{
    ...
    public function actionIndex()
    {
        ...
        $sqlCount1 = "SELECT COUNT(DISTINCT v.visit_id) as amount
            FROM close_visits v 
            WHERE 
             v.claim_datetime BETWEEN CURDATE() AND NOW()";
        ...
        $totalQuery = "SELECT COUNT(DISTINCT v.visit_id) as amount
            FROM close_visits v 
            WHERE 
             v.claim_datetime BETWEEN '2024-10-01 00:01' AND '2025-09-30 23:59'";

		$data = \yii::$app->db14->createCommand($totalQuery)->queryOne(); // ใช้ queryOne เพราะผลลัพธ์จะมีแค่แถวเดียว

		$total = $data['amount']; // เข้าถึงค่า 'amount' จากผลลัพธ์
...
###################  ตรวจสอบและบันทึกเฉพาะรายการที่ตรงกับวันนี้เท่านั้น เพื่อจะได้ Visit_id ที่ตรงเนื่องจากใช้การ LOOP ##########################################
public function actionCheck()
{
    $cids = Yii::$app->request->post('chkDel', []);

    // ตัวแปรนับจำนวนรายการที่นำเข้าได้
    $importedCount = 0;
        ...
        foreach ($cids as $r) {
            ...
            foreach ($claimsByDate as $date => $claims) {
                if ($date === $today && !empty($claims['EP'])) {
                    // ถ้ามี EP และเป็นวันนี้ ให้เลือก EP
    ...
    // แจ้งเตือนจำนวนที่นำเข้า
    echo "<script type='text/javascript'>
            alert('นำเข้าข้อมูลได้ทั้งหมด " . $importedCount . " รายการ');
          </script>";
...
###################  กรณีดึงข้อมูลมา มี EP หลายค่า หลายวัน บันทึกได้แต่ Visit_id จะไม่ตรง ##########################################################
/*
public function actionCheck()
{
    $cids = Yii::$app->request->post('chkDel', []);
