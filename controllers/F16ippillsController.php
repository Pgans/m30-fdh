...
class F16ippillsController extends \yii\web\Controller
{
    public function actionIndex()
    {
      ...
      ORDER BY 
        ...
        AND l.messagecode <> ' ' AND l.users = 'ippills'
        ...
        FROM log_fdh_ipd_ck l 
        WHERE l.d_update BETWEEN DATE_SUB(NOW(), INTERVAL 1 DAY) AND NOW()
        AND l.messagecode <> 'success' AND l.users = 'ippills'
        ORDER BY l.d_update DESC
     ...
     public function actionData()
    {
        ...
        foreach ($vn as $r) {
           ...
           // echo $an;
            ...
            $tables = ['adp', 'ins', 'pat', 'ipd', 'iop', 'idx', 'dru', 'cha', 'cht' ];
            foreach ($tables as $table) {
                $query = "SELECT main_query FROM fdh_ippills WHERE main_table = '$table'";
            ...
            if ($data && isset($data['token30'])) {
                ...
                $statusMessageTh = isset($responseDecoded['data'][0]['status_message_th']) ? $responseDecoded['data'][0]['status_message_th'] : '';
                 ...
                 if ($visitCount > 0) {
                     ...
                     $logSQLUpdate = "UPDATE log_fdh_ipd_ck SET pid = :pid, messages = :messages, messagecode = :messagecode, response = :response, users = 'ippills', d_update = NOW() WHERE visit_id = :visit_id AND an = :an";
                     ...
                     $logSQLInsert = "INSERT INTO log_fdh_ipd_ck (visit_id, pid, an, messages, messagecode, response, users, d_update) VALUES (:visit_id, :pid, :an, :messages, :messagecode, :response, 'ippills', NOW())";
