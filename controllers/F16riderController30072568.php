    ...
    public function actionData()
    {
        ...
        foreach ($vn as $r) {
            ...
            foreach ($tables as $table) {
                $query = "SELECT main_query FROM fdh_rider WHERE main_table = '$table'";
                $results = $db14->createCommand($query)->queryAll();

                #### Copy ตรวจสอบแฟ้ม PAT คัด Person_id ที่ซ้ำกันออก #######๒๒๒๒๒๒๒๒๒๒๒๒๒๒๒๒๒๒๒๒๒๒๒๒
                foreach ($results as $result) {
                    ...
                    if ($table === 'pat') {
                    // Remove duplicate person_id entries for 'pat' table
                    $uniqueData = [];
                    foreach ($data as $row) {
                        $personId = $row['person_id']; // Adjust this if the field name is different
            ...
            while ($row = fgetcsv($file, 0, "|")) {
                ...
                if ($visitCount > 0) {
                    ...
                    $logSQLUpdate = "UPDATE log_fdh_opd_ck SET pid = :pid, messages = :messages, messagecode = :messagecode, response = :response,
                     users = 'rider', datereg = :datereg, d_update = NOW() WHERE visit_id = :visit_id";
                    ...
                    VALUES (:visit_id, :pid, :messages, :messagecode, :response, 'rider',:datereg ,NOW())";
