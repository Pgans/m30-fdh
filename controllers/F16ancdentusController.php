...
class F16ancdentusController extends \yii\web\Controller
{
public function actionIndex()
{
    // 1. รับค่าวันที่และฟิลเตอร์ผ่าน POST หรือ GET
    ...
    // แยกตัวแปร SQL โดยใส่เวลา 00:00:00 ถึง 23:59:59 เพื่อใช้ใน BETWEEN ป้องกันข้อมูลตกหล่น
    $date1_sql = $date1x . ' 00:00:00';
    $date2_sql = $date2x . ' 23:59:59';

    // 2. Query หลัก ($sqlData) - แก้ไข Ambiguous Column 'icd10' เรียบร้อยแล้ว
    $sqlData = "
        SELECT 
    ...
    // ดึงข้อมูลผ่าน db7 ตามของเดิม
    $rawData = Yii::$app->db7->createCommand($sqlData, [
        ':date1' => $date1_sql,
        ':date2' => $date2_sql
    ])->queryAll();
...
$vn = Yii::$app->request->post('selection', []);
$visits = [];
...
foreach ($vn as $r) {
    ...
    if ($table === 'pat') {
        ...
        $results = $db->createCommand($queryIns)
            ->bindValue(':table', $table)
            ->queryAll();

        // ถ้าไม่มีข้อมูลใน fdh_ins ให้ใช้จาก fdh_ancdent_us
        if (empty($results)) {
            $queryHerb = "SELECT main_query FROM fdh_ancdent_us WHERE main_table = :table";
            $results = $db->createCommand($queryHerb)
                ->bindValue(':table', $table)
                ->queryAll();
        }
    }
#############################################################################
		...
		foreach ($results as $result) {
			...
			$data = $db->createCommand($mainQueryResult)->queryAll();
...
return $this->redirect(['index']); // กลับไปยัง action ที่ต้องการ
}
##################################################################################################################
		...
		if (file_exists($chtFilePath)) {
			...
			while ($row = fgetcsv($file, 0, "|")) {
				...
				if ($visitCount > 0) {
				 ...
				 users = 'ancdentus', datereg = :datereg, d_update = NOW() WHERE visit_id = :visit_id";
				...
				VALUES (:visit_id, :pid, :messages, :messagecode, :response, 'ancdentus',:datereg ,NOW())";
...
public function actionExports()
{
	...
	$zipFilename = $baseDirectory . 'F16_10953_AncDentUs_' . $currentDateTime . '.zip';
