...
class F16opaeController extends \yii\web\Controller
{
...
(SELECT 
       ...
       , CONCAT(
          ...
          LEFT JOIN lab_lists l ON l.lab_id = lr.lab_id 
					...
					LEFT JOIN authen_kiosk ak ON ak.visit_id = o.visit_id and p.cid = ak.cid
        ...
        WHERE  v.users = 'opae' AND v.messagecode <> ''
		#AND v.messages <> ''
        ...
        AND l.messagecode <> '' AND l.users = 'opae'
        ...
        AND l.messagecode <> 'success' AND l.users = 'opae'
        ORDER BY l.d_update DESC
...
public function actionData()
{
    ...
    $tables = ['adp', 'ins', 'odx', 'opd', 'pat', 'cha', 'cht', 'aer', 'oop', 'dru', 'orf'];
    ...
    foreach ($vn as $r) {
        ...
        foreach ($tables as $table) {
            $query = "SELECT main_query FROM fdh_opae WHERE main_table = :table";
            $results = $db2->createCommand($query)->bindValue(':table', $table)->queryAll();
                ...
                while ($row = fgetcsv($file, 0, "|")) {
                    ...
                    if ($visitCount > 0) {
                            ...
                            $logSQLUpdate = "UPDATE log_fdh_opd_ck SET pid = :pid, messages = :messages, messagecode = :messagecode, response = :response,
                             users = 'opae', datereg = :datereg, d_update = NOW() WHERE visit_id = :visit_id";
                            ...
                            VALUES (:visit_id, :pid, :messages, :messagecode, :response, 'opae',:datereg ,NOW())";
    ...
    public function actionExports()
        {
            ...
            $zipFilename = $baseDirectory . 'F16_10953_opae_' . $currentDateTime . '.zip';
