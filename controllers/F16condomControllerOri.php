...
class F16condomController extends \yii\web\Controller
{
...
(SELECT 
       ...
       , CONCAT(
        ...
        WHERE  v.users = 'condom' AND v.messagecode <> ''
		#AND v.messages <> ''
        ...
        AND l.messagecode <> '' AND l.users = 'condom'
        ...
        AND l.messagecode <> 'success' AND l.users = 'condom'
        ORDER BY l.d_update DESC
   ...
   public function actionData1()
    {
        ...
        foreach ($vn as $r) {
            ...
            foreach ($tables as $table) {
                 $query = "SELECT main_query FROM fdh_condom WHERE main_table = '$table'";
      ...
      # Yii::$app->session->setFlash('warning', 'กรุณาตรวจสอบไฟล์ข้อมูลก่อนส่งไปยัง API');
            ...
            while ($row = fgetcsv($file, 0, "|")) {
                ...
                if ($visitCount > 0) {
                        ...
                        $logSQLUpdate = "UPDATE log_fdh_opd_ck SET pid = :pid, messages = :messages, messagecode = :messagecode, response = :response,
                         users = 'condom', datereg = :datereg, d_update = NOW() WHERE visit_id = :visit_id";
                        ...
                        VALUES (:visit_id, :pid, :messages, :messagecode, :response, 'condom',:datereg ,NOW())";
