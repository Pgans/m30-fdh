...
class F16erextController extends \yii\web\Controller
{
    public function actionIndex()
    {
       ...
       , CONCAT(
          ...
          WHERE o.IS_CANCEL = 0
				...
				AND (o.UNIT_REG in ('11') or o.UNIT_ID in ('11'))
				AND o.unit_reg not in (03,04,05)
				AND (TIME(o.REG_DATETIME) BETWEEN '16:30' AND '23:59' OR TIME(o.REG_DATETIME) BETWEEN '00:00' AND '08:30' or DAYOFWEEK(o.reg_datetime) in(1,7)or date(o.reg_datetime) in (SELECT holidays.h_date FROM holidays))
        ...
        WHERE v.messagecode = 'success' AND v.users = 'erext'
        ...
        AND l.messagecode = 'success' AND l.users = 'erext'
    ...
    public function actionData()
    {
        ...
        foreach ($vn as $r) {
            ...
            foreach ($tables as $table) {
                $query = "SELECT main_query FROM fdh_erext WHERE main_table = '$table'";
    ...
    private function sendDataToAPI($visit, $hn)
    {
        ...
        if ($data && isset($data['token30'])) {
            ...
            VALUES ('$visit', '$hn', '$message', '$message_th', 'erext', NOW())";
