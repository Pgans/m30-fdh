...
class F16denturesController extends \yii\web\Controller
{
...
(SELECT 
       ...
       , CONCAT(
          ...
          LEFT JOIN service_units e ON o.UNIT_REG=e.unit_id
					LEFT JOIN opd_operations o1 ON o.VISIT_ID=o1.VISIT_ID AND o1.IS_CANCEL = 0 
					LEFT JOIN icd9cm oo ON o1.icd9=oo.icd9
	     ...
	     LEFT JOIN receipts re ON re.visit_id = o.visit_id AND re.is_cancel = 0
          ...
          #AND p.NATN_ID = 99
		      AND oo.code = '99.97'
         # AND o.visit_id in (0003164748,0003167211,0003167270,0003168477,0003167413,0003168512,0003171316,0003171761)
        ...
        WHERE  v.users = 'dentures' AND v.messagecode <> ''
		#AND v.messages <> ''
        ...
        AND l.messagecode <> '' AND l.users = 'dentures'
        ...
        AND l.messagecode <> 'success' AND l.users = 'dentures'
        ORDER BY l.d_update DESC
    ...
    public function actionData()
    {
        ...
        foreach ($vn as $r) {
            ...
            foreach ($tables as $table) {
                $query = "SELECT main_query FROM fdh_dentures WHERE main_table = '$table'";
                ...
                while ($row = fgetcsv($file, 0, "|")) {
                    ...
                    if ($visitCount > 0) {
                            ...
                            $logSQLUpdate = "UPDATE log_fdh_opd_ck SET pid = :pid, messages = :messages, messagecode = :messagecode, response = :response,
                             users = 'dentures', datereg = :datereg, d_update = NOW() WHERE visit_id = :visit_id";
                            ...
                            VALUES (:visit_id, :pid, :messages, :messagecode, :response, 'dentures',:datereg ,NOW())";
