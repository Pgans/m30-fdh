...
class F16opucepController extends \yii\web\Controller
{
...
(SELECT @n :=@n +1 'No'
       ...
       , CONCAT(
          ...
          LEFT(GROUP_CONCAT(DISTINCT TRIM(icd.ICD10_TM)), 30) AS Diag,
					GROUP_CONCAT(DISTINCT ic.code) as oper,
          ...
          LEFT JOIN lab_lists l ON l.lab_id = lr.lab_id 
					INNER JOIN prescriptions ps on ps.visit_id = o.visit_id AND ps.is_cancel = 0
					LEFT JOIN opd_operations op ON op.visit_id = o.visit_id AND op.is_cancel = 0
					LEFT JOIN icd9cm ic ON op.icd9 = ic.icd9
          ...
          WHERE o.IS_CANCEL = 0
				...
				AND o.PT_STATES in (1,2)
				AND g.hospmain = '10953' 
			AND o.visit_id  not in (SELECT vs.visit_id from log_fdh_opd_ck vs )
			GROUP BY o.VISIT_ID
		) AS data,
        (SELECT @n := 0) AS init
      ORDER BY 
        ...
        WHERE v.messagecode = 'success' AND v.users = 'opucep'
        ...
        AND l.messagecode = 'success' AND l.users = 'opucep'
    ...
    public function actionData()
    {
        ...
        foreach ($vn as $r) {
            ...
            $tables = ['adp', 'ins', 'odx', 'opd', 'pat', 'cha', 'cht','aer','oop','dru'];
            foreach ($tables as $table) {
                $query = "SELECT main_query FROM fdh_opucep WHERE main_table = '$table'";
                ...
                foreach ($results as $result) {
                    ...
                    // ตรวจสอบว่า $table ไม่ใช่ค่าว่างก่อนใช้กับ $baseDirectory
                    if (!empty($table)) {
    ...
    private function sendDataToAPI($visit, $hn)
    {
        ...
        if ($data && isset($data['token30'])) {
            ...
            $logSQL = "REPLACE INTO log_fdh_opd_ck(visit_id, pid, messagecode, response, users, d_update) 
            VALUES ('$visit', '$hn', '$message', '$message_th', 'opucep', NOW())";
