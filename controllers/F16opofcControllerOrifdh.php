...
public function actionData()
{
    ...
    foreach ($vn as $r) {
        ...
        foreach ($tables as $table) {
            $query = "SELECT main_query FROM fdh_opofc WHERE main_table = :table";
        ...
        // ตรวจสอบและแจ้งเตือนถ้ามีแฟ้มที่ไม่มีข้อมูล
if (!empty($emptyRecords)) {
    $messages = [];
    foreach ($emptyRecords as $hn => $tables) {
        $messages[] = "HN: $hn ไม่มีข้อมูลในตาราง: " . implode(', ', $tables);
    }
    Yii::$app->session->setFlash('error', "❌ พบแฟ้มไม่มีข้อมูล:\n" . implode("\n", $messages));
    //return $this->redirect(['index']);
}
                ...
                while ($row = fgetcsv($file, 0, "|")) {
                    ...
                    if ($visitCount > 0) {
                            ...
                            $logSQLUpdate = "UPDATE log_fdh_opd_ck SET pid = :pid, messages = :messages, messagecode = :messagecode, response = :response,
                             users = 'opofc', datereg = :datereg, d_update = NOW() WHERE visit_id = :visit_id";
                            ...
                            VALUES (:visit_id, :pid, :messages, :messagecode, :response, 'opofc',:datereg ,NOW())";
