...
FROM (
    ...
    FROM (
        SELECT 
            ...
            CONCAT(
                CASE 
                    ...
                    WHEN TIMESTAMPDIFF(YEAR,c.BIRTHDATE,NOW()) < 20 
                         AND c.sex='1' AND c.MARRIAGE='4' THEN 'สามเณร'
                    WHEN TIMESTAMPDIFF(YEAR,c.BIRTHDATE,NOW()) >= 20 
                         AND c.sex='1' AND c.MARRIAGE='4' THEN 'พระภิกษุ'
                    ...
                    WHEN TIMESTAMPDIFF(YEAR,c.BIRTHDATE,NOW()) >= 15 
                         AND c.sex='2' AND c.MARRIAGE='1' THEN 'นางสาว'
                    ELSE 'นาง'
                END,
        ...
        FROM opd_visits o 
        LEFT JOIN cost_visits cv ON o.VISIT_ID=cv.visit_id
        LEFT JOIN cid_hn b ON o.HN=b.HN
        LEFT JOIN population c ON b.CID=c.CID

        INNER JOIN prescriptions p 
            ON p.visit_id=o.visit_id 
           AND p.DRUG_ID NOT IN ('0371','0372','0369')

        INNER JOIN visit_invoice v 
            ON v.visit_id=o.visit_id 

        INNER JOIN drugs d 
            ON d.drug_id=p.DRUG_ID 
           AND d.drug_id=v.drug_id

        INNER JOIN opd_diagnosis od 
            ON od.visit_id=o.visit_id 
           AND od.is_cancel=0 
           AND od.dxt_id=1

        LEFT JOIN icd10new icd 
            ON icd.icd10=od.icd10 
           AND icd.icd10<>''
        ...
        /* คัดเฉพาะที่มีอยู่ใน hosp3400 */
        LEFT  JOIN hospitals hp on hp.hosp_id=g.hospmain #AND hp.is_ubon = 1
	...
	public function actionCheckData()
{
    ...
    try {
        ...
        // ✅ adp บังคับแล้ว เหลือแค่ dru ไม่บังคับ
        $notRequired = ['dru'];
        ...
        foreach ($tables as $table) {
            ...
            try {
                $results = $db2->createCommand(
                    "SELECT main_query FROM fdh_herbnew WHERE main_table = :t"
...
############################### เริ่มการ Loop ส่งทีละราย Visit ##################################################################################
  ...
  public function actionData()
{
    ...
    foreach ($vn as $r) {
        ...
        $tables = ['adp', 'ins', 'odx', 'opd', 'pat', 'cha', 'cht', 'dru'];
        $fileData = [];
        $emptyTables = []; // เก็บชื่อแฟ้มที่ไม่มีข้อมูล (ยกเว้น adp)
        ...
        foreach ($tables as $table) {
            ...
            // ✅ ยกเว้นการตรวจค่าว่างสำหรับ adp และ dru
				if (!in_array($table, ['adp', 'dru']) && empty($fileData[$table])) {
				$emptyTables[] = $table;
			}
        }

        // หากมีแฟ้มที่ว่าง (ยกเว้น adp) ให้แจ้งเตือน
            ...
            while ($row = fgetcsv($file, 0, "|")) {
                ...
                if ($visitCount > 0) {
                        ...
                        $logSQLUpdate = "UPDATE log_fdh_opd_ck SET pid = :pid, messages = :messages, messagecode = :messagecode, response = :response,
                         users = 'herbnew', datereg = :datereg, d_update = NOW() WHERE visit_id = :visit_id";
                        ...
                        VALUES (:visit_id, :pid, :messages, :messagecode, :response, 'herbnew',:datereg ,NOW())";
