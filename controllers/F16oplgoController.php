...
public function actionData()
{
    ...
    try {
        ...
        // 2. แยก Sub-folder ชั่วคราวตามเลข $visit ป้องกันไฟล์เขียนทับกัน
        ...
        // [⚡ จุดจูนความเร็วสูงสุด] ยุบ 2 ลูปใหญ่เหลือลูปเดียว: ดึงตารางไหนเสร็จ -> เขียนไฟล์นั้นทันที -> เคลียร์แรมทิ้งทันที
        foreach ($tables as $table) {
            
            // ดึงโครงสร้าง Query สำหรับตารางปัจจุบัน
            $results = $db2->createCommand("SELECT main_query FROM fdh_oplgo WHERE main_table = :table")
                ->bindValue(':table', $table)
                ->queryAll();

            // ตัวแปรเก็บข้อมูลชั่วคราวในระดับตาราง (ไม่เอาไปปนกับตารางอื่น)
            $currentTableData = [];
            ...
            foreach ($results as $result) {
                ...
                // วนลูปย้ายข้อมูลเข้าอาเรย์ย่อย
                foreach ($data as $row) {
                    $currentTableData[] = $row;
                }
                
                unset($data); // เคลียร์ผลลัพธ์ย่อยออกจากแรมทันที
            }
            unset($results); // เคลียร์โครงสร้าง Query ของตารางนี้ออก

            // สั่งเขียนไฟล์ .txt ของตารางปัจจุบันทันที ไม่ต้องรอให้ครบ 11 ตาราง
            $filePath = $fullPath . strtoupper($table) . '.txt';
            $header = $this->getHeaderForTable($table);
            
            $this->exportToTextFile($currentTableData, $filePath, $header);

            // [🔥 จุดประหยัดแรมสูงสุด] เขียนไฟล์เสร็จ 1 แฟ้ม ล้างข้อมูลในแรมของแฟ้มนั้นทิ้งทันที!
            unset($currentTableData);
        }
...
private function sendToNhsoApi($payload, $visits = [], $hn = null)
{
    ...
    foreach ($visits as $visit) {
        ...
        if ($visitCount > 0) {
            // อัปเดต
            $logSQLUpdate = "UPDATE log_fdh_opd_ck SET pid = :pid, messages = :messages, messagecode = :messagecode, response = :response, users = 'oplgo', d_update = NOW() WHERE visit_id = :visit_id";
            ...
            $logSQLInsert = "INSERT INTO log_fdh_opd_ck (visit_id, pid, messages, messagecode, response, users, d_update)
                             VALUES (:visit_id, :pid, :messages, :messagecode, :response, 'oplgo', NOW())";
...
public function actionExports()
    {
        ...
        $zipFilename = $baseDirectory . 'F16_10953_OP_LGO_' . $currentDateTime . '.zip';
        ...
        $folderInZip = 'nhso_opd/';
        
        // Add files to the 'nhso_opd' folder in the zip file
...
########################################################################################################
public function actionListFilesPartial($visit)
{
    // ใช้ Double Quotes ("") เพื่อให้ระบบแปลงตัวแปร $visit เป็นค่าจริง
    $dirPath = Yii::getAlias("@webroot/uploads/nhso_opd/$visit");
    
    // ตรวจสอบก่อนว่ามีโฟลเดอร์นี้อยู่จริงไหม ป้องกัน Error
    if (is_dir($dirPath)) {
        $files = scandir($dirPath);
        // กรองเอา . และ .. ออกไปด้วยเพื่อให้เหลือแต่ชื่อไฟล์จริง ๆ
        $files = array_diff($files, ['.', '..']);
    } else {
        $files = [];
    }

    // ส่งค่า $visit ไปที่ View ด้วย เพื่อเอาไว้ใช้สร้างลิงก์สำหรับเปิดอ่านไฟล์
    return $this->renderPartial('listFiles', [
        'files' => $files,
        'visit' => $visit
    ]);
}

public function actionReadFile($visit, $fileName)
{
    // เพิ่ม $visit เข้ามาใน Path เพื่อให้ไปถูกโฟลเดอร์
    $filePath = Yii::getAlias("@webroot/uploads/nhso_opd/$visit/" . $fileName);
    ...
    if (file_exists($filePath)) {
        ...
        throw new \yii\web\NotFoundHttpException('ไม่พบไฟล์ที่ต้องการ: ' . $fileName);
    }
}
 ############################################################################################################   
