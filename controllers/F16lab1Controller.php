...
class F16lab1Controller extends \yii\web\Controller
{
     public function actionIndex()
{
    // ── 1. รับค่าวันที่และ statusFilter ─────────────────────
    $request = Yii::$app->request;

    $date1x       = $request->post('date1',        $request->get('date1',        date('Y-m-d', strtotime('-5 days'))));
    $date2x       = $request->post('date2',        $request->get('date2',        date('Y-m-d')));
    $statusFilter = $request->post('statusFilter', $request->get('statusFilter', 'all'));

    $date1_sql = $date1x . ' 00:00:00';
    $date2_sql = $date2x . ' 23:59:59';

    // ── 2. Query หลัก (ตรงตาม query ที่ใช้งานได้) ───────────
    $sqlData = "SELECT
        @n := @n + 1 AS 'No',
        data.*
    FROM (
        SELECT
            ...
            log.d_update,
			log.users,
            IFNULL(cv.stm_claim,     0.00) AS amount,
            IFNULL(cv.ret_statement, 0.00) AS ret_statement,
        ...
        LEFT  JOIN prescriptions ps   ON ps.visit_id = o.visit_id  AND ps.is_cancel = 0
                                      AND ps.drug_id IN ('2280','0664','0263','0491','2358','0262')
        ...
        LEFT  JOIN uc_inscl g         ON c.CID = g.CID
                                      ...
                                      AND TRIM(g.hospmain) <> ''
        LEFT  JOIN hosp_sss h         ON c.CID = h.CID
                                      AND (h.date_abort > DATE(o.REG_DATETIME) OR DAY(h.DATE_ABORT) = 0)
                                      AND TRIM(h.HOSP_ID) <> ''
        ...
        WHERE o.IS_CANCEL = 0
            ...
            AND o.visit_id NOT IN (
                SELECT ipd_reg.visit_id FROM ipd_reg WHERE ipd_reg.is_cancel = 0
            )
        GROUP BY o.VISIT_ID
    ) AS data,
    (SELECT @n := 0) AS init
    ...
    public function actionData()
{
    ...
    if (empty($vn)) {
        Yii::$app->session->setFlash('error', 'ไม่มีข้อมูลที่เลือก');
    ...
    foreach ($vn as $r) {
        ...
        // ✅ ตรวจสอบค่า visit และ hn
        if (empty($visit) || empty($hn)) {
            Yii::warning("Invalid visit or hn from value: $r", __METHOD__);
            continue;
        }
        ...
        foreach ($tables as $table) {
            try {
                ...
                $query = "SELECT main_query FROM fdh_lab1 WHERE main_table = :table";
                $results = $db14->createCommand($query)
                ...
                foreach ($results as $result) {
                    ...
                    Yii::info("Running query for table [$table]: $mainQueryResult", __METHOD__);
                    ...
                    try {
                        ...
                        // ✅ log error ของ main query แต่ไม่หยุดทำงาน
                        Yii::error(
                            "Query failed for table [$table], visit [$visit]: "
                            . $mainQueryResult
                            . " | Error: " . $e->getMessage(),
                            __METHOD__
                        );
                        Yii::$app->session->setFlash('warning',
                            "ตาราง $table visit $visit เกิดข้อผิดพลาด: " . $e->getMessage()
                        );
                        continue; // ข้ามแค่ query นี้ ไม่หยุดทั้งหมด
                    }
                }

            } catch (\Exception $e) {
                // ✅ log error ของ fdh_lab1 query
                Yii::error(
                    "Failed to fetch main_query for table [$table]: " . $e->getMessage(),
                    __METHOD__
                );
                continue;
            }
    ...
    // ✅ ตรวจสอบว่ามีข้อมูลก่อน export
    if (empty($fileData)) {
        Yii::$app->session->setFlash('error', 'ไม่พบข้อมูลสำหรับ export');
        return $this->redirect(['index']);
    }

    // ✅ ตรวจสอบ visit และ hn ก่อนใช้งานต่อ
    if (empty($visit) || empty($hn)) {
        Yii::$app->session->setFlash('error', 'ไม่พบข้อมูล visit หรือ HN');
        return $this->redirect(['index']);
    }
    ...
    foreach ($fileData as $table => $data) {
        try {
            ...
            // Remove duplicates and convert to Windows CRLF for PAT.txt
            if ($table === 'pat') {
            ...
            Yii::error(
                "Failed to export table [$table]: " . $e->getMessage(),
                __METHOD__
            );
            Yii::$app->session->setFlash('warning',
                "Export ตาราง $table ล้มเหลว: " . $e->getMessage()
            );
        }
    }
    ...
    try {
        ...
        Yii::error("sendDataToAPI failed: " . $e->getMessage(), __METHOD__);
        Yii::$app->session->setFlash('error',
            'ส่งข้อมูลไปยัง API ล้มเหลว: ' . $e->getMessage()
        );
    }
                ...
                $visitCount = Yii::$app->db2->createCommand($logSQLCheck)
                    ...
                    if ($visitCount > 0) {
                        ...
                        $logSQLUpdate = "UPDATE log_fdh_opd_ck SET pid = :pid, messages = :messages, messagecode = :messagecode, response = :response,
                         users = 'lab1', datereg = :datereg, d_update = NOW() WHERE visit_id = :visit_id";
                        ...
                        VALUES (:visit_id, :pid, :messages, :messagecode, :response, 'lab1',:datereg ,NOW())";
...
###########################################################################################################
	...
	public function actionExports()
    {
        ...
        $zipFilename = $baseDirectory . 'F16_10953_lab1_' . $currentDateTime . '.zip';
