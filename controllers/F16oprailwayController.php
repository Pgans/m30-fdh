...
class F16oprailwayController extends \yii\web\Controller
{
...
public function actionData()
{
    ...
    try {
        ...
        foreach ($tables as $table) {
            $results = $db2->createCommand("SELECT main_query FROM fdh_oprailway WHERE main_table = :table")
...
private function sendToNhsoApi($payload, $visits = [], $hn = null)
{
    ...
    foreach ($visits as $visit) {
        $messages = 'ส่งข้อมูล OP-SRT สำเร็จ';
        $messagecode = '200';
        ...
        if ($visitCount > 0) {
            // อัปเดต
            $logSQLUpdate = "UPDATE log_fdh_opd_ck SET pid = :pid, messages = :messages, messagecode = :messagecode, response = :response, users = 'opsrt', d_update = NOW() WHERE visit_id = :visit_id";
            ...
            $logSQLInsert = "INSERT INTO log_fdh_opd_ck (visit_id, pid, messages, messagecode, response, users, d_update)
                             VALUES (:visit_id, :pid, :messages, :messagecode, :response, 'opsrt', NOW())";
...
public function actionExports()
    {
        ...
        $zipFilename = $baseDirectory . 'F16_10953_OP_SRT_' . $currentDateTime . '.zip';
