...
class F16homewardController extends \yii\web\Controller
{
    public function actionIndex()
    {
        ...
        AND ir.dsc_dt <> '0'
        #and ir.dsc_dt BETWEEN DATE_SUB(NOW(),INTERVAL 5 DAY)  AND NOW()
        AND o.INSCL in ('03','04','33','00','23')
        AND ir.WARD_NO = '50'
        #AND o.visit_id  not in (SELECT vs.visit_id from log_all.log_fdh_ipd vs )
        GROUP BY o.VISIT_ID
        ORDER BY ir.adm_id           
        ) AS data,
          (SELECT @n := 0) AS init
        ...
        WHERE  v.users = 'homeward' AND v.messagecode <> ''
        ...
        AND l.messagecode <> ' ' AND l.users = 'homeword'
        ...
        FROM log_fdh_ipd_ck l 
        WHERE l.d_update BETWEEN DATE_SUB(NOW(), INTERVAL 1 DAY) AND NOW()
        AND l.messagecode <> 'success' AND l.users = 'homeward'
        ORDER BY l.d_update DESC
        ...
        foreach ($vn as $r) {
           ...
           // echo $an;
            ...
            foreach ($tables as $table) {
                $query = "SELECT main_query FROM fdh_homeward WHERE main_table = '$table'";
    ...
    private function sendDataToAPI($visit, $an)
    {
        ...
        $sqltoken = "SELECT MAX(token) as token30 FROM fdh_token WHERE staff_id = 'eddy'";
        ...
        if ($data && isset($data['token30'])) {
            ...
            $statusMessageTh = isset($responseDecoded['data'][0]['status_message_th']) ? $responseDecoded['data'][0]['status_message_th'] : '';
             ...
             if ($visitCount > 0) {
                 ...
                 $logSQLUpdate = "UPDATE log_fdh_ipd_ck SET pid = :pid, messages = :messages, messagecode = :messagecode, response = :response, users = 'homeward', d_update = NOW() WHERE visit_id = :visit_id AND an = :an";
                 ...
                 $logSQLInsert = "INSERT INTO log_fdh_ipd_ck (visit_id, pid, an, messages, messagecode, response, users, d_update) VALUES (:visit_id, :pid, :an, :messages, :messagecode, :response, 'homeward', NOW())";
...
###################################################################################################
    ...
    public function actionRunCurl()
    {
        ...
        curl_setopt_array($curl, array(
            ...
            CURLOPT_POSTFIELDS => json_encode([
                'user' => 'jeerapong.10953',
                'password_hash' => '9ED2F0D3A9BC7F52CC028816AEFFCF103FC7965B634783B12FC7A863020BC422',
                'hospital_code' => '10953'
            ]),
            CURLOPT_HTTPHEADER => [
                'Content-Type: application/json',
        ...
        try {
            ...
            Yii::$app->session->setFlash('success', 'New token สร้างสำเร็จ jeerapong');
        } catch (Exception $e) {
	 ...
	 public function actionExports()
    {
        ...
        $zipFilename = $baseDirectory . 'F16_10953_IpNormal_' . $currentDateTime . '.zip';
