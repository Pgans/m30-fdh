...
class F16herbController extends \yii\web\Controller
{
    public function actionIndex()
    {
       ...
       , CONCAT(
          ...
          LEFT JOIN lab_lists l ON l.lab_id = lr.lab_id 
					INNER JOIN prescriptions ps on ps.visit_id = o.visit_id AND ps.is_cancel = 0 AND ps.drug_id in ('2280','0664','0263','0491','2358','0262','2443','2503') AND ps.rx_amount <> '0'
          ...
          WHERE o.IS_CANCEL = 0
				AND o.REG_DATETIME BETWEEN '2025-05-01 00:01' AND '2025-05-10 23:59'
        ...
        WHERE  v.users = 'hurb' AND v.messagecode <> ''
		#AND v.messages <> ''
        ...
        AND l.messagecode <> '' AND l.users = 'hurb'
   ...
   public function actionData()
{
    ...
    if ($table === 'pat') {
        ...
        if (empty($results)) {
            $queryHerb = "SELECT main_query FROM fdh_hurb WHERE main_table = :table";
    ...
    foreach ($results as $result) {
        ...
        try {
            ...
            Yii::error("Query error in $table: " . $e->getMessage());
            continue;
        }
        ...
        $fileData[$table] = array_merge($fileData[$table], $data);
            ...
            // ตรวจสอบหากไม่มีข้อมูลในแฟ้ม
            if (empty($fileData[$table])) {
                $emptyTables[] = $table;
            }
        }

        // ถ้ามีแฟ้มที่ว่าง ให้แจ้งเตือนและไม่ดำเนินการส่งข้อมูล
            ...
            while ($row = fgetcsv($file, 0, "|")) {
                ...
                if ($visitCount > 0) {
                        ...
                        $logSQLUpdate = "UPDATE log_fdh_opd_ck SET pid = :pid, messages = :messages, messagecode = :messagecode, response = :response,
                         users = 'hurb', datereg = :datereg, d_update = NOW() WHERE visit_id = :visit_id";
                        ...
                        VALUES (:visit_id, :pid, :messages, :messagecode, :response, 'hurb',:datereg ,NOW())";
