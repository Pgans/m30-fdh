...
class F16opthaimed32Controller extends \yii\web\Controller
{
...
(SELECT DATE_FORMAT(o.reg_datetime, '%Y-%m-%d %H:%i') as 'regdate'
       ...
       , CONCAT(
          ...
          LEFT(GROUP_CONCAT(DISTINCT TRIM(icd.ICD10_TM)), 30) AS Diag,
					...
					GROUP_CONCAT(DISTINCT dt.tmt) as tmt,
                    dr.drug_name,
          ...
          FROM opd_visits o 
			...
			LEFT JOIN opd_diagnosis d1 ON d1.visit_id = o.visit_id AND d1.dxt_id = 1 AND d1.is_cancel = 0
			...
			-- ยา
			LEFT JOIN prescriptions pr ON o.visit_id = pr.visit_id AND pr.rx_amount <> '0'
			LEFT JOIN drugs dr ON dr.drug_id = pr.drug_id
			LEFT JOIN drug_thamed32 dt ON pr.drug_id = dt.drug_code  -- 🔗 เชื่อมเพื่อใช้ TMT
			...
			LEFT JOIN authen_kiosk ak ON p.CID = ak.cid 
				AND DATE(o.REG_DATETIME) = DATE(ak.d_update)
			...
			WHERE o.IS_CANCEL = 0
			  ...
			  AND dt.drug_code IS NOT NULL  -- ✅ ตรวจสอบว่า match กับ drug_thamed32 แล้ว
      ...
      ORDER BY 
        ...
        WHERE  v.users = 'thaimed32' AND v.messagecode <> ''
		#AND v.messages <> ''
        ...
        AND l.messagecode <> '' AND l.users = 'thaimed'
        ...
        AND l.messagecode <> 'success' AND l.users = 'thaimed32'
        ORDER BY l.d_update DESC
    ...
    public function actionData()
    {
        ...
        foreach ($vn as $r) {
            ...
            $tables = ['ins', 'odx', 'opd', 'pat', 'cha', 'cht','dru'];
            foreach ($tables as $table) {
                $query = "SELECT main_query FROM fdh_thaimed32 WHERE main_table = '$table'";
                ...
                while ($row = fgetcsv($file, 0, "|")) {
                    ...
                    if ($visitCount > 0) {
                            ...
                            $logSQLUpdate = "UPDATE log_fdh_opd_ck SET pid = :pid, messages = :messages, messagecode = :messagecode, response = :response,
                             users = 'thaimed32', datereg = :datereg, d_update = NOW() WHERE visit_id = :visit_id";
                            ...
                            VALUES (:visit_id, :pid, :messages, :messagecode, :response, 'thaimed',:datereg ,NOW())";
    ...
    public function actionExports()
        {
            ...
            $zipFilename = $baseDirectory . 'F16_10953_thaimed_' . $currentDateTime . '.zip';
