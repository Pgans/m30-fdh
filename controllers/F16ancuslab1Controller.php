...
class F16ancuslab1Controller extends \yii\web\Controller
{
...
/*
        foreach ($tables as $table) {
            $query = "SELECT main_query FROM fdh_anc_us_lab1 WHERE main_table = '$table'";
...
##################################################################
    ...
    if ($table === 'pat') {
        ...
        // ถ้าไม่มีข้อมูลใน fdh_ins ให้ใช้จาก fdh_anc_us_lab1
        if (empty($results)) {
            $queryHerb = "SELECT main_query FROM fdh_anc_us_lab1 WHERE main_table = :table";
            ...
            while ($row = fgetcsv($file, 0, "|")) {
                ...
                if ($visitCount > 0) {
                        ...
                        $logSQLUpdate = "UPDATE log_fdh_opd_ck SET pid = :pid, messages = :messages, messagecode = :messagecode, response = :response,
                         users = 'ancuslab1', datereg = :datereg, d_update = NOW() WHERE visit_id = :visit_id";
                        ...
                        VALUES (:visit_id, :pid, :messages, :messagecode, :response, 'ancuslab1',:datereg ,NOW())";
