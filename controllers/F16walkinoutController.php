...
class F16walkinoutController extends \yii\web\Controller
{
...
(SELECT 
       ...
       , CONCAT(
          ...
          LEFT(GROUP_CONCAT(DISTINCT TRIM(icd.ICD10_TM)), 30) AS Diag,
					...
					g.hospmain, left(hpt.hosp_name,40) as hospname,
					log.messagecode,
		  ...
		  LEFT JOIN receipts re ON re.visit_id = o.visit_id AND re.is_cancel = 0
					...
					INNER JOIN hospitals hpt on hpt.hosp_id=g.hospmain AND hpt.is_ubon = 0
          WHERE o.IS_CANCEL = 0
        ...
        WHERE  v.users = 'walkinout' AND v.messagecode <> ''
		#AND v.messages <> ''
        ...
        AND l.messagecode <> '' AND l.users = 'walkinout'
        ...
        AND l.messagecode <> 'success' AND l.users = 'walkinout'
        ORDER BY l.d_update DESC
  ...
  public function actionData()
{
    ...
    if (empty($vn)) {
        Yii::$app->session->setFlash('error', "❌ ไม่พบข้อมูลที่เลือก");
    ...
    $tables = ['adp', 'ins', 'odx', 'opd', 'pat', 'cha', 'cht', 'oop', 'aer', 'orf', 'dru'];
    $tablesToCheck = ['adp', 'ins', 'pat', 'opd', 'odx', 'cha', 'cht']; // เฉพาะแฟ้มที่ต้องเช็ค

    foreach ($vn as $r) {
        if (strlen($r) < 10) {
            Yii::$app->session->setFlash('error', "❌ ข้อมูล visit ไม่ถูกต้อง");
            return $this->redirect(['index']);
        }
        ...
        if (empty($hn) || empty($visit)) {
            Yii::$app->session->setFlash('error', "❌ ค่า HN หรือ Visit ว่าง");
            return $this->redirect(['index']);
        }
        ...
        foreach ($tables as $table) {
             $query = "SELECT main_query FROM fdh_walkinout WHERE main_table = '$table'          
			 ";
            $results = $db->createCommand($query)->bindValue(':table', $table)->queryAll();

            if (empty($results)) {
                Yii::info("ไม่พบ main_query สำหรับตาราง: $table", __METHOD__);
                continue;
            }
            ...
            // ตรวจสอบเฉพาะตารางที่อยู่ใน $tablesToCheck
            if (in_array($table, $tablesToCheck, true) && empty($fileData[$table])) {
                $emptyTables[] = $table;
            }
        }

        // แจ้งเตือนเฉพาะกรณีที่ตารางที่ต้องเช็คไม่มีข้อมูล
                ...
                while ($row = fgetcsv($file, 0, "|")) {
                    ...
                    if ($visitCount > 0) {
                            ...
                            $logSQLUpdate = "UPDATE log_fdh_opd_ck SET pid = :pid, messages = :messages, messagecode = :messagecode, response = :response,
                             users = 'walkinout', datereg = :datereg, d_update = NOW() WHERE visit_id = :visit_id";
                            ...
                            VALUES (:visit_id, :pid, :messages, :messagecode, :response, 'walkinout',:datereg ,NOW())";
    ...
    public function actionExports()
        {
            ...
            $zipFilename = $baseDirectory . 'F16_10953_ฝังยาคุม-upt_' . $currentDateTime . '.zip';
