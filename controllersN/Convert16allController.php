...
class Convert16allController extends \yii\web\Controller
{
    ...
    {
        ########################################################################################################
        $sqlData = "select l.id, l.visit_id, l.pid, l.`status`, l.messagecode, l.response, l.users, l.d_update
        FROM log_dm l 
        WHERE l.d_update BETWEEN CURDATE() AND NOW() AND l.messagecode <> 'success'
        ORDER BY l.d_update DESC
            ";
            $rawData = \Yii::$app->db143->createCommand($sqlData)->queryAll();
    ...
    {
        try {
            $command = Yii::$app->db16->createCommand("CALL all_ucs_ncd_screen()");
            $rowsAffected = $command->execute();

            Yii::$app->session->setFlash('success', "เพิ่มเติมข้อมูลในตาราง All_ucs_ncd สำเร็จ. จำนวน: $rowsAffected");
        } catch (\Exception $e) {
            Yii::$app->session->setFlash('error', 'Error executing stored procedure: ' . $e->getMessage());
        }
    ...
    public function actionUpdate()
    {
        ...
        try {
            ...
            $sql400 = "INSERT INTO adp (hn, an, dateopd, type, code, qty, rate, seq, cagcode, dose, ca_type, serialno, totcopay, use_status, total)
                ...
                '100.00' AS total
            FROM
                all_ucs_ncd_screen n
            WHERE
                n.symptoms LIKE ('%อายุ15-34 ปี%')
                AND n.screen_date BETWEEN '$date1' AND '$date2' 
...
$commandDelete->execute();
    ...
    odx: ' . $totalodx);
            ...
            $transaction->commit();
            return $this->redirect(['convert16all/index']);
        } catch (\Exception $e) {
    ...
    public function actionExports()
    {
      ...
      from adp ";
        ...
        $zipFilename = $baseDirectory . 'F16_10953_ALL_UCS' . $currentDateTime . '.zip';
        $zip = new ZipArchive();
