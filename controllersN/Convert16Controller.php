...
class Convert16Controller extends \yii\web\Controller
{
    ...
    public function actionUpdate()
    {
        ...
        try {
            // Build and execute the first SQL statement
            $sql1 = "DELETE FROM pat WHERE hn is null";
            $command1 = $db->createCommand($sql1);
            $command1->execute();

            // Build and execute the second SQL statement
            $sql2 = "UPDATE pat SET hcode = '10953' WHERE  hcode = '99809'";
            $command2 = $db->createCommand($sql2);
            // $command2->bindValue(':value2', $value2);
            $command2->execute();

            $sql3 = "UPDATE pat SET nation = '099' WHERE nation != '099' ";
            $command3 = $db->createCommand($sql3);
            $command3->execute();

            $sql4 = "UPDATE pat SET hn =(select hn FROM mathhn WHERE pat.person_id = mathhn.cid )";
            $command4 = $db->createCommand($sql4);
            $command4->execute();

            $sql5 = "ALTER TABLE opd ADD COLUMN pid  varchar(255) NULL AFTER hn ";
            $command5 = $db->createCommand($sql5);
            $command5->execute();

            $sql6 = "UPDATE opd SET pid = hn";
            $command6 = $db->createCommand($sql6);
            $command6->execute();

            $sql7 = "UPDATE opd INNER JOIN mathhn ON opd.pid = mathhn.pid SET opd.hn = mathhn.hn";
            $command7 = $db->createCommand($sql7);
            $command7->execute();

            $sql8 = "UPDATE opd SET clinic = '01500' WHERE hn != '' ";
            $command8 = $db->createCommand($sql8);
            $command8->execute();

            $sql9 = "ALTER TABLE opd DROP COLUMN pid ";
            $command9 = $db->createCommand($sql9);
            $command9->execute();
            $sql99 = "DELETE FROM opd WHERE detail NOT LIKE ('%มารับบริการตรวจคัดกรอง%') ";
            $command99 = $db->createCommand($sql99);
            $command99->execute();
            ################### CHA #############################
            $sql15 = "UPDATE cha INNER JOIN mathhn ON mathhn.cid = cha.person_id SET cha.hn = mathhn.hn ";
            $command15 = $db->createCommand($sql15);
            $command15->execute();
            ################### CHT #############################
            $sql16 = "UPDATE cht INNER JOIN mathhn ON mathhn.cid = cht.person_id SET cht.hn = mathhn.hn ";
            $command16 = $db->createCommand($sql16);
            $command16->execute();
            #####################################################
            ################### DRU #############################
            $sql17 = "UPDATE dru INNER JOIN mathhn ON mathhn.cid = dru.person_id SET dru.hn = mathhn.hn ";
            $command17 = $db->createCommand($sql17);
            $command17->execute();
            $sql18 = "UPDATE dru SET hcode = '10953' WHERE  hcode = '99809' ";
            $command18 = $db->createCommand($sql18);
            $command18->execute();
            $sql19 = "UPDATE dru SET clinic = '01500' WHERE hn != '' ";
            $command19 = $db->createCommand($sql19);
            $command19->execute();
            #####################################################
            ################### INS #############################
            $sql20 = "UPDATE ins INNER JOIN mathhn ON ins.cid = mathhn.cid SET ins.hn = mathhn.hn ";
            $command20 = $db->createCommand($sql20);
            $command20->execute();
            $sql201 = "UPDATE ins SET hcode = '10953' WHERE  hcode = '99809' ";
            $command201 = $db->createCommand($sql201);
            $command201->execute();
            #####################################################
            ################### ODX #############################
            $sql21 = "UPDATE odx INNER JOIN mathhn ON mathhn.cid = odx.person_id SET odx.hn = mathhn.hn ";
            $command21 = $db->createCommand($sql21);
            $command21->execute();
            $sql22 = "UPDATE odx SET clinic = '015' WHERE hn != '' ";
            $command22 = $db->createCommand($sql22);
            $command22->execute();
            $sql220 = "UPDATE  odx  SET drdx = '33744' WHERE drdx = '4711183066' ";
            $command220 = $db->createCommand($sql220);
            $command220->execute();
            $sql221 = "UPDATE  odx  SET drdx = '28234' WHERE drdx = '33744' ";
            $command221 = $db->createCommand($sql221);
            $command221->execute();
            $sql222 = "UPDATE  odx  SET drdx = '28234' WHERE drdx = '4911048362' ";
            $command222 = $db->createCommand($sql222);
            $command222->execute();
            /*
            $sql223 = "DELETE FROM odx WHERE LEFT(diag, 1) = 'R' ";
            $command223 = $db->createCommand($sql223);
            $command223->execute();
            */
            #####################################################
            ################### OOP #############################
            $sql23 = "UPDATE oop INNER JOIN mathhn ON mathhn.cid = oop.person_id SET oop.hn = mathhn.hn ";
            $command23 = $db->createCommand($sql23);
            $command23->execute();
            $sql24 = "UPDATE oop SET clinic = '01500' WHERE hn != ''  ";
            $command24 = $db->createCommand($sql24);
            $command24->execute();
            $sql241 = "UPDATE  oop  SET dropid = '33744' WHERE dropid = '4711183066' ";
            $command241 = $db->createCommand($sql241);
            $command241->execute();
            $sql242 = "UPDATE  oop  SET dropid = '28234' WHERE dropid = '33744' ";
            $command242 = $db->createCommand($sql242);
            $command242->execute();
            $sql243 = "UPDATE  oop  SET dropid = '28234' WHERE dropid = '4911048362' ";
            $command243 = $db->createCommand($sql243);
            $command243->execute();
            #####################################################
            ################### ORF #############################
            $sql25 = "UPDATE orf INNER JOIN opd ON orf.seq = opd.seq SET orf.hn = opd.hn ";
            $command25 = $db->createCommand($sql25);
            $command25->execute();
            $sql26 = "UPDATE orf SET clinic = '01500' WHERE hn != ''  ";
            $command26 = $db->createCommand($sql26);
            $command26->execute();
            #####################################################
            ################### LABFU #############################
            $sql27 = "UPDATE labfu INNER JOIN mathhn ON mathhn.cid = labfu.person_id SET labfu.hn = mathhn.hn";
            $command27 = $db->createCommand($sql27);
            $command27->execute();
            $sql271 = "UPDATE labfu SET hcode = '10953' WHERE  hcode = '99809'  ";
            $command271 = $db->createCommand($sql271);
            $command271->execute();
            #####################################################
            #############  ADP ###########################
            ...
            '4' AS type,
            CASE
                WHEN o.detail LIKE ('%อายุ15-34 ปี%') THEN '12001'
                WHEN o.detail LIKE ('%อายุ35-59 ปี%') THEN '12002'
                WHEN s.bsl <> '' THEN '12003'
            END AS code,
            '1' AS qty,
            CASE
                WHEN o.detail LIKE ('%อายุ15-34 ปี%') THEN '100.00'
                WHEN o.detail LIKE ('%อายุ35-59 ปี%') THEN '150.00'
                WHEN s.bsl <> '' AND s.age_year BETWEEN '35' AND '59' THEN '40.00'
            END AS rate,
            o.seq,
            '' AS cagcode,
            '' AS dose,
            ...
            END AS total
            FROM opd o
            INNER JOIN pat p ON o.hn = p.hn
            LEFT JOIN ins i ON o.seq = o.seq
            LEFT JOIN sheet s ON s.cid = p.person_id AND s.age_year < '60'
            WHERE
            (CASE
                WHEN o.detail LIKE ('%อายุ15-34 ปี%') THEN '100.00'
                WHEN o.detail LIKE ('%อายุ35-59 ปี%') THEN '150.00'
            ...
            /*
            $sql10 = "DELETE FROM adp WHERE hn is null ";
            $command10 = $db->createCommand($sql10);
            $command10->execute();
            $sql11 = "ALTER TABLE adp  ADD COLUMN  pid  varchar(255) NULL AFTER hn ";
            $command11 = $db->createCommand($sql11);
            $command11->execute();
            $sql12 = "UPDATE adp SET pid = hn ";
            $command12 = $db->createCommand($sql12);
            $command12->execute();
            $sql13 = "UPDATE adp INNER JOIN mathhn ON mathhn.pid = adp.pid SET adp.hn = mathhn.hn ";
            $command13 = $db->createCommand($sql13);
            $command13->execute();
            $sql14 = "ALTER TABLE adp DROP COLUMN pid ";
            $command14 = $db->createCommand($sql14);
            $command14->execute();
            $sql140 = "UPDATE adp SET clinic = '01500' WHERE hn != ''  ";
            $command140 = $db->createCommand($sql140);
            $command140->execute();
            */
    ...
    public function actionExports()
    {
      ...
      from adp ";
        ...
        $this->exportToTextFile(
            $results2,
            $baseDirectory . 'CHA.txt',
            ['HN', 'AN', 'DATE', 'CHRGITEM', 'AMOUNT', 'PERSON_ID', 'SEQ']
        );

        $this->exportToTextFile(
            $results3,
            $baseDirectory . 'CHT.txt',
            ['HN', 'AN', 'DATE', 'TOTAL', 'PAID', 'PTTYPE', 'PERSON_ID', 'SEQ', 'OPD_MEMO', 'INVOICE_NO', 'INVOICE_LT']
        );
        ...
        $zipFilename = $baseDirectory . 'F16_10953_' . $currentDateTime . '.zip';
        //$zipFilename = $baseDirectory . '_F16_10953.zip';
        $zip = new ZipArchive();
