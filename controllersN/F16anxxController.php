    ...
    public function actionSendx()
{
    ...
    if ($data && isset($data['token30'])) {
        ...
        $file_paths = array(
          //  $file_path = $_SERVER['DOCUMENT_ROOT'] . '/uploads/F16_claim/INS.txt';

            Yii::getAlias('@webroot') . 'uploads/F16_claim/INS.txt',
            Yii::getAlias('@webroot') . 'uploads/F16_claim/PAT.txt'
        );

        // Initialize the post data array
        $post_data = array(
            "type" => "txt",
            "file" => Yii::getAlias('@webroot') . 'uploads/F16_claim/INS.txt',
            "file" => Yii::getAlias('@webroot') . 'uploads/F16_claim/PAT.txt',
        );
        // แสดงข้อมูลใน $post_data
    echo "Post Data:\n";
    print_r($post_data);
    echo "\n\n";
        ...
        curl_setopt_array($curl, array(
            CURLOPT_URL => 'https://uat-fdh.inet.co.th/api/v2/data_hub/16_files/',
...
public function actionSend()
{
    ...
    if ($data && isset($data['token30'])) {
        ...
        $file_paths = [
            __DIR__ . '/../web/uploads/F16_claim/PAT.txt', // Adjust paths as needed
