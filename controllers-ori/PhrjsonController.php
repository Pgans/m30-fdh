...
class PhrjsonController extends \yii\web\Controller
       ...
       , CONCAT(
        ...
        ,left(e.unit_name,10) 'unit_name', 
         p.TELEPHONE as 'Telephone'
          ...
          WHERE b.IS_CANCEL = 0 
          #AND b.REG_DATETIME BETWEEN SUBDATE(CURDATE(), INTERVAL 30 DAY) AND CURDATE()
          ...
          AND b.REG_DATETIME BETWEEN '2022-01-01 00:01' AND '2022-06-30 23:59'
    ...
    FROM
        log_phr v
     ORDER BY v.d_update DESC  LIMIT 10
        ";
    ...
    WHERE 
        c.hn = '120842'  AND LEFT(p.cid, 5) <> '00000'
";
...
LEFT JOIN hospitals hs1 ON hs1.hosp_id = uc.hospsub
   where o.visit_id = '0002967856'";
       ...
       ,o.BP_DIAS as bp_diastolic
        ...
        WHERE o.visit_id='0002967856'
        ";
          $vitalSignsData = \yii::$app->db14->createCommand($strVituals )->queryAll();
               $vitalSigns = [];
               foreach ($vitalSignsData as $vitalSignsRow) {
                   $vitalSign = [
                       "body_weight" => [
                           "status" => "final",
                           "valueQuantity" => [
                               "value" => $vitalSignsRow["bw"],  // Replace with the actual column containing body weight
                               "unit" => "kg"
                           ]
                       ],
                       "body_height" => [
                           "status" => "final",
                           "valueQuantity" => [
                               "value" => $vitalSignsRow["height"],  // Replace with the actual column containing body height
                               "unit" => "cm"
                           ]
                       ],
                       "body_temp" => [
                           "status" => "final",
                           "valueQuantity" => [
                               "value" => $vitalSignsRow["tt"],  // Replace with the actual column containing body temperature
                               "unit" => "cel"
                           ]
                       ],
                       "bp_systolic" => [
                           "status" => "final",
                           "valueQuantity" => [
                               "value" => $vitalSignsRow["sbp"],  // Replace with the actual column containing systolic blood pressure
                               "unit" => "mmHg"
                           ],
                           "interpretation" => [],
                   ],
                       "bp_diastolic" => [
                           "status" => "final",
                           "valueQuantity" => [
                               "value" => $vitalSignsRow["bp_diastolic"],  // Replace with the actual column containing diastolic blood pressure
                               "unit" => "mmHg"
                           ],
                           "interpretation" => [
    ...
    $encounter["Medication"] = empty($medications) ? [] : $medications;
           ...
           ,'แพทย์' as participant_type
                   ...
                   #GROUP BY o.visit_id
            ";
            $dataAppointment = \yii::$app->db14->createCommand($strAppointment)->queryAll();
...
curl_close($curl);
        ...
        if (strlen($response) > 0) {
            $strSQL = "REPLACE INTO log_phr (visit_id, cid , pid,status, messagecode ,response , users, regdate,d_update) VALUES ('$visit','$cid','$hn','$ok200','$ok','$result' ,'api70','$regdate',NOW())";
