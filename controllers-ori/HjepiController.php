    ...
    ##################################################################     
      ...
      //\Yii::$app->response->format = \yii\web\Response::FORMAT_JSON;
       ...
       foreach ($vn  as $r) {
        ...
        #CURLOPT_URL => '192.168.200.63:30014/claimdmht/exportEpi',  192.168.200.92:30012/claimdmht/exportEpi
		CURLOPT_URL => '192.168.200.92:30012/claimdmht/exportEpi',  
...
############################INSERT TABLE Log_dmht#############################
         ...
         if (strlen($response) > 0 ) {
               $strSQL = "REPLACE INTO log_dt (visit_id, pid,status, messagecode ,response , users,d_update) VALUES ('$visit_id','$hn',$status,'$message','$message_th' ,'hjepi',NOW())";
    ...
    WHERE 
        ...
        AND DATE_FORMAT( visitepi.dateepi, '%Y-%m-%d' ) BETWEEN '2023-07-01'  AND NOW()
        ...
        ";
         ...
         $data = \yii::$app->db142->createCommand($total)->queryAll();
          ...
          public function actionLoghpv()  {
            ...
            FROM log_dt l 
            INNER JOIN mbase_data1.cid_hn c ON c.hn = l.pid
            ...
            return $this->render('log_hpv', [
                ...
                public function actionLogerr()  {
                    ...
                    AND l.users = 'hjepi' AND l.messagecode <> 'success'
                    ORDER BY l.d_update DESC
                    ";
                    $rawData = \yii::$app->db142->createCommand($sql)->queryAll();
                    try {
