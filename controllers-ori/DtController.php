    ...
    public function actionDeleteError($visit_id)
{
    try {
        ...
        // ดำเนินการลบข้อมูล
        $rowsAffected = Yii::$app->db143->createCommand()->delete('log_dt', ['visit_id' => $visit_id])->execute();
...
{
    try {
        $command = Yii::$app->db143->createCommand("DELETE FROM log_dt  WHERE (status <> '200' OR messagecode <> 'OK') AND status <> '400'");
    ...
    return $this->redirect(['dt']); // Replace 'index' with the actual route you want to redirect to.
}
}
