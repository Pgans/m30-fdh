...
##########################################################################
    ...
    public function actionD506()
    {
        ...
        FROM log_epidem l 
        WHERE l.d_update BETWEEN CURDATE() AND NOW() AND l.messagecode = '200'
...
//echo $json;
 ...
 c506.code506 as'epidem_report_group_code',
 ...
 i.ICD10_TM  as 'principal_diagnosis_icd10' ,
 i2.ICD10_TM as 'diagnosis_icd10_list',
'' as 'organism' ,
'' as 'complication',
 CASE
...
LEFT  JOIN code506 c506 ON i.ICD10_TM BETWEEN c506.code_min AND c506.code_max
LEFT JOIN opd_diagnosis o2 ON o2.visit_id = a.visit_id AND o2.is_cancel = 0  AND o2.DXT_ID = 1
LEFT JOIN icd10new i2 ON i2.icd10 = o2.icd10  
 WHERE 
 a.visit_id ='$visit'
 #a.visit_id ='0003038676'
AND (SELECT code506.code506 FROM code506 WHERE i.ICD10_TM BETWEEN code506.code_min AND code506.code_max)
GROUP BY a.visit_id
  ";
...
############################INSERT TABLE Log_Epidem#############################
         ...
         if (strlen($response) > 0 ) {
               $strSQL = "REPLACE INTO log_epidem (visit_id, pid,status, messagecode ,response , users,d_update) VALUES ('$visit','$cid','','$messagecode','$response' ,'d506',NOW())";
   ...
   {
       try {
           $command = Yii::$app->db143->createCommand("DELETE FROM log_epidem  WHERE messagecode <> '200'  AND d_update >= CURDATE()-2");
           $rowsAffected = $command->execute();
   
           Yii::$app->session->setFlash('success', "ลบข้อมูลสำเร็จ  จำนวน: $rowsAffected record ถูกลบออกแล้ว");
       } catch (\Exception $e) {
           Yii::$app->session->setFlash('error', 'เกิดข้อผิดพลาดในการลบ: ' . $e->getMessage());
       }
   
       return $this->redirect(['d506']); // Replace 'index' with the actual route you want to redirect to.
   }
